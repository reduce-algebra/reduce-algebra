...
mk_cdiffop(ell_bur,1,{1},1)$
for all sym let ell_bur(sym)=td(sym,t)-td(sym,x,2)-2*u*td(sym,x)-2*u_x*sym$

% This is the list of variables from which the unknown function `ansatz' depends
even_vars:=for i:=0:4 join selectvars(0,i,dep_var,all_parametric_der)$
for each el in even_vars do depend(ansatz,el)$
split_vars:=cde_difflist(all_parametric_der,even_vars)$

total_eq:=ell_bur(ansatz)$
