...
% ansatz: sym=x*(something of degree 1)+t*(something of degree 3)
%  +(something of degree 0)
% NOTE: no terms of degree 1!
...
x*(for each el in grd1 sum (c(ctel:=ctel+1)*el))+
%(for each el in grd2 sum (c(ctel:=ctel+1)*el))+
t*(for each el in grd3 sum (c(ctel:=ctel+1)*el))$
...
%%Q: is it correct to initialize_equations in the following way?
