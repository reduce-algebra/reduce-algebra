% non-local Hamiltonian structures on KdV
% R. Vitolo, 10 March 2010

in "~/local_fermi/Reduce/packages/cdiff/tools21.red"$
in "~/local_fermi/Reduce/packages/cdiff/supervf.red"$
in "~/local_fermi/Reduce/packages/cdiff/integrator.red1"$
in "~/local_fermi/Reduce/packages/cdiff/afk"$
in "~/local_fermi/Reduce/packages/cdiff/losop"$
in "~/local_fermi/Reduce/packages/cdiff/heho"$
in "~/local_fermi/Reduce/packages/cdiff/polynom"$
...
% use this for the first nonlocal Hamiltonian operator ([phi]=3)
%% phi:=
...
%% (for each el in grd1 sum (c(ctel:=ctel+1)*el))*ext 8
%% $

% use this for the second nonl. H. op. [phi]=5
phi:=
(for each el in grd8 sum (c(ctel:=ctel+1)*el))*ext 50+
(for each el in grd5 sum (c(ctel:=ctel+1)*el))*ext 51+
(for each el in grd3 sum (c(ctel:=ctel+1)*el))*ext 52+

(for each el in grd7 sum (c(ctel:=ctel+1)*el))*ext 3+
(for each el in grd6 sum (c(ctel:=ctel+1)*el))*ext 4+
(for each el in grd5 sum (c(ctel:=ctel+1)*el))*ext 5+
(for each el in grd4 sum (c(ctel:=ctel+1)*el))*ext 6+
(for each el in grd3 sum (c(ctel:=ctel+1)*el))*ext 7+
(for each el in grd2 sum (c(ctel:=ctel+1)*el))*ext 8+
(for each el in grd1 sum (c(ctel:=ctel+1)*el))*ext 9+
(for each el in grd0 sum (c(ctel:=ctel+1)*el))*ext 10
$
