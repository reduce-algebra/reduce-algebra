...
them. \texttt{PART(\meta{list},n)}\ttindex{PART} for example will return the
$n^{th}$ element of a list. \texttt{LENGTH}\ttindex{LENGTH} will return the
...
\texttt{SECOND}\ttindex{SECOND} returns the second member of a list.  An error
...
\texttt{REST}\ttindex{REST} returns its argument with the first element
...
The operator \texttt{REVERSE}\ttindex{REVERSE} returns its argument with the
...
switch \sw{LISTARGS},\ttindex{LISTARGS} if on, will globally inhibit
such distribution.  Secondly, one can inhibit this distribution for a
specific operator by the declaration \texttt{LISTARGP}.\ttindex{LISTARGP} For
