...
In input of the form \texttt{x=(a~..~b)}, the parentheses may be omitted.
...
3.5.  The parentheses may be omitted.
Note, that the bounds can be algebraic
...
%% Then the minimum of the sum of absolute squares is located instead.
%% FJW: Currently, this code is commented out.  Not sure why!

However, if only one variable is involved and the starting point is
specified as an interval containing the required solution, then the
...
\item[num\_solve]  $(exp_1, var_1[=val_1~..~val_2][,accuracy=a][,iterations=i])$
...
\texttt{num\_solve} tries to find a zero/solution of the
expression(s).  The result is a list of equations, where the variables
are equated to the coordinates of the result point.

\hypertarget{reserved:JACOBIAN}{}
The Jacobian matrix is stored as a side effect in the shared variable
\texttt{jacobian}\ttindextype[NUMERIC]{jacobian}{shared variable}.
This applies even to a univariate problem, which is treated as a
special case of the multivariate problem (when the Newton method is
used).
