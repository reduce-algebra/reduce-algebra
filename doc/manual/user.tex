...
%
% atensor/*.tex - only atensor.ps -- linked to PDF on web site
% crack/conlaw.tex -- linked to PDF on web site
% crack/crack.tex -- linked to PDF on web site
% crack/v3tools.tex -- linked to PDF on web site
% gentran/gentran.tex -- too long! -- linked to PDF on web site
% mathml/*.tex -- linked to PDFs on web site
% occal/ocpaper.tex
% rtrace/rdebug.tex -- PSL only
% scope/scope.tex -- too long! -- linked to PDF on web site
% susy2/susy2.tex -- linked to PDF on web site
% tri/*.tex -- too long! -- linked to PDF on web site
...
\hyperref[package:CRACK]{\package{CRACK}} to solve the overdetermined
...
\package{CRACK} is a package for solving overdetermined systems of
partial or ordinary differential equations (PDEs, ODEs).  Examples of
programs which make use of \package{CRACK} (finding symmetries of
...
\hyperref[package:APPLYSYM]{\package{APPLYSYM}} package.
...
describing mathematics, primarily within web pages.  It has two
flavours: \emph{Presentation MathML}, like \TeX, is intended for
displaying mathematics, and its meaning can be ambiguous.
\emph{Content MathML}, like \href{https://openmath.org/}{OpenMath}, is
intended for conveying meaning, and how it is displayed is determined
by the display software.

This package allows REDUCE to input and output \emph{content} MathML.

Author: Luis Alvarez-Sobreviela
\index{Alvarez-Sobreviela, Luis}\index{People!Alvarez-Sobreviela, Luis}

\begin{sloppypar}
  Further documentation is available at
  \url{https://reduce-algebra.sourceforge.io/extra-docs/mathml.pdf}.
\end{sloppypar}

\newpage

\section{MATHMLOM: REDUCE OpenMath/MathML Interface}
\indexpackage{MATHMLOM}

This package provides easy to use commands to translate
\href{https://openmath.org/}{OpenMath} into
\href{https://www.w3.org/Math/}{MathML} and vice versa.  Its principal
features are:
\begin{itemize}
\item translation of OpenMath into MathML 1.0 or MathML 2.0;
\item translation of MathML 1.0 and MathML 2.0 into OpenMath;
\item generation of MathML and OpenMath compliant code.
\end{itemize}
...
\begin{sloppypar}
  Further documentation is available at
  \url{https://reduce-algebra.sourceforge.io/extra-docs/mathmlom_user.pdf}.
\end{sloppypar}
...
The topics %% in the tutorial and
in \texttt{sshelp()\$} are:
\begin{itemize}
\item{Purpose}
...
\index{Popowicz, Ziemowit}\index{People!Popowicz, Ziemowit}
...
\section{V3TOOLS: Computations with Polynomials of Products of 3-Vectors}
\indexpackage{V3TOOLS}

This package performs the following computationally expensive and
non-trivial computations with polynomials of products of 3-component
vectors: generation of scalar vector expressions according to a
multiple weighting scheme where each vector has multiple weights;
conversion of scalar vector expressions between different
representations; computation of Poisson brackets between vector
expressions; determining whether a given scalar vector expression is
functionally dependent on a list of other scalar vector expressions.
Possible applications are the classification of integrable `vectorial'
Hamiltonians and the compactification of results for publication.
...
\begin{sloppypar}
  Further documentation is available at
  \url{https://reduce-algebra.sourceforge.io/extra-docs/v3tools.pdf}.
\end{sloppypar}
