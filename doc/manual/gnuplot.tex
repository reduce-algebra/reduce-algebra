...
\item A range for a variable: this has the form
  ...
  However, for 3D plots (only), one or more of the variables
  \texttt{plot\_xrange}, \texttt{plot\_yrange}, \texttt{plot\_zrange}
  can be assigned intervals of the form \texttt{(<lower\_bound>
    ..\ \allowbreak <upper\_bound>)} that set the default ranges for
  the $x$, $y$, $z$ directions.  The $z$ direction corresponds to the
  function values or surface height and setting a $z$-range flattens
  the graph at the range limits, which is especially useful for
...
\item A plot option, either as a fixed keyword, e.g.,
  ...
  Further details are \hyperref[gnuplot:plot-options]{given below}.
\end{itemize}
...
order of the two independent variables is by default apparently
arbitrary.\footnote{In fact, it is determined in a non-obvious way by
the order in which the variables appear in the internal form of the
expression to be plotted.}  This means that in 2D it is apparently
arbitrary which variable is plotted horizontally and which vertically,
and in 3D the coordinate system may be left-handed.  (See the examples
at the end of this section.)  However, if both independent variables
are specified explicitly via options of the form \texttt{<variable> =
  <range>} then their order is preserved: in 2D the first variable is
plotted horizontally and the second vertically; in 3D the first and
second independent variables and the dependent variable form a
...
(Note the left-handed coordinate system; for a right-handed coordinate
system specify $x$ and $y$ ranges in that order.)
