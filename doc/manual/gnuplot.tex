...
the popular {\Gnuplot} graphing utility described at
\url{https://gnuplot.sourceforge.net/}.  It allows you to display
functions in 2D and surfaces in 3D on a variety of output devices
including X terminals, PC monitors, and postscript and {\LaTeX} printer
files.
...
\item A range for a variable: this has the form
  ...
  evaluate to numbers. If no range is specified the default range for
  independent variables is \texttt{(-10~..~10)} and the range for the dependent
  variable is set to the maximum for the {\Gnuplot} executable (using double
  ...
  subdivision parameter overrides the value of the option \texttt{points} for
  this variable.
...
The options below are implemented using the {\Gnuplot} \texttt{set}
(or \texttt{unset} if the option begins with \texttt{no}) command,
which offers far more control than is currently exposed via this
interface.  Note that the {\Gnuplot} \texttt{reset} command, which
clears the effect of most \texttt{set} commands (not \texttt{set term}
or \texttt{set output}) from previous calls of \texttt{plot}, is
always issued before any \texttt{set} commands.  See Commands /
Set-show / \emph{option} and Commands / Reset in the {\Gnuplot}
...
\end{verbatim}
  ...
  \item \texttt{size="s\_x,s\_y"}: rescale the graph (not the window) where
    ...
    are $s_x=1,s_y=1$.  Note that scaling factors greater than 1 will often
...
The option below is implemented using the {\Gnuplot} \texttt{with}
command, which offers far more control than is currently exposed via
this interface.  See Commands / Plot / With in the {\Gnuplot}
documentation or the {\Gnuplot} Help for details.
\begin{itemize}
\item
  \begin{sloppypar}
    ...
    \texttt{xyerrorbars}, \texttt{yerrorbars}: set the data display
    style.
  \end{sloppypar}
\end{itemize}
