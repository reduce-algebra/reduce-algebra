...
\item A function to plot; a function can be
  \begin{itemize}
    ...
    \item an equation with a symbol on the left-hand side and an expression with
      one or two unknowns on the right-hand side, e.g. \texttt{dome=\allowbreak
        1/(x\^{}2+y\^{}2)}.
...
\item A range for a variable; this has the form
  ...
  floats on most IEEE machines and single floats under DOS).  Additionally the
...
In three dimensions only one surface can be shown per call.  Also an
...
The following pictures are taken from a collection of sample plots
(\texttt{gnuplot.tst}) and a set of tests for plotting special functions.  The
pictures are made using the \texttt{qt} {\Gnuplot} device and using the menu of
the graphics window to export to PDF or PNG.
...
Chebyshev's T polynomials and Bessel Y functions.
