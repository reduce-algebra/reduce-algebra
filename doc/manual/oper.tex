...
\ttindex{ATAN2}\ttindex{CI}\ttindex{COS}
...
\texttt{PRECISE\_COMPLEX}\ttindex{PRECISE\_COMPLEX} should be set on
...
Consider \texttt{df(u,x,y,z)}.  If non of \texttt{x,y,z} are equal to \texttt{u}
then the order of differentiation is commuted into a canonical form, unless the
switch \texttt{NOCOMMUTEDF}\ttindex{NOCOMMUTEDF} is turned on (default is off).
if at least one of \texttt{x,y,z} is equal to \texttt{u} then the order of
differentiation is \emph{not} commuted and the derivative is emph{not} simplified
to zero, unless the switch \texttt{COMMUTEDF}\ttindex{COMMUTEDF} is turned on.
It is off by default.

If \texttt{COMMUTEDF} is off and the switch \texttt{SIMPNONCOMDF}\ttindex{SIMPNONCOMDF}
is on then simplify as follows:
\begin{verbatim}
        DF(U,X,U)        ->  DF(U,X,2) / DF(U,X)
        DF(U,X,N,U)      ->  DF(U,X,N+1) / DF(U,X)
\end{verbatim}
\emph{provided} \texttt{U} depends only on the one variable \texttt{X}.
This simplification removes the non-commutative aspect of the derivative.

\hypertarget{switch:EXPANDDF}{}
If the switch \texttt{EXPANDDF}\ttindex{EXPANDDF} is turned on then REDUCE uses
...
Turning on the switch \texttt{ALLOWDFINT}\ttindex{ALLOWDFINT} allows
"differentiation under the integral sign", i.e. 
\begin{verbatim}
        DF(INT(Y, X), V) -> INT(DF(Y, V), X)
\end{verbatim}
if this results in a simplification.  If the switch \texttt{DFINT}\ttindex{DFINT}
is also turned on then this happens regardless of whether the result simplifies.
