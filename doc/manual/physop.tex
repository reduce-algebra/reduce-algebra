...
The NONCOM2 package redefines the \f{NONCOM} \ttindex{NONCOM} statement in
...
\item[\f{STATE U;} ] \ttindex{STATE} declares \f{U} to be a state, i.e.\ an
object on
...
by the unit operator \f{UNIT} \ttindex{UNIT}.
...
Finally a command \f{CLEARPHYSOP} \ttindex{CLEARPHYSOP}
...
For this purpose the switch \sw{CONTRACT} \ttindex{CONTRACT} has to
...
this purpose,
2 \underline{scalar} operators \f{COMM} \ttindextype{COMM}{operator} and
\f{ANTICOMM} \ttindextype{ANTICOMM}{operator} are available.
...
For this purpose, a function \f{OPAPPLY} \ttindex{OPAPPLY} has been
defined.
