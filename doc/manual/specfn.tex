...
A nice collection of example plots for special functions can be found
...
\ttindextype{erf}{(error function) operator}\ttindextype{erfinv}{(inverse error function) operator}
...
It also includes \f{erfinv} and \f{erfcinv}, which are the inverse
functions of respectively erf and erfc.  To use them,
the \package{SPECFN} package must be loaded; they do not autoload the
package.
...
\ttindextype[SPECFN]{iGamma}{operator}\ttindextype[SPECFN]{m\_gamma}{operator}
There are two incomplete gamma functions defined in the literature, see the DLMF:NIST
chapter on \href{https://dlmf.nist.gov/8.2#i}{Incomplete Gamma functions}:
\begin{align*}
  \gamma(a, z) = \int_0^z e^{-t}t^{a-1}\, \mathrm{d}t, \\
  \Gamma(a, z) = \int_z^{\infty} e^{-t}t^{a-1}\, \mathrm{d}t.
\end{align*}
called (unnormalised or unregularized) lower and upper incomplete
gamma function, respectively.
$\gamma(a,z)$ is provided by the binary function \f{m\_gamma}, $\Gamma(a,z)$ by
the two argument version of the function \f{Gamma}.

The normalised (or regularized) incomplete gamma function $P(a,z)$ is
provided by the binary function \f{iGamma} and is defined as
...
The unnormalised (or unregularized) and nomalised (or regularized)
incomplete Beta funtions are defined by
\begin{align*}
B_x(a,b) & = \int_0^x t^a (1-t)^b\, \mathrm{d}t,\\
I_x(a,b) & = \frac{B_x(a,b)}{B(a,b)}
\end{align*}
respectively. $I_x(a,b)$ is represented by the ternary function
\texttt{ibeta(a,b,x)}.
...
\ttindextype[SPECFN]{KummerM}{operator}\ttindextype[SPECFN]{KummerU}{operator}%
\ttindextype[SPECFN]{StruveH}{operator}\ttindextype[SPECFN]{StruveL}{operator}%
\ttindextype[SPECFN]{Lommel1}{operator}\ttindextype[SPECFN]{Lommel2}{operator}%
\ttindextype[SPECFN]{WhittakerM}{operator}\ttindextype[SPECFN]{WhittakerW}{operator}
...
\ttindextype[SPECFN]{zeta}{operator}\index{Riemann Zeta function!\package{Specfn} package}%
\index{Zeta function!\package{Specfn} package}
\index{$\zeta$ function!\package{Specfn} package}
...
For historical reasons the unary function \f{dilog} is defined as
\[  \int_1^z \frac{\log(t)}{1-t}\, \mathrm{d}t = \mathrm{Li}_2(1-v).\]
...
series in $t$ using either the \package{TPS} or \package{TAYLOR} package, extracting the $n$th term
and multiplying by $n!$. The use of the \package{TPS} package is probably preferable here
...
$\mathrm{erf}^{-1}(z)$ & \f{erfinv(z)}\\
$1-\mathrm{erf}(z)$ & \f{erfc(z)}\\
$\mathrm{erfc}^{-1}(z)$ & \f{erfcinv(z)}\\
$\mathrm{Ei}(z)$ & \f{Ei(z)}\\
...
\multicolumn{1}{l}{normalized (lower)}\\
incomplete Gamma $P(a,z)=\frac{\textstyle\gamma(a,z)}{\textstyle\Gamma(a)}$ & \f{iGamma(a,z)}\\
incomplete (lower) Gamma $\gamma(a,z)$ & \f{m\_gamma(a,z)}\\
incomplete (upper) Gamma $\Gamma(a,z)$ & \f{Gamma(a,z)}\\
...
\fbox{
\begin{tabular}{r m{73mm}}
\rule{0pt}{16pt}Function & Operator\\[5pt]
$Y_n^{m}(x,y,z,r2)$ & \f{SolidHarmonicY(n,m,x,y,z,r2)}\\
$Y_n^{m}(\theta,\phi)$ & \f{SphericalHarmonicY(n,m,theta,phi)}\\[5pt]
$\displaystyle\ThreeJSymbol$ & \f{ThreeJSymbol(\{j1,m1\},\{j2,m2\},\newline\{j3,m3\})}\\[4mm]
$( j_1m_1j_2m_2 \mid j_1j_2j_3 - m_3 )$ & \f{Clebsch\_Gordan(\{j1,m1\},\{j2,m2\},\newline\{j3,m3\})}\\[4mm]
