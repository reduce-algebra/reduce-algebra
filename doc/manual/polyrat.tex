...
off, they would remain unchanged. The switch \sw{exp} is normally on.
Note that if the complete factoring (\sw{allfac}) option is in force it affects
the output independently of the setting of \sw{exp}.
...
\begin{syntax}
   \texttt{factorize(}(\meta{polynomial}\optional{,prime integer}\texttt{)}:\meta{list},
\end{syntax}
...
\begin{syntax}
  \texttt{resultant(}\meta{polynomial},\meta{polynomial},\meta{kernel}\texttt{)}:\meta{polynomial}.
\end{syntax}
...
namely, with $a$ and $b$ not dependent on $x$:
\begin{align*}
  \mathrm{resultant}(p(x),q(x),x) &= (-1)^{\deg(p)\cdot\deg(q)}\cdot\mathrm{resultant}(q(x),p(x),x) \\
  \mathrm{resultant}(a,q(x),x) &= a^{\deg(p)} \\
  \mathrm{resultant}(a,b,x) &= 1
\end{align*}
%\end{samepage}
