...
\end{verbatim}
declares \texttt{x} to be a $2 \times 1$ (column) matrix, \texttt{y}
to be a $3 \times 4$ matrix and \texttt{z} a matrix whose size is to
be declared later.
...
Syntax:
\begin{syntax}
  \texttt{mateigen(}\meta{exprn:matrix\_expression},\meta{id:identifier}\texttt{)}\,:\,\textit{list}
\end{syntax}
...
Syntax:
\begin{syntax}
  \texttt{tp(}\meta{exprn:matrix\_expression}\texttt{)}\,:\,\textit{matrix}
\end{syntax}
...
Syntax:
\begin{syntax}
  \texttt{trace(}\meta{exprn:matrix\_expression}\texttt{)}\,:\,\textit{algebraic}
\end{syntax}
The operator \texttt{trace} is used to represent the trace of a square matrix.

\subsection{COFACTOR Operator}\ttindextype{cofactor}{operator}
...
Syntax:
\begin{syntax}
  \texttt{nullspace(}\meta{exprn:matrix\_expression}\texttt{)}\,:\,\textit{list}
\end{syntax}
\texttt{nullspace} calculates for a matrix $A$ a list of linearly
...
In addition to the {\REDUCE} matrix form, \texttt{nullspace} accepts
as input a matrix given as a list of lists that is interpreted as a
list of matrix rows.  If that form of input is chosen, the vectors in
the result will be represented by lists as well.  This additional
input syntax facilitates the use of \texttt{nullspace} in applications
different from classical linear algebra.
...
Syntax:
\begin{syntax}
  \texttt{rank(}\meta{exprn:matrix\_expression}\texttt{)}\,:\,\textit{integer}
\end{syntax}
...
be interpreted either as a list of matrix rows or a set of equations.
