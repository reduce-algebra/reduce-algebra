...
computations are based can be found in refs.~\cite{Many,KKV}. We invite the
interested reader to have a look at the website \cite{gdeq} which contains
useful reseources in the above mathematical area. A book on integrable systems
and \cde is currently being written \cite{KVV-book} with more examples and
...
written by M.C. Nucci \cite{nucci1,nucci2}, F. Oliveri (\textsc{ReLie},
\cite{relie}), F. Schwartz (SPDE, \REDUCE official distribution) T. Wolf
(APPLYSYM and CONLAW in the official \REDUCE\ distribution,
\cite{wolfcl,wolfsy,wbde,wolfsy0}).
...
total derivatives can be found in \cite{Many,Olv2nd}.
...
generalized symmetries) \cite{Many,Olv2nd}.
...
\emph{generalized vector fields on jets} \cite{Many,Olv2nd}.
...
generalized, \cite{Olv2nd}) symmetries of Burgers'equation $B=u_t-u_{xx}+2uu_x=0$.
...
\cite{Olv2nd}) of conservation laws to higher (or generalized) symmetries.
