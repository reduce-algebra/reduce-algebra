...
along the real axis from 0 to the integer multiple ($n$ say) of $\pi$ nearest to $\phi$ followed
by the straight line segment from between $x=\sin(n\pi)$ and $x=\sin \phi$. When $\phi$ is real the
...
Note also that if the contour contains one or more  branch points, there are sign ambiguities as a
branch point is crossed depending on which branch of the square root in the integrand is
chosen. It seems logical to always choose the principal branch.
...
\ttindextype[ELLIPFN]{RG}{operator}
...
\cite{Carlson:99} \& \cite{CarlsonFitzSimmons:00} showed how each of these can be
...
\end{eqnarray*}
Clearly $U_{\alpha\beta}=U_{\beta\alpha}=U_{\gamma\delta}=U_{\delta\gamma}$ and thus there
...
For integrals of the third kind the 'awkward' case where $X_1$ or $Y_1$ vanishes,
...
\vdots\\
$\mathrm{arccs}(u,k)$ & \f{arccs(u,k)}\\[5pt]
