...
\end{syntax}
where \meta{keyword} is one of the following: \f{closed}, \f{quasilinear}, \f{pfaffian} or
...
\hypertarget{operator:EDS_COORDINATES}{}
\begin{syntax}
...
\header{Command}
    &\header{Function}\tabularnewline\hline
...
\literal{coframing($S$)}
    &constructs a \meta{coframing} capable of supporting the given
     \meta{system}\tabularnewline\hline
...
\header{Command}
    &\header{Function}\tabularnewline\hline
\literal{coframing($E$)}
    &extracts the underlying \meta{coframing}\tabularnewline\hline
\literal{coframing()}
...
\literal{restrictions($M$)}\nl \literal{restrictions($E$)}
    &extracts the inequalities describing the restrictions
     in the \meta{coframing}\tabularnewline\hline
\literal{system($E$)}
    &extracts the \meta{system} part of $E$\tabularnewline\hline
...
\literal{properties($E$)}
    &returns the currently known properties of the \meta{EDS} $E$ as a list of
     equations \meta{keyword}$ = $\meta{value}\tabularnewline\hline
...
\literal{pullback($E$,$f$)}\nl \literal{pullback($S$,$f$)}\nl \literal{pullback($\Omega$,$f$)}
    &pulls back the first argument using the \meta{map} $f$\tabularnewline\hline
\literal{pullback($M$,$f$)}
    &returns a \meta{coframing} $N$ suitable as the source for $f:N\to M$\tabularnewline\hline
\literal{restrict($E$,$f$)}\nl \literal{restrict($S$,$f$)}\nl \literal{restrict($\Omega$,$f$)}
    &restricts the first argument to the points specified by the
     \meta{map} $f$\tabularnewline\hline
...
\literal{integral\_element($E$)}
    &find a random \meta{integral element} of $E$\tabularnewline\hline
\literal{prolong($E$)}
...
\literal{edsdebug}
    ...
    in addition to that produced by \literal{edsverbose}\tabularnewline\hline
\literal{edssloppy}
...
\literal{invert($X$)}
    &returns the inverse \meta{transform} $X^{-1}$\tabularnewline\hline
...
\literal{exfactors($\Omega$)}
    &as for \literal{linear\_divisors}, but with the additional (non-linear) factor\tabularnewline\hline
...
\literal{mkdepend($list$)}
    &restores variable dependencies destroyed by \literal{pde2eds}\tabularnewline\hline
\literal{disjoin($\{f,g,...\}$)}
...
\literal{symbol\_relations($E$,$\pi$)}
    ...
    matrix, represented by 2-indexed \meta{1-form} variables $\pi^a{}_i$\tabularnewline\hline
\literal{symbol\_matrix($E$,$\xi$)}
    &returns the symbol matrix for a quasilinear \meta{EDS} $E$ as a function
    of \meta{0-form} variables $\xi_i$\tabularnewline\hline
\literal{characteristic\_variety($E$,$\xi$)}
    &returns equations describing the characteristic variety of
    $E$ in terms of \meta{0-form} variables $\xi_i$\tabularnewline\hline
\end{commandtable}
