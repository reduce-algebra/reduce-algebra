...
%****************************************************************
%
%  Copyright (C) 1989,1990,1991,1992 by Rainer M. Schoepf,
%   all rights reserved.
%
...
taylor!:version := "1.4b";      % version number of the package
taylor!:date!* := "16-Apr-92"; % release date
...
exports !*tay2f, !*tay2q, copy!-list, cst!-taylor!*, get!-degree,
        has!-taylor!*, make!-cst!-coefficient, make!-cst!-coefflis,
        make!-cst!-powerlist, make!-taylor!*, multintocoefflist,
        nzerolist, resimptaylor, set!-taycfpl, set!-taycfsq,
        set!-taycoefflist, set!-tayflags, set!-tayorig,
        set!-taytemplate, taycfpl, taycfsq, taycoefflist,
        taydegreesum, tayflags, tayflagscombine, taygetcoeff,
        taylor!*p, taylor!-kernel!-sf!-p, taylor!-kernel!-sq!-p,
        taymakecoeff, taymincoeff, taymultcoeffs, tayorig,
        taytemplate, taytpelpoint, taytpelorder, taytpelvars, tayvars,
        tpdegreelist;

imports

% from REDUCE kernel:
        !*p2f, !*p2q, add!-degrees, domainp, eqcar, kernp, lastpair,
        lc, ldeg, mksp, multsq, mvar, nlist, numr, over, red, resimp,
        smember, subs2;
...
% from REDUCE kernel
        denr, domainp, kernp, mvar, neq, numr, prin2t, rerror;
...
exports add!-degrees, add!.comp!.tp!., comp!.tp!.!-p,
        delete!-superfluous!-coeffs, enter!-sorted, exceeds!-order,
        find!-non!-zero, inv!.tp!., mult!.comp!.tp!., taydegree!-lessp,
...
% from the header module:
        get!-degree, make!-cst!-powerlist, taycfpl, taycfsq,
        taycoefflist, taytemplate, taytpelorder, taytpelpoint,
        taytpelvars,
...
symbolic procedure mult!.comp!.tp!. (u, v, div!?);
  ...
  begin scalar w, w1, w2, cf1, cf2;
    ...
    if null cf1 then cf1 := make!-cst!-powerlist taytemplate u
     else cf1 := taycfpl car cf1;
    cf2 := taycoefflist v;
    while cf2 and null numr taycfsq car cf2 do cf2 := cdr cf2;
    if null cf2 then cf2 := make!-cst!-powerlist taytemplate v
     else cf2 := taycfpl car cf2;
    u := taytemplate u; v := taytemplate v;
    if length u neq length v then return nil;
   loop:
    if not (taytpelvars car u = taytpelvars car v
...
symbolic procedure inv!.tp!. u;
  ...
  % of terms.
  %
  begin scalar w, w2, cf;
        integer !#terms!-1, dl, mindg;
    cf := taycoefflist u;
    while cf and null numr taycfsq car cf do cf := cdr cf;
    if null cf then cf := make!-cst!-powerlist taytemplate u
     else cf := taycfpl car cf;
    u := taytemplate u;
   loop:
    dl := get!-degree car cf;
    mindg := - dl;
    !#terms!-1 := taytpelorder car u - dl;
    w2 := (!#terms!-1 + dl) . w2;
    w := {taytpelvars car u, taytpelpoint car u, mindg + !#terms!-1}
          . w;
    u := cdr u; cf := cdr cf;
...
% from the REDUCE kernel:
        ...
        prepsq, reval, reversip, simp, simp!*, subs2, subsq, typerr,

% from the header module:
        !*tay2q, get!-degree, make!-taylor!*, multintocoefflist,
        resimptaylor, set!-tayorig, taycfpl, taycfsq, taycoefflist,
        tayflags, taymakecoeff, tayorig, taytemplate, taytpelorder,
        taytpelpoint, tayvars, taylor!-kernel!-sq!-p, taymincoeff,
...
% from the header module:
        ...
        make!-taylor!*, multintocoefflist, taycfpl, taycfsq,
...
symbolic procedure quottaylor1 (tay, lu, lv);
  ...
  begin scalar clist, lminu, lminv, aminu, aminv, ccmin, coefflis;
    ...
    coefflis := makecoeffs for each term in pair (ccmin, tay) collect
           (car term . (taytpelorder cdr term - get!-degree car term));
...
% from the REDUCE kernel:
        !*f2q, !*n2f, !*p2f, !*p2q, !*t2f, addd, addf, addsq, apply1,
        denr, domainp, exptsq, invsq, kernp, mk!*sq, multf, multpf,
        multsq, mvar, neq, nth, numr, over, pdeg, prepsq, quotsq,
        reversip, sfp, simp, simp!*, smemqlp, tc, to, tpow,

% from the header module:
        !*tay2f, !*tay2q, comp!.tp!.!-p, has!-taylor!*, find!-non!-zero,
    ...
    if taylor!-kernel!-sf!-p dd
      then if taylor!-kernel!-sf!-p nm
        then if comp!.tp!.!-p (mvar nm, mvar dd)
          then !*tay2q resimptaylor quottaylor (mvar nm, mvar dd)
         else nm ./ dd
      ...
      then if not smemqlp (tayvars mvar nm, dd)
             then !*tay2q resimptaylor multtaylorsq (mvar nm, 1 ./ dd)
            else if null !*taylorautoexpand or has!-taylor!* dd
             then nm ./ dd
            else
              !*tay2q resimptaylor quottaylor (mvar nm,
...
symbolic procedure taysimpf u;
  ...
  begin scalar tay, notay, x, flg;
    ...
    if null !*taylorautoexpand and smemqlp (tayvars tay, notay)
      then return addf (!*tay2f tay, notay);
    if not null notay then
      if flg then return addf (!*tay2f tay, notay)
       else <<
         notay := numr taylorexpand (!*f2q notay, taytemplate tay);
         return !*tay2f addtaylor (mvar notay, tay) >>
     else return !*tay2f tay
  end$
...
symbolic procedure multpowerintotaylor (p, tk);
  ...
  if not smemqlp (tayvars tk, p)
    then !*tay2f multtaylorsq (tk, !*p2q p)
   else if !*taylorautoexpand
    then !*tay2f multtaylor (tk, mvar numr taylorexpand (!*p2q p,
                                     taytemplate tk))
   else multpf (p, !*tay2f tk)$
...
% from the header module:
        ...
        taytemplate, taytpelorder, taytpelpoint, taytpelvars,
...
% from the REDUCE kernel:
        !*f2q, diffsq, lastpair, ldepends, multsq, nth, over, to,

% from the header module:
        !*tay2q, make!-cst!-coefflis, make!-taylor!*, taycfpl, taycfsq,
        taycoefflist, tayflags, taymakecoeff, tayorig, taytemplate,
...
symbolic procedure taydiffp (u, v, n);
  ...
  %
  !*tay2q if n=1 then difftaylor (u, v)
...
symbolic procedure tayrevert1pole (tp, cf, el, i, okrnl, krnl,
  ...
  begin scalar x, y, z;
    ...
    if taytpelpoint car y neq 0
      then taylor!-error('not!-implemented,"(Taylor series reversion)")
     else <<
