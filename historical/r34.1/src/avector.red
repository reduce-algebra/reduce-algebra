...
% Here we re-define the routine VARPRI to include 3-vectors. We
% add a single line to the routine.
...
symbolic procedure varpri(u,v,w);
   ...
   %  in the current set (or NIL otherwise).
    ...
    v := setvars v;
    if vecp u then return vecpri(u,'mat);
...
%   if null x then xx := 'vec;
    ...
    varpri(v0,list('setq, list(xx,aeval first !*coords),v0),'only);
    varpri(v1,list('setq, list(xx,aeval second !*coords),v1),'only);
    varpri(v2,list('setq, list(xx,aeval third !*coords),v2),'only);
    terpri!* t;
  end;

% This routine is the 'hook' for VARPRI
...
% Separate the operator from its operands
    ...
    vecop := car u;
    vargs := mapcar(cdr u,'vecsimp!*);

% Select a course of action dependent upon the operator

    if y := get(vecop,'vectorfunction) then
    % We must check (if the op is an arithmetic operator)
    % to ensure that there are vectors in the argument list
    << if (flagp(vecop,'vectorfn) or (flagp(vecop,'varithop) and
       hasonevector vargs)) then v := apply(y,list vargs)
                            else v := aeval append(list(vecop),vargs)
    >>
    else if flagp(vecop,'vectormapping) then
    << % Check that the argument is really a vector
      y := car vargs;
      v := if threevectorp y then vectorapply(vecop,y,cdr vargs)
                             else scalarapply(vecop,y,cdr vargs)
    >>
    else <<!*vectortracelevel!* := 0;
           rerror(avector,4,
                  list(vecop,"is not a valid vector operator"))>>;
  >>;
  if !*vtrace then <<
    y := threevectorp v;
...
symbolic procedure vectorapply(vecop,v,args);
  begin scalar vv,x,y;
  x := mkvect(2);
  ...
  << % Apply the operation to each component
    y := getv(vv,k);
    y := if null args then aeval list(vecop,y)
                      else aeval append(list(vecop,y),args);
    putv(x,k,y);
  >>;
  return x
  end;
...
% (Amended by M. MacCallum)
         ...
         then return apply('vectordot, list(mapcar(u, 'vecsimp!*)) )
