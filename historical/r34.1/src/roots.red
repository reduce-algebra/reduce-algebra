...
Comment
  Revisions:
    ...
    20 May 92.  Mod 1.93 improves polynomial handling by using big
...
global '(roots!-mod); roots!-mod := "Mod 1.93, 20 May 1992.";
...
% Version and Date:  Mod 1.93, 20 May 1992.

% Copyright (c) 1988,1989,1990,1991,1992. Stanley L. Kameny. All Rights
% Reserved.
...
fluid '(!*bftag !*pcmp !*rvar !*strm lims!# mltr!# emsg!# !*xo !*gfp
 dmode!* !*powergcd !:prec!: !*msg pfl!# acfl!# accm!# froot!#);
...
symbolic procedure roots p;
   <<lims!# := nil; uniroots(p,1)>> where froot!#=nil;

symbolic procedure firstroot p;
   <<lims!# := nil; uniroots(p,1)>> where froot!#=t;

symbolic procedure root_val x;
  % produces list of root values at system precision (or greater if
  % required to separate roots.)
   <<x := roots x; precision p; x>>
     where rootacc!#=p,!*noeqns=t where !*msg=nil,p=precision 0;

symbolic procedure roots_at_prec x;
  % produces list of root equations at system precision (or greater if
  % required to separate roots.)
   <<x := roots x; precision p; x>>
     where rootacc!#=p where !*msg=nil,p=precision 0;

symbolic operator roots,firstroot,root_val,roots_at_prec;
...
symbolic operator nearestroot,gfnewt,gfroot;
...
loop: ac := accupr(q,p,r);
      if pfl!# then
         <<if (s := acc!# - (ac := ac + ss!#))>0 then
              <<acfl!# := t; accm!# := accm!#-s>>;
           acc!# := ac;
           trmsg12(r := if rl then rootrnd gfrl r else gfrtrnd r);
           return r>>
...
%unfluid '(!*bftag !*pcmp !*rvar !*strm lims!# mltr!# emsg!# !*xo !*gfp
% pfl!# acfl!# accm!# froot!#);

%unfluid '(cpxt!# acc!# sprec!# pfactor!# rr!# ss!# !*xn !*zp !*xobf
% !*noeqns !*msg rootacc!#);
...
fluid '(!*trroot !*rootmsg !*multiroot !*bigfloat !*roundbf !:prec!:
  !*msg !*bftag !*sqfree !*ratroot !*nosqfr rootacc!#);
switch trroot,rootmsg,multiroot,ratroot;
global '(!!eps !!nfpd !!flim !!flint bfz!*);
global '(!!shbinfl rootsreal rootscomplex auton!* den!* lm!#);
flag('(rootsreal rootscomplex),'share);
fluid '(!*pfsav pr!# ra!# acc!# bfl!# emsg!# eps!#);
fluid '(!*xmax !*xmax2 !*rvar nwmax!# lgmax!# nozro!# !1rp);
fluid '(!*xo !*cmplex !*pcmp !*multrt lgmax!# nwmax!#);
...
symbolic procedure nwterrfx(n,cp);
  if n<3 then 0 else
   fix((n-2)*sqrt max(0,0-15+if !*bftag then !:prec!: else !!nfpd)
     *if cp then 4 else 1);
...
symbolic procedure prxflot(pr,p);
   <<!:prec!: := if setflbf(rlrt!# or pr>!!nfpd) then pr else !!nfpd;
     bfrndem bfloatem p>>;
...
symbolic procedure rootprec n;
   rprec!# := if numberp n and (n := abs fix n)>!!nfpd then n;
...
symbolic procedure ckprec p;
   <<find!!nfpd(); find!!flim(); bf!!eps();
    ...
    % the following line is corrected so :prec: restores properly.
     pr!# := !:prec!:; !:prec!: := max(!:prec!:,acc!#+2,!!nfpd+2);
     ...
     % the following change improves roots, solve interface.
      if !*noeqns then
         <<rootscomplex := rootsreal := nil;
           return 'list . for each j in c collect outmode j>>
         else
...
symbolic procedure automod p; % p is always returned in bfloat form.
   ...
   begin integer n,s,s2; scalar a,d,m,nl,pr,nc,dd; rr!# := 0;
      if null cdr p then <<n := !:prec!:; go to sel>>;
  ...
  nl := for each c in cdr reverse p collect xnsiz cdr c;
      ...
      setflbf nil; !:prec!: := pr;
 cfl: if errorp errorset!*(list('cflotem,mkquote p),nil)
         then go to bfl else return p;
 sel: if not !*bftag then go to cfl;
 bfl: setflbf t; !:prec!: := n; return p end;
...
symbolic procedure reformup q; if domainp q then q else
  % returned q will be bfloat.
   begin scalar c,fg,d; integer n;
      ...
      if n+2>!:prec!: then !:prec!: := n+2;
      return q end;

%unfluid '(!*pfsav ra!# acc!# rat!# bfl!# !*noeqns);
%unfluid '(!*xmax !*xmax2 !*rvar nwmax!# lgmax!# nozro!# sprec!#);
%unfluid '(!*xo rprec!# !*cmplex !*pcmp !*multrt emsg!# incmsg!$);
%unfluid '(!*keepimp pr!# !1rp sqf!# exp!# !*bfsh vars!# rlrt!#);
...
fluid '(acc!# !*intp !*multrt !*strm !*cmplex sprec!# !*xo !*rvar);
...
%unfluid '(acc!# !*intp !*multrt !*strm !*cmplex sprec!# !*xo !*rvar);
%unfluid '(!*bftag incracc!# lims!# pfactor!# pr1rp!# !*xnlist accm!#);
%unfluid '(rootacc!#);
...
symbolic procedure accuroot(y,p,xo);  % p,xo,!*xn all bfloat
   begin scalar rprec,b,c,n,rl,x,pr0,ps,y0;
      ps := !:prec!:; rl := bfnump (y0 := y := gf2bf y); b := !*bftag;
      !*xo := xo; !*bftag := t;
 ...
 ret: if acfl!# then
         <<!:prec!: := prec!# := prm!# := max(rprec,prm!#); go to r3>>;
      prec!# := !:prec!:;
  ...
  r3: trmsg12(y := if rl then rootrnd gfrl y else gfrtrnd y);
      setflbf b; !*xn := gf2bf !*xn; return y end;
...
symbolic procedure prreq(p,x,rl);
  % find required precision to find root at x in polynomial p.
   begin scalar p1,x1,rx;
      ...
      return if bfzp x1 then nil
         else (1+ep!: round!:mt(
            divbf(timbf(rx,'!:bf!: . (1 . (acc!#+2))),x1),1)
            where !:prec!:=8) end;
...
symbolic procedure allroots(p,p0); % p is always bfloated at this call.
  % do the actual work of finding roots of p in appropriate environment.
   begin scalar q,n,n0,c,cc,cprq,rln,cpn,qf,ac,y,er,rl,z,mb,inc,prec,xo,
        pf,xof,qbf,sprec,b,red,sw,pfl!#,acfl,acfl!#;
        integer req,npi,accm!#,prec!#,r15n,prm!#;
      prec := !:prec!:; polrem!$ := polnix!$ := nil;
      ac := acc!#; n0 := caar lastpair p; red := not p0;
...
strt: n := n0; !*gfp := qbf := p; c := cc := !*mb := pf := nil;
      cprq := n-req;
      if not !*pcmp then cprq := cprq/2; rln := cpn := prm!# := 0;
root: qf := nil; if b then <<q := qbf; go to r0>>;
      q := if errorp(q := errorset!*(list('cflotem,mkquote qbf),nil))
...
symbolic procedure nrstroot(p,rr,trib);
   begin
      scalar x,c,d,dx,xm,r,cp,n,m,s,p1,prx!#,acc,pp,m1,
        !*multiroot,cpxt!#,pfactor!#,nrst!$,intv!#,pfl!#,acfl!#;
...
symbolic procedure nrstrt0(q,r,p1);
   begin scalar rr,x,b,pr,ps,p2,qf; pmsg pbfprint q; b := !*bftag;
      ...
      if errorp(q := errorset!*(list('cflotem,mkquote qf),nil))
        or errorp(r := errorset!*(list('gf2flr,mkquote rr),nil))
         then go to r1 else <<q := car q; r := car r>>;
      if (x := gfrootset(q,r,b)) then go to r3;
  r1: q := qf; b := !*bftag := t; r := gf2bf rr;
  r2: x := gfrootfind(q,r); !*xobf := gf2bf !*xo;
  r3: if not !*hardtst then x := ckacc(q,if p1 then p1 else q,!*xn);
      ...
      pr := !:prec!: := prec!#;
      if not !*bftag then
         <<q := qf; p1 := bfloatem p1; !*xo := gf2bf !*xo;
           b := !*bftag := t>>;
...
% lgmax!# !*xo !*keepimp !1rp emsg!# !*zp accm!# lims!# !*bfsh prm!#
% !*strm mltr!# !*bftag incmsg!$ !*zx1 !*xmax pfactor!# rprec!# rr!#
% ss!# prx!# nrst!$ intv!# tht!# !*xobf pfl!# acfl!# froot!#);
...
fluid '(!*gfp !*xnlist !*intp tht!# !*strm lims!# mltr!# pfactor!#
 ...
 !*xn pr1rp!# prx!# nrst!$ intv!# sh!# !:prec!: rprec!# rlrt!# prm!#
 pfl!# acfl!#);
...
symbolic procedure rtsreal p;
 ...
 % sturm series, isolater and rootfind.
   begin scalar acr,acs,n,q,r,x,y,!*strm,pr,apr,!*bftag,pfl!#,acfl!#;
      integer accm!#,prec!#,prm!#; pr := !:prec!:;
      !*bftag := rlrt!# := t;
...
symbolic procedure limadj m;
   if not m then lims!#
      ...
      if remainder(m,2)=0 then list 1 else nil
   else
   ((if remainder(m,2)=0 and car lval a * car lval b<0 then
       list(0 . 1,if ratlessp(lval c,lval d) then d else c)
     else if ratlessp(lval c,lval d) then list(c,d) else list(d,c))
    where c=lpwr(a,m),d=lpwr(b,m) where a=car lims!#,b=cadr lims!#);
