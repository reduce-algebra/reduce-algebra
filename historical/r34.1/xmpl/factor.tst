...
% If you want deterministic behavior, set randomseed* before each test.

% global '(randomseed!*);

% symbolic(randomseed!* := 300000);
...
algebraic procedure test(prob,nfac);
  begin
    ...
    symbolic prin2t list("The random seed is",randomseed!*);
    m := factorize p;
    symbolic (basetime := time() - basetime);
    symbolic lpri list("Time =",basetime);
    symbolic terpri();
    q := for each j in m product j;
    if (length m=nfac) and (p=q) then return ok;
