  ...
  (if zerop x then bfz!* else
   begin scalar s,r; integer d;
     ...
     % convert x to an integer equivalent;
      r := normbf read!:num x;
      d := ep!: r+msd!: mt!: r - !:bprec!:;
      x := x*2.0**-d; x := x + 0.5;
      x := fix x;
      return make!:ibf (if s then -x else x, d) end)
       where !:bprec!:=!!nbfpd;
...
symbolic procedure bfprin!:lst (lst, ex, dotpos);
  begin integer j;
    ...
    for each char in lst do <<
      prin2!* char; j := j + 1; dotpos := dotpos - 1;
      if j = 5 then << if !*nat and !*bfspace then prin2!* '! ;
                       j := 0 >>;
      if dotpos = 0 then << prin2!* '!.; j := j + 1 >>;
      if j = 5 then << if !*nat and !*bfspace then prin2!* '! ;
                       j := 0 >> >>;
    if ex = 0 then return;
    if not (!*nat and !*bfspace) then prin2!* "E"
     else if j=0 then << prin2!* "E "; j := 2 >>
...
normbf round!:mt(v,if !:print!-prec!:
                   then ceiling(log2of10*!:print!-prec!:)
                   else !:bprec!: - 3);
...
symbolic procedure bfsqrt x;
 % computes sqrt x by Newton's method.
  ...
  begin  scalar nx,dx,dc,k7,nf,ep;
...
symbolic procedure iexpt(x, n);
   ...
   if not (n > 0) then error(0, "iexpt argument <= 0")
    ...
    else if remainder(n, 2) = 0 then iexpt(x*x, n/2)
    else x*iexpt(x*x, (n - 1)/2);
...
symbolic procedure expt(x,y);
   ...
   begin integer iy,ip,p; scalar sy,fy,r;
...
exports chkint!*, chkrn!*, convprec, convprec!*, deg2rad!*, erbdck,
...
symbolic procedure precision0 n;
  % called from algebraic call of precision.
   if n member '((nil) () (reset))
     ...
     or n<0 then
       rerror(arith,3,"positive numeric value or `RESET' required")
...
symbolic procedure !*rd2rn x;
 ...
 %  but uses a heuristic to avoid the extra work when not needed.
   ...
   % temporary write statements here.
    % write r1," -> ",d1," new"; if not ov then write " <== best";
    % terpri();
    % write " ",r2," -> ",d2," old"; if ov then write " <== best";
    % terpri();
     precision p;
...
imports bfloat, bfnzp, bftrim!:, bfzp, chkint!*, chkrn!*, convprec,
        convprec!*, crim, crrl, ep!:, erbdck, errorp, errorset!*,
...
symbolic procedure crprep1 u;
 % a and d are 1,-1,or rounded.
  (if not numberp d and rd!:zerop d then a
   else if not numberp a and rd!:zerop a then crprimp d
   else if d=-1 then {'difference,a,'i}
   else if not numberp d and rd!:minusp d
     then {'difference,a,crprimp rd!:minus d}
   else {'plus,a,crprimp d}) where a=car u,d=cdr u;
...
exports deg2rad!*, quotient!:, rad2deg!*, rdacos!*, rdacosd!*,
        ...
        rdsecd!*, rdsech!*, rdsgn!*, rdsin!*, rdsind!*, rdsinh!*,
