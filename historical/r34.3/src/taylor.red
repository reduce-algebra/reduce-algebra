...
!*tayrestart!* := nil;         % set by Taylor!-error!* if expansion is
                               ...
                               % increased.
taylor!:version := "1.8";     % version number of the package
taylor!:date!* := "18-Aug-93"; % release date
...
symbolic smacro procedure multintocoefflist (coefflis, sq);
  ...
  for each p in coefflis collect
    taymakecoeff(taycfpl p, subs2 resimp multsq (taycfsq p, sq));
...
exports add!-degrees, add!.comp!.tp!., check!-for!-cst!-taylor,
        ...
        taylor!*!-onep, taylor!*!-zerop, tp!-greaterp,
...
symbolic procedure taylorexpand!-diff2(sq,el,flg);
   begin scalar l,v,singlist,c0,tay,l0;
     ...
     if not taylor!-kernel!-sq!-p c0
       then tay := make!-taylor!*(taymakecoeff(l0,c0) . v,{el},nil,nil)
      else <<c0 := mvar numr c0;
             tay := make!-taylor!*(
                      nconc(for each pp in taycoefflist c0 collect
                              taymakecoeff(append(taycfpl pp,l0),
                                           taycfsq pp),
                            v),
                            aconc!*(taytemplate c0,el),
                            nil,nil)>>;
     tay := !*tay2q!* tay;
