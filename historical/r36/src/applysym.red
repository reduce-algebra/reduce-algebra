...
load_package crack, crackapp;

create!-package('(applysym),'(contrib crack));

fluid '(print_ fname_ nfct_ time_ facint_ logoprint_);
      ...
      if symanz=1 then genlist:=sym_(1)
        ...
        <<lisp
          <<terpri()$
            ...
            terpri()$
            write"Enter an expression with `sy_(i)' for the i'th ",
          ...
          h1:=aread(nil);
          if h1 then <<
            for each h2 in xlist do
                if h1 then if df(h1,h2) neq 0 then h1:=nil;
            for each h2 in ylist do
                if h1 then if df(h1,h2) neq 0 then h1:=nil;
            if h1=nil then lisp <<
              terpri();
              write"The coefficients of the sy_(i) must be constant, ",
...
begin scalar xslist,yslist,el3,h$
  ...
  lisp <<terpri()$
         ...
         terpri()$
         write
          "The symmetry variable is ",reval algebraic smv,", i.e. the ",
...
%    write "-----------------------------------------------",
%    "---------------------------"$ terpri()$terpri()$
%    write "This is SIMILARITY - a program for calculating similarity";
%    write
%       "variables and performing corresponding point-transformations";
%    write "for given infinitesimal symmetries.";
%    terpri()
%  >>                            else
...
%         write"must not vanish."$terpri()$
      ...
      <<% If no symmetry variable is found (trans2={}) then proceed
        ...
        if trans2={} then << % take any variable
          ...
          while 1+sub(u_=first h,pde)=0
             do h:=rest h; % no similarity var.
          for each e1 in fargs u_ do nodepend u_,e1;
          h:=first h;
          trans2:={u_ - h};
    ...
    %         lisp( cons('ff,cdr reval algebraic el1)));terpri()$
              write
                "A suggestion for this function ff(..) yields:";terpri()
            >>;
            h:=einfachst(el1,u_);
    ...
    %                                       else h:=num(h+   first el1);
            ...
            if yesp "Do you like this choice?" then h
              ...
              repeat <<
                lisp <<
                  write
                     "Put in an alternative expression which "$terpri()$
                  write
...
%        <<
          ...
          if h2 neq nil then <<
            ...
            "Shall the dependence on the symmetry variable be dropped?"
              then
              <<for each el3 in ulist do nodepend el3,smv;
                vlist:=rest vlist>>;
              eqlist:=for each el3 in eqlist collect <<
                el3:=factorize num el3;
                for each el4 in el3 product
...
symbolic procedure simplifiziere(q)$
begin
 scalar n,one_argument_functions_;
 one_argument_functions_:= '(abs acos acosd acosh acot acotd acoth acsc
                        ...
                        log log10 sec secd sech sin sind sinh sqrt
                        tan tand tanh minus)$
 return
 if not pairp q then q else
 if member(car q,one_argument_functions_) or
    ((car q = 'expt) and (numberp caddr q)) then simplifiziere(cadr q)
                                            else <<
  n:=ncontent(q);
  if n=1 then q
...
write "soln=",h;
 for each h in xlist do nodepend h,s;
 return h
end$ % of charsyspsfi
...
begin
  ...
  lisp <<
    ...
    printold:=print_;  print_:=nil;
    timeold := time_;  time_ :=nil;
    facintold:=facint_;facint_:=t;
  >>;
  % lisp(print_:=100);  %############
  ...
  for each x in soln do <<
   ...
   if (e1 neq {}) and (length e1 + length second x = length flist) then
    % all remaining conditions are algebraic (non-differential) in all
    % the functions of flist?
  <<e2:={};
    e3:={};
    for each e4 in third x do if freeof(flist,e4) then e3:=e4 . e3
                                                  else e2:=e4 . e2;
    if (length cs) = (length e3) then <<
     % sufficiently many integrations done
     ...
     if e2 neq {} then <<
      ...
      for each e4 in e2 do <<        % for each unsolved function
       for each e5 in second x do << % for each solved expression
        n:=lisp(lderiv(reval algebraic rhs e5,reval algebraic e4,
                       list(reval algebraic s)));
        if (n neq 0) and (n neq nil) then
        if lisp length algebraic n = 3 then n:=1
                                       else n:=lisp cadddr algebraic n;
  ...
  lisp <<print_:=printold; time_:=timeold; facint_:=facintold>>;
  return
  if lcopy={} then <<for each x in flist do nodepend x,s; {}>>
              else s . lcopy
...
begin scalar i, q, qlist, cq, cqlist, ode, soln, tran,
             xcop, s, s1, x, xx, scal, qlin, prev_depend;
  symbolic put('ff,'simpfn,'simpiden)$
...
%  soln:=charsyspsfi(xlist,cqlist,u,ode,den f);
  ...
  if soln={} then << % try all other coefficients as factors
    repeat <<
      ...
      if (s neq s1) and (scal neq 0) then << lisp
  ...
  if soln neq {} then <<
    ...
    <<cq:=first solve(first s,second s);
      tran:=.(<<x:={};
                ...
                x
              >>,
%                  intsimplify(q,cons(reval algebraic u,
%                                      cdr reval algebraic xlist)) >>,
              tran);
    >>;
    for each s in xlist do if s neq s1 then nodepend s,s1
  >>;
