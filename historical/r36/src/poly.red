...
create!-package('(poly polrep quotf gcd exptf kernel mksp reord dmode
                 ...
                 polyop decompose interpol subs2q subs3q subs4q horner),
                nil);
...
symbolic procedure quotfx1(p,q);
   ...
   if null p then quotfxerr(p,q)
    ...
    a:  if (dp:=ldeg p) <dq then return quotfxerr(p,q);
        ...
        y := w .** n;
        if null f then p := cutf(p,w,dp #- n #- 1); f:=t;
        q:=cutf(q,w,dq #- n #- 1);
        p := addf(p,multf(if n=0 then q else multpf(y,q),negf x));
...
symbolic procedure cutf(u,x,n);
   ...
   if n #< 1 then u else cutf1(u,x,n);

symbolic procedure cutf1(u,x,n);
   if domainp u or mvar u neq x or ldeg u #< n then nil else
...
symbolic procedure gcdf(u,v);
   ...
   begin scalar !*exp;
      ...
      u := if domainp u or domainp v or not !*ezgcd
              or free!-powerp u or free!-powerp v then gcdf1(u,v)
            else ezgcdf(u,v);
...
symbolic procedure gcdf2(u,v);
   ...
   begin scalar asymplis!*,w,z;
      ...
      % only need to make sure v does not have a leading noncom opr.
      if noncomp mvar v then z := 1
       else if mvar u eq mvar v
...
fluid '(!*nosubs !*sub2 asymplis!* subfg!* wtl!*);

% fluid '(varstack!*);

global '(!*resubs powlis!*);
...
fluid '(!*modular !*balanced_mod current!-modulus alglist!* dmode!*);
...
(lambda x; if x=0 then nil else if x=1 then 1 else '!:mod!: . x)
           general!-modular!-expt(cdr u,n);
...
symbolic procedure modprep!: u;
   <<u := cdr u;
     if !*balanced_mod and u+u>current!-modulus then u-current!-modulus
      else u>>;
...
symbolic procedure fctrf u;
   ...
   (begin scalar !*ezgcd,!*gcd,denom,x,y;
      if domainp u then return list u
       else if ncmp!* and not noncomf u then ncmp!* := nil;
      !*gcd := t;
...
symbolic procedure factor!-prim!-sqfree!-f u;
   ...
   begin scalar x,y,!*msg,r;
      r := !*rounded;
      if r or !*complex or !*rational then
...
symbolic procedure sqfrf!* u; caar sqfrf u;

put('square_free,'polyfn,'sqfrf!*);
...
symbolic procedure rationalizesq u;
   ...
   begin scalar !*structure,!*sub2,x;   % Modified by R. Liska.
      ...
      return if domainp denr u then u
              ...
              then rationalizesq exptchksq
                      subs2q (multf(numr u,x) ./ multf(denr u,x))
             else u
   end;
...
fluid '(!*allowdfint !*fjwflag frlis!* subfg!* wtl!*);
    ...
    h:  % Final check for possible kernel deriv.
        if car u eq 'df
          then if depends(cadr u,v)
             and not(cadr u eq v) % (df (df v A) v) ==> 0
                 then
...
global '(!*resubs powlis!* powlis1!* simpcount!* simplimit!*);
