...
%
%     fancy!-symbol!-length  the number of horizontal units needed for the symbol.
%                      A standard character has 2 units.
 ...
 fancy!-level
  begin scalar x;
    ...
    if (x:=get(op,'fancy!-infix!-symbol)) 
           then fancy!-special!-symbol(x,get(op,'fancy!-symbol!-length) or 4)
      else
...
symbolic procedure fancy!-prefix!-operator(u);
 % Print as function, but with a special character.  
   begin scalar sy;
     ...
     if sy then fancy!-special!-symbol(sy,get(u,'fancy!-symbol!-length) or 2) else
     fancy!-prin2!*(u,t);
   end;
