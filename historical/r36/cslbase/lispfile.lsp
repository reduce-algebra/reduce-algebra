%
% Create Standard Lisp versions of the source files for CSL where I keep
% the master versions coded in RLISP.
%
% Since this is (probably) not done very often I build a version of
% RLISP specially for the occasion.
%
% Run this as
%           csl -z lispfile.lsp
%
% or        csl -z lispfile.lsp -D@cslbase="<directory for csl base>"
%           (eg  csl -z lispfile.lsp -D@cslbase="d:\acn\cslbase")

(spool "lispfile.log")

(load!-module 'compat)

% Note that if too much of the system has changed or been damaged it may
% be necessary to disable the compiler here (by commenting out the next
% couple of lines).  This code should then run correctly but painfully
% slowly.
...
%
% If the files compat.fsl and compiler.fsl had been corrupted you
% could go
%   (rdf "compat.lsp")
...
% the associated RLISP source files.  Note that REDUCE 3.3 is now a
% VERY old and out-of-date system, and the code here is only from the
% parser from it - no algebra is included.
%
...
procedure setpchar c;		% set prompt, return old one
...
symbolic procedure initreduce;        %. initial declarations for REDUCE
  <<statcounter := 0;
    date!* := compress('!" . append(explode2 "RLISP 3.3, ",
...
out "$cslbase/extras.lsp"$
on defn$
in "$cslbase/extras.red"$
off defn$
out t$
"extras.lsp created from extras.red";

out "$cslbase/compiler.lsp"$
on defn$
in "$cslbase/compiler.red"$
off defn$
out t$
"compiler.lsp created from compiler.red";

out "$cslbase/ccomp.lsp"$
on defn$
in "$cslbase/ccomp.red"$
off defn$
out t$
"ccomp.lsp created from ccomp.red";
