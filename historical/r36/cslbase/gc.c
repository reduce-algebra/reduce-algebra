/* File gc.c                    Copyright (c) Codemist Ltd, 1990-95 */

/*
 * Garbage collection.
 *    Fourth major version - now using Foster-style
 *    algorithm for relocating vector heap, and support for handling
 *    BPS via segmented handles. Pointer-reversing mark phase to go with same.
 *
...
/* Signature: 302ffcb4 07-Mar-2000 */
...
CSLbool gc_method;     /* YES if copying, NO if sliding */
int32 gc_number = 0;

static int32 cons_cells, symbol_heads, strings, user_vectors,
 ...
 * chain must be marked, word-aligned pointers.
 */
...
case TAG_VECTOR:
        ...
        i = (int32)doubleword_align_up(length_of_header(h));
        if (is_mixed_header(h)) 
            i = 16;  /* Only use first few pointers */
        while (i >= 8)
        {   i -= 4;
 ...
 */
            if (i == 4)
/*
 * When descending into the first (or only) word of a vector I heave the
...
scan_vector_more:
        ...
        {   Lisp_Object w2;
            ...
            p = *(Lisp_Object *)((char *)b - TAG_VECTOR + 4);
 ...
 */
            {   if (!is_immed(p) && p != nil)
                {   *(Lisp_Object *)((char *)b - TAG_VECTOR + 4) =
                        flip_mark_bit_p(w);
                    goto descend;
                }
                else
 ...
 */
            ...
            {   if (is_immed(w2)) w2 = flip_mark_bit_i(w2);
                ...
                *(Lisp_Object *)((char *)b - TAG_VECTOR + 4) = w;
                goto descend;
            }
 ...
 * to hurt me.
 */
        pp = (Lisp_Object *)((char *)p - TAG_VECTOR + i - 4);
        goto top;
    }
}
 ...
 */
    ...
    char *top_low = (char *)doubleword_align_up((int32)top_page),
         *bottom_low = (char *)doubleword_align_up((int32)bottom_page);
    char *top_start = top_low + CSL_PAGE_SIZE,
         *bottom_start = bottom_low + CSL_PAGE_SIZE;
    char *top_fringe = top_low + qcar(top_low),
         *bottom_fringe = bottom_low + qcar(bottom_low);
 ...
 * of type (unsigned) ought to mend things.
 */
            {   unsigned int len = (unsigned int)((char *)fringe - top_low);
                qcar(top_low) = (Lisp_Object)len;
 ...
 */
                ...
                bottom_low = (char *)doubleword_align_up((int32)bottom_page);
                bottom_start = bottom_low + CSL_PAGE_SIZE;
                fringe = (Lisp_Object)(bottom_low + qcar(bottom_low));
...
/* scan down from the top to find a free cell, unmarking is I go */
        ...
        {   top_start -= sizeof(Cons_Cell);
            ...
            {   top_page_number++;
                ...
                top_low = (char *)doubleword_align_up((int32)top_page);
                top_start = top_low + CSL_PAGE_SIZE;
                top_fringe = top_low + qcar(top_low);
                continue;
            }
            p = qcar(top_start);
            if (is_immed_or_cons(p) && !is_cons(p))
 ...
 */
{
    ...
    char *new_low = (char *)doubleword_align_up((int32)new_page);
    ...
    {   void *page = vheap_pages[page_number];
        ...
        *(Lisp_Object *)fr = nil;
        ...
        {   Header h;
            ...
            h1 = h;
            while (!is_odds(h1))
            {   Lisp_Object h2 = *(Lisp_Object *)clear_mark_bit_p(h1);
                if (is_vector(h2))
                    /* forwarding pointer for relocated cons cell */
                    h1 = (Lisp_Object)((char *)h2 - TAG_VECTOR + (h1 & 4));
                else h1 = h2;
            }
 ...
 * type and in bad cases the subtraction might overflow.
 */
            free = (unsigned int)(new_high - p);
            if (l > (int32)free)
            {   new_page_number++;
                new_page = vheap_pages[new_page_number];
                new_low = (char *)doubleword_align_up((int32)new_page);
 ...
 */
                if (is_vector(h1))
                    h = (Lisp_Object)((char *)h1 - TAG_VECTOR + (h & 4));
                else
    ...
    {   void *page = vheap_pages[page_number];
        ...
        *(Lisp_Object *)fr = set_mark_bit_h(TAG_ODDS + (8<<10));
        ...
        {   Header h;
            ...
            free = (unsigned int)(new_high - p);
            if (l > (int32)free)
            {   unsigned int len = (unsigned int)(p - new_low);
                *(int32 *)new_low = (int32)len;
/*
 ...
 */
            ...
            *(Lisp_Object *)p = clear_mark_bit_h(h);
            while ((l -= 4) != 0)
            {   p += 4;
                low += 4;
                *(int32 *)p = *(int32 *)low;
    ...
    {   unsigned int len = (unsigned int)(p - new_low);
#ifdef CLEAR_OUT_MEMORY
        unsigned int free = (unsigned int)(new_high - p);
#endif
        *(int32 *)new_low = (int32)len;
#ifdef CLEAR_OUT_MEMORY
        while (free != 0)
...
/*              relocate(&(s->pname));  can never be a cons cell */
        ...
        case TYPE_COMPLEX_NUM:
                relocate((Lisp_Object *)(low+4));
                relocate((Lisp_Object *)(low+8));
                other_mem += 8;
                break;
#endif
        case TYPE_MIXED1:
...
{
    char *p1 = (char *)doubleword_align_up((int32)p);
...
static int32 trailing_heap_pages_count,
 ...
 */
                    ...
                    {   char *hl = (char *)heaplimit;
                        void *p;
                        unsigned int len = (unsigned int)(fr - (hl - SPARE) + 8);
                        qcar(hl - SPARE) = (Lisp_Object)len;
                        qcar(fr) = SPID_GCMARK;
                        if (pages_count == 0)
                        {   abort();
                            return;
                ...
                {   char *d = data_of_bps(a) - 4, *rr;
                    ...
                    alloc_size = (int32)doubleword_align_up(len);
                    bytestreams += alloc_size;
                    for (;;)
                    {   char *cf = (char *)codefringe,
                             *cl = (char *)codelimit;
                        unsigned int free = (unsigned int)(cf - cl);
                        if (alloc_size > (int32)free)
                        {
                            void *p;
                            if (codelimit != 0)
                            {   unsigned int len = (unsigned int)(cf - (cl - 8));
                                qcar(cl - 8) = (Lisp_Object)len;
                            }
                            if (pages_count == 0)
                            {   abort();
                                return;
                            ...
                            cl = (char *)doubleword_align_up((int32)p);
                        ...
                        *(Header *)d = *p = TAG_BPS +
                        ...
                        /* Wow! How obscure!! */
                        *(Header *)rr = h;
                        {   unsigned32 *s = (unsigned32 *)d;
                            unsigned32 *d = (unsigned32 *)rr;
                            *++d = *++s;
    ...
    term_printf("Header is %.8lx\n", (long)h);
                ...
                {   char *vl = (char *)vheaplimit;
                    unsigned int free = (unsigned int)(vl - vfr);
                    if (len > (int32)free)
                    {   unsigned int free1 = (unsigned int)(vfr - (vl - (CSL_PAGE_SIZE - 8)));
                        qcar(vl - (CSL_PAGE_SIZE - 8)) = (Lisp_Object)free1;
                        ...
                        vfr = (char *)doubleword_align_up((int32)p1) + 8;
                        vl = vfr + (CSL_PAGE_SIZE - 16);
                        vheaplimit = (Lisp_Object)vl;
                        free1 = (unsigned int)(vfr - (vl - (CSL_PAGE_SIZE - 8)));
                        qcar(vl - (CSL_PAGE_SIZE - 8)) = (Lisp_Object)free1;
                        continue;
                    }
                    *(Lisp_Object *)a = *p = (Lisp_Object)(vfr + tag);
                    *(Header *)vfr = h;
                    {   Lisp_Object *s = (Lisp_Object *)a;
                        Lisp_Object *d = (Lisp_Object *)vfr;
                        vfr = vfr + len;
                        *++d = *++s;
                        while ((len -= 8) != 0)
                        {   *++d = *++s;
        ...
        case CONT:
                ...
                {   tr_fr = tr_fr - sizeof(Cons_Cell);
                    ...
                    {   char *w;
                        ...
                        w = (char *)doubleword_align_up((int32)p1);
                ...
                {   Header h;
                    ...
                    {   int32 len = doubleword_align_up(length_of_header(h));
                        tr = tr_vfr;
                        tr_vfr = tr_vfr + len;
                        switch (type_of_header(h))
                        {
        ...
        {   void *page = vheap_pages[j];
            ...
            {   Header h = *(Header *)low;
                ...
                {   Lisp_Object e = qenv(low + TAG_SYMBOL);
                    ...
                    {   e = qcar(e);
                        if (is_bps(e))
                        {   Header ch = *(Header *)(data_of_bps(e) - 4);
 ...
 */
                ...
                else low += (int32)doubleword_align_up(length_of_header(h));
            }
        }
    }
    if ((what & 6) == 0)
...
static CSLbool reset_limit_registers(int32 vheap_need,
                                  int32 bps_need, int32 native_need)
/*
 * returns YES if after resetting the limit registers there was
 * enough space left for me to proceed.
 */
{
    void *p;
    nil_as_base
    ...
    {   char *vh = (char *)vheaplimit,
             *vf = (char *)vfringe;
        len = (unsigned int)(vh - vf);
    }
    if (vheap_need > (int32)len)
    {   char *vf, *vh;
        if (full) return NO;
        p = pages[--pages_count];
        zero_out(p);
        vheap_pages[vheap_pages_count++] = p;
        vf = (char *)doubleword_align_up((int32)p) + 8;
        vfringe = (Lisp_Object)vf;
        vh = vf + (CSL_PAGE_SIZE - 16);
        vheaplimit = (Lisp_Object)vh;
        len = (unsigned int)(vf - (vh - (CSL_PAGE_SIZE - 8)));
        qcar(vh - (CSL_PAGE_SIZE - 8)) = (Lisp_Object)len;
    }
    {   char *cl = (char *)codelimit,
             *cf = (char *)codefringe;
        len = (unsigned int)(cf - cl);
    }
    if (bps_need != 0 && bps_need >= (int32)len)
    {   char *cl;
...
{   nil_as_base
    char *fr = (char *)fringe,
         ...
         *vl = (char *)vheaplimit,
         *cl = (char *)codelimit;
    unsigned int len = (unsigned int)(fr - (hl - SPARE));
    qcar(hl - SPARE) = (Lisp_Object)len;
    len = (unsigned int)(vf - (vl - (CSL_PAGE_SIZE - 8)));
    qcar(vl - (CSL_PAGE_SIZE - 8)) = (Lisp_Object)len;
    if (codelimit != 0)
    {   len = (unsigned int)(cf - (cl - 8));
        qcar(cl - 8) = (Lisp_Object)len;
    }
}

static void lose_dead_hashtables()
/*
...
Lisp_Object reclaim(Lisp_Object p, char *why, int stg_class, int32 size)
{
    int32 i;
    clock_t t0, t1, t2, t3;
    int bottom_page_number;
    Lisp_Object *sp, nil = C_nil;
    int32 vheap_need = 0, bps_need = 0, native_need = 0;
    stop_after_gc = 0;
 ...
 */
            ...
            heaplimit = doubleword_align_up((int32)pp);
            qcar(heaplimit) = CSL_PAGE_SIZE;
            vl = (char *)heaplimit;
 ...
 */
            ...
            vf = (char *)doubleword_align_up((int32)pp) + 8;
            vfringe = (Lisp_Object)vf;
 ...
 * heap to fit into.
 */
        copy(&((Lisp_Object *)nil)[current_package_offset]);
/*
 ...
 */
            pun = (int32)page;
            pun1 = (int32)((char *)page + CSL_PAGE_SIZE + 8);
