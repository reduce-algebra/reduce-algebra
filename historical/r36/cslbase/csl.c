/*  csl.c                            Copyright (C) 1989-99 Codemist Ltd */
...
/* Signature: 22abf0f6 07-Mar-2000 */
...
{
    Lisp_Object nil = C_nil, w;
    if (miscflags & HEADLINE_FLAG) err_printf("+++ Error bad args for %s\n", s);
    if ((w = qvalue(break_function)) != nil &&
        symbolp(w) &&
        qfn1(w) != undefined1)
    {   (*qfn1(w))(qenv(w), nil);
...
{
    Lisp_Object nil = C_nil, w;
    if (miscflags & HEADLINE_FLAG) err_printf("+++ Error: %s\n", s);
    if ((w = qvalue(break_function)) != nil &&
        symbolp(w) &&
        qfn1(w) != undefined1)
    {   (*qfn1(w))(qenv(w), nil);
 ...
 */
                    ...
                    else init_flags &= ~INIT_EXPANDABLE;
                    if (*w == '/')
