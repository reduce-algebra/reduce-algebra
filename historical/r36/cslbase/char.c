/*  char.c                           Copyright (C) 1989-99 Codemist Ltd */

/*
 * Character handling.
 */

/* Signature: 786dda9c 18-Feb-1999 */
...
#define characterify(c)                                \
    if (is_symbol(c)) c = pack_char(0,0,               \
...
#else
#define characterify(c) \
    if (is_symbol(c)) c = pack_char(0,0, ucelt(qpname(c), 0))
#endif
...
static bool chartest(Lisp_Object c)
{
    if (!is_char(c))
