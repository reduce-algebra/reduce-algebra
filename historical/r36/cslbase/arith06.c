/*  arith06.c                         Copyright (C) 1990-99 Codemist Ltd */
...
/* Signature: 787eef23 07-Mar-2000 */
...
{
    if (is_fixnum(a))
    {   unsigned32 r = (unsigned32)a + 0x10;
        /* fixnums have data shifted left 4 bits */
        if (r == ~0x7ffffffe)     /* The ONLY possible overflow case here  */
            a = make_one_word_bignum(1 + int_of_fixnum(a));
        else return onevalue((Lisp_Object)r);  /* the cheap case */
    }
 ...
 */
{
    int32 top, bottom, kk, bits;
    int32 rtop = 0, rbottom = 0;
    CSLbool was_fixnum = NO, was_negative = NO, round_up;
    if (is_fixnum(k) && (int32)k >= 0) kk = int_of_fixnum(k);
 ...
 */
        ...
        {   int32 wk1 = (kk-1) / 31, bk1 = (kk-1) % 31;
            int32 bit = ((int32)1) << bk1;
...
Lisp_Object Lplus2(Lisp_Object nil, Lisp_Object p, Lisp_Object q)
{
    ...
    {   int32 c = int_of_fixnum(p) + int_of_fixnum(q);
        int32 w = c & fix_mask;
...
Lisp_Object Levenp(Lisp_Object nil, Lisp_Object a)
{
    ...
    {
case TAG_FIXNUM:
        return onevalue(((int32)a & 0x10) == 0 ? lisp_true : nil);
case TAG_NUMBERS:
        if (is_bignum(a))
 ...
 * 32-bit linear congruential mistakes.
 */

static unsigned32 random_number_seed[55];
 ...
 * the implicit call of this nature when CSL starts doe snot do much),
...
static void randomize()
{
    ...
    {   MD5_Init();
        ...
        MD5_Final((unsigned char *)&random_number_seed[i]);
...
void Csrand(unsigned32 seed, unsigned32 seed2)
{
/*
 ...
 */
    MD5_Init();
    MD5_Update((unsigned char *)random_number_seed,
               4*sizeof(random_number_seed[0]));
    MD5_Final((unsigned char *)&random_number_seed[0]);
/*
 * The remainder of the vector gets filled using a simple linear
 * congruential scheme.
 */
    i = 4;
    seed = random_number_seed[0];
    while (i<55)
 ...
 */
        p = v*(0x7fffffff/v);
        do q = ((unsigned32)Crand()) >> 1; while (q > p);
    ...
    restart:
        ...
        for (;;)
        {   w = (0xffffffffU/((unsigned32)msd+1U))*((unsigned32)msd+1U);
            do w1 = (unsigned32)Crand(); while (w1 >= w);
            w1 = w1%((unsigned32)msd+1U);
            bignum_digits(r)[len] = w1;
            if ((int32)w1 != msd) break;
/*
 ...
 */
        for (len--;len>=0; len--)
            bignum_digits(r)[len] = ((unsigned32)Crand())>>1;
 ...
 */
        do
        {   v = ((double)(int32)(Crand() & 0x7fffffff)) / TWO_31;
            v += (double)(int32)(Crand() & 0x7fffffff);
            v /= TWO_31;
            v *= d;
        } while (v == d);
 ...
 */
        do
        {   v = (float)(int32)(Crand() & 0x7fffffff)/(float)TWO_31;
            v = v*d.f;
        } while (v == d.f);
        d.f = v;
        return onevalue((d.i & ~(int32)0xf) + TAG_SFLOAT);
    }
...
Lisp_Object Lmd5(Lisp_Object env, Lisp_Object a)
{
    ...
    {   len = length_of_header(numhdr(a));
        MD5_Init();
        for (i=4; i<len; i+=4)
        {   sprintf((char *)md, "%.8lx", (unsigned long)bignum_digits(a)[(i-4)/4]);
            MD5_Update(md, 8);
        }
    }
    ...
    {   len = length_of_header(vechdr(a));
        MD5_Init();
        MD5_Update((unsigned char *)(a - TAG_VECTOR + 4), len-4);
    }
    else checksum(a);
    MD5_Final(md);
 ...
 */
    if (v4 != 0 || (v3 & 0x40000000) != 0) len = 24;
    else if (v3 != 0 || (v2 & 0x40000000) != 0) len = 20;
    else if (v2 != 0 || (v1 & 0x40000000) != 0) len = 16;
    else if (v1 != 0 || (v0 & 0x40000000) != 0) len = 12;
    else if ((v0 & fix_mask) != 0) len = 8;
    ...
    {
case 24:
case 20: bignum_digits(r)[4] = v4; /* zeros out padding word as necessary */
         bignum_digits(r)[3] = v3;
case 16:
case 12: bignum_digits(r)[2] = v2;
         bignum_digits(r)[1] = v1;
case 8:  bignum_digits(r)[0] = v0;
         break;
    }
...
Lisp_Object Lmd60(Lisp_Object env, Lisp_Object a)
{
    ...
    v0 &= 0x7fffffff;
    if (v1 != 0 || (v0 & 0x40000000) != 0) len = 12;
    else if ((v0 & fix_mask) != 0) len = 8;
    else return onevalue(fixnum_of_int(v0));
    r = getvector(TAG_NUMBERS, TYPE_BIGNUM, len);
    errexit();
