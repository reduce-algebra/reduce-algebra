...
\begin{quotation}
\noindent
\f{MKLIST KERNLIST FREQUENCY SEQUENCES \nl
...
ODDP FOLLOWLINE  == RANDOMLIST \\
PERMUTATIONS CYCLICPERMLIST COMBNUM COMBINATIONS  \\
SYMMETRIZE REMSYM SORTNUMLIST SORTLIST EXTREMUM \\
DEPATOM FUNCVAR IMPLICIT EXPLICIT KORDERLIST \\
SIMPLIFY CHECKPROPLIST EXTRACTLIST}
\end{quotation}
...
an ``unbrokable'' one and, for that reason, behaves
...
variables of which it depends. Constants are eliminated.
\begin{verbatim}
...
it returns the list of variables which the various depend declarations
imply.
\begin{verbatim}
...
\end{verbatim}
\item[iv.] A function \f{SIMPLIFY} which takes an arbitrary expression
is available which {\em forces} down-to-the-botttom simplification of
an expression. It is usefull with \f{SYMMETRIZE}. It has also proved
usefull to simplify some output expressions of the package
EXCALC\ (E. Schruefer). Here is an example:
\begin{verbatim}
...
{\bf interactively}. To functions are provided:\\
...
algebraic mode desappear in the symbolic form (there it becomes
...
an elementary facility to write polynomials in a distributive form. So,
...
one for hyperbolic functions \f{HYPREDUCE} and \f{HYPEXPAND}
...
\end{verbatim}
\item[ii.]
Two functions for the manipulation of logarithmic expression are
available. They are \f{PLUSLOG} and \f{CONCSUMLOG}.
Both function take an arbitrary expression as argument. The first one
transforms products, ratios and exponentiations which appear under a
\verb+log+ respectively into sums,  differnces of log's and products
of exponents with log's. The second one does the opposite job.
\ei
...
\f{MKIDM} is a generalization of \f{MKID}. It allows to connect
...
\end{verbatim}
This function allows to make loops on matrices like in the following
