...
der(~n)*del(~n) => if n neq 3 then 0 else if n eq 3 then d(1)^2,
del(~n)*der(~n) => if n neq 3 then 0 else if n eq 3 then d(1)^2,
...
prykr(~f,~g)    => if g eq 1 then der(1)*f else if g eq 2 then der(2)*f,
prykl(~f,~g)    => if g eq 1 then f*del(1) else if g eq 2 then f*del(2)}$
%adjoint
...
tryk1:={zan(~f,0,~m) => (-1)**m*d(1)**m,
       zan(~f,3,~m) =>
               if abra_kadabra eq 2 then (-1)**m*der(1)*der(2)*d(1)**m
         else  if abra_kadabra eq 1 then (-1)**(m+1)*der(2)*der(1)*d(1)**m
         else  if abra_kadabra eq 3 then (-1)**m*der(3)*d(1)**m,
...
!@f_f(~f,1,~n)    =>    if abra_kadabra eq 2 then bos(f,1,n) else
                        if not freeof(f_chiral,f) then 0 else bos(f,1,n),
!@f_f(~f,2,~n)    =>    if abra_kadabra eq 2 then bos(f,2,n) else
                        if not freeof(f_antychiral,f) then 0 else  bos(f,2,n),
!@f_f(~f,3,~n)    =>    if abra_kadabra eq 2 then bos(f,3,n) else
if not freeof(b_chiral,f) then - bos(f,0,n+1) else
if abra_kadabra eq 1 and not freeof(b_antychiral,f) then 0 else
if abra_kadabra eq 3 and not freeof(b_antychiral,f) then bos(f,0,n+1) else
bos(f,3,n),
!@g_g(~f,0,~n)    => fer(f,0,n),
!@g_g(~f,1,~n)    =>    if abra_kadabra eq 2 then fer(f,1,n) else
                        if not freeof(b_chiral,f) then 0 else fer(f,1,n),
!@g_g(~f,2,~n)    =>    if abra_kadabra eq 2 then fer(f,2,n) else
                        if not freeof(b_antychiral,f) then 0 else fer(f,2,n),
!@g_g(~f,3,~n)    =>    if abra_kadabra eq 2 then fer(f,3,n) else
if not freeof(f_chiral,f) then -fer(f,0,n+1) else
if abra_kadabra eq 1 and not freeof(f_antychiral,f) then 0 else
if abra_kadabra eq 3 and not freeof(f_antychiral,f) then fer(f,0,n+1) else
fer(f,3,n)}$
...
bos(~f,3,~n) => if abra_kadabra eq 1 then
   if not freeof(b_chiral,f) then - bos(f,0,n+1) else
...
else if abra_kadabra eq 3 then
   if not freeof(b_chiral,f) then - bos(f,0,n+1) else
...
bos(~f,3,~n,~k) => if abra_kadabra eq 1 then
...
fer(~f,3,~n) => if abra_kadabra eq 1 then
        ...
        tet(2)*tet(1)*gras(mkid(f,mkid(f,2)),n+1)/2
else if abra_kadabar eq 3 then
        if not freeof(f_chiral,f) then - fer(f,0,n+1)
...
procedure pr(n,ww);
       ...
       if n=3 then if abra_kadabra eq 3 then ewa:=der(3)*ww else
        ewa:=der(1)*pr(2,ww);
...
procedure fpart(wx);
       ...
       ewa:=if abra_kadabra eq 2 then (wx where tryk5) else (wx where tryk16);
...
procedure koza(wx,wi,wn);
        ...
        am:=
if ew3 eq f and wi eq 0 or ew3 eq f and wi eq 3 then
                !@x_y*!@g_g(ew1,wi,wn) else
if ew3 eq f and wi eq 1 or ew3 eq f and wi eq 2 then
                !@x_y**2*!@f_f(ew1,wi,wn) else
if ew3 eq b and wi eq 0 or ew3 eq b and wi eq 3 then
                !@x_y**2*!@f_f(ew1,wi,wn) else
if ew3 eq b and wi eq 1 or ew3 eq b and wi eq 2 then
...
procedure w_comb(as,m,a,bb);
       ...
       if wod1 eq 0 then 0 else  part(wod1,k);
       ola:=if adam eq 0 then 0 else
       if part(adam,0)=minus then -adam else adam;
       wx:=if ola eq 0 then 1 else
...
(w_comb(ww,k,mkid(mkid(a,k),a),b)+
 ...
 w_comb(ww,k-1,mkid(mkid(a,k),d),b)*(if abra_kadabra eq 3 then
der(3) else  der(1)*der(2)))*d(1)**(n-k);>>;
        remfac fer,bos; kap:=length(ewa); ola:=0;
        for k:=1:kap do << maj:=if ewa eq 0 then 0 else
        if kap equal 1 then ewa else part(ewa,k);
        maj1:=if maj eq 0 then 1 else (maj where tryk4);
        ola:=ola+ss(k+1)*maj/maj1;>>;
        ela:=ss(0)*d(1)**n+ss(1)*(if abra_kadabra eq 3 then der(3) else
        der(1)*der(2))*d(1)**(n-1)+ola;factor fer,bos;
        if abra_kadabra eq 3 then ela:=(ela where {der(1)*der(2) => der(3)});
       return ela end$
%********************************************%
%*** module jacobi  *************************%
%********************************************%
...
procedure jacob(p,w,m);
      ...
      operator !#a,!#b,!#c,!@a,!@b,!@c;
       ...
       as1:=for k:=1:as collect for l:=1:as sum
        ...
        if wx eq 1 then !@b(l) else
        if wx eq 2 then !@c(l) else
        if wx eq 3 then !@a(l),0,0));
       for k:=1:as do <<  bos(part(w,k)):=part(as1,k);>>;
       !@ela:=(!@ala where tryk10);
       wod:=for k:=1:as sum for l:=1:as sum
bos(if wx eq 1 then !@c(k) else
    if wx eq 2 then !@a(k) else
    if wx eq 3 then !@b(k),0,0)*!@ela(k,l)*
bos(if wx eq 1 then !@a(l) else
    if wx eq 2 then !@b(l) else
    if wx eq 3 then !@c(l),0,0);
