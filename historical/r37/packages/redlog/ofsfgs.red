% ----------------------------------------------------------------------
% $Id: ofsfgs.red,v 1.9 1999/03/23 07:41:37 dolzmann Exp $
% ----------------------------------------------------------------------
...
lisp <<
   ...
   ofsf_gs_rcsid!* := "$Id: ofsfgs.red,v 1.9 1999/03/23 07:41:37 dolzmann Exp $";
   ofsf_gs_copyright!* := "Copyright (c) 1995-1999 by A. Dolzmann and T. Sturm"
>>;
...
procedure ofsf_gsc(f,atl);
   ...
   % somehow simpler than [f]. This procedure sets the switch [groebopt]
   % temporary off. The parameters defined by [torder] are temporarily
   % overloaded by a [torder({},revgradlex)] statement.
...
procedure ofsf_gsd(f,atl);
   ...
   % atomic formulas, which are considered to describe a theory. An
   % formula equivalent to [f] or ['inctheo] is returned. The
   % returned formula is somehow simpler than [f]. This procedure sets
   % the switch [groebopt] temporary off. The parameters defined by
   % [torder] are temporary overloaded by a [torder({},revgradlex)]
   % statement.
   begin scalar w,svrlgsvb;
      svrlgsvb := !*rlgsvb;
      if !*rlverbose and !*rlgsvb then on1 'rlgsvb else off1 'rlgsvb;
...
procedure ofsf_gsn(f,atl);
   ...
   % formulas, which are considered to describe a theory. An
   % formula equivalent to [f] or ['inctheo] is returned. The
   % returned formula is somehow simpler than [f]. This procedure sets
   % the switch [!*groebopt] temporary off. The parameters defined by
   % [torder] are temporary overloaded by a [torder({},revgradlex)]
   % statement. This procedure calls in dependency of the structure of
   % [f] either [ofsf_gsc] or [ofsf_gsd]. The following heuristic is
   % used: Is [f] a conjunction of atomic formulas or a disjunction of
   % formulas with at least one complex formula then [ofsf_gsd] is
   % called; in other cases [ofsf_gsc] is called.
   if rl_tvalp f then
      f
...
procedure ofsf_gssimplify0(f,atl);
   ...
   % theory. A formula is returned. This procedure binds temporarily
   % some global variables of the groebner packages.
   begin scalar res,oldtorder,!*groebopt,svkord;
      svkord := kord!*;
      oldtorder := cdr torder('((list) revgradlex));
      if !*rlgsutord then
	 torder(oldtorder);
      res := ofsf_gssimplify(f,atl);
      torder oldtorder;
      kord!* := svkord;
      return res;
   end;
