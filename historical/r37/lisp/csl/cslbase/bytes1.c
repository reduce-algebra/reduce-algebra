/*
 * bytes1.c              Copyright (C) 1991-2002, Codemist Ltd
 *
 *
...
/* Signature: 6aaf56de 10-Oct-2002 */
...
static Lisp_Object *do_freebind(Lisp_Object bvec, Lisp_Object *stack)
{
   ...
   {   Lisp_Object v = *(Lisp_Object *)((intxx)bvec + k - TAG_VECTOR);
       push(qvalue(v));
       qvalue(v) = C_nil;
   }
/*
...
static Lisp_Object *do_freerstr(Lisp_Object *stack)
{
    ...
    while (n>CELL)
    {   Lisp_Object v = *(Lisp_Object *)((intxx)bv + n - (CELL + TAG_VECTOR));
        n -= CELL;
        pop(qvalue(v));
    }
    return stack;
...
#ifdef MEMORY_TRACE
#define next_byte (cmemory_reference((intxx)ppc), *ppc++)
#else
