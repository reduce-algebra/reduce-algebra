/*  arith06.c                         Copyright (C) 1990-2002 Codemist Ltd */
...
/* Signature: 1e60864f 10-Oct-2002 */
 ...
 */
    if (is_fixnum(a) && is_fixnum(b))
        return onevalue(Lispify_predicate((int32)a<(int32)b));
    r = lessp2(a, b);
    errexit();
    return onevalue(Lispify_predicate(r));
}
...
Lisp_Object Lmd5(Lisp_Object env, Lisp_Object a)
{
    ...
    {   len = length_of_header(vechdr(a));
        MD5_Init();
        MD5_Update((unsigned char *)(a + CELL - TAG_VECTOR), len-CELL);
    }
    else checksum(a);
    MD5_Final(md);
