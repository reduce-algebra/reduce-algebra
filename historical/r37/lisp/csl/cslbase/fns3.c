/*  fns3.c                          Copyright (C) 1989-2002 Codemist Ltd */
...
/* Signature: 679133c1 10-Oct-2002 */
...
case 4:
    ...
    p = (hashcode % (unsigned32)(size >> 1))*2;
/*
 ...
 * a non-zero stride).
 */
    hashstride = (1 + (hashcode % (unsigned32)((size >> 1)-1)))*2;
    hashgap = -1;
...
case 4:
    ...
    p = (hcode % (unsigned32)(size >> 1))*2;
    hstride = (1 + (hcode % (unsigned32)((size >> 1)-1)))*2;
/*
...
Lisp_Object Lsmkvect(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("make-simple-string", n);
    nn = int_of_fixnum(n);
    w = getvector(TAG_VECTOR, TYPE_STRING, nn+CELL);
    errexit();
    nn = (intxx)doubleword_align_up(nn+CELL);
    while (nn > CELL)
    {   nn -= CELL;
        *(intxx *)((char *)w - TAG_VECTOR + nn) = 0;
    }
    return onevalue(w);
}
...
Lisp_Object Lmkvect8(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("mkvect8", n);
    nn = int_of_fixnum(n);
...
Lisp_Object Lmkvect16(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("mkvect16", n);
...
Lisp_Object Lmkvect32(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("mkvect32", n);
...
Lisp_Object Lmkfvect32(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("mkfvect32", n);
    nn = 4*int_of_fixnum(n);
...
Lisp_Object Lmkfvect64(Lisp_Object nil, Lisp_Object n)
{
    Lisp_Object w;
    intxx nn;
    if (!is_fixnum(n) || (intxx)n<0) return aerror1("mkfvect64", n);
    nn = 8*int_of_fixnum(n);
#ifndef ADDRESS_64
    nn += 4; /* get the doubles aligned */
#endif
 ...
 */
{
    ...
    pop(s);
    errexit();
    i = (intxx)doubleword_align_up(n+CELL);
    while (i > CELL) /* pre-fill target vector with zero */
    {   i -= CELL;
        *(intxx *)((char *)w - TAG_VECTOR + i) = 0;
    }
    h1 = elt(s, 3);
 ...
 */
        *(int16 *)((char *)p + o) = (int16)int_of_fixnum(x);
        break;
#endif
case 4:
        *(int32 *)((char *)p + o) = (int32)int_of_fixnum(x);
        break;
    }
...
Lisp_Object Lnative_address1(Lisp_Object nil, Lisp_Object x)
{
    ...
    {   if (!is_fixnum(x)) return aerror1("native-address", x);
        ...
        {   n = (-n) - 1;
            ...
            else p = (intxx)useful_functions[n];
        }
        else p = (intxx)address_of_var(n);
    }
    n1 = p & fix_mask;
...
static Lisp_Object MS_CDECL Lmake_string(Lisp_Object nil, int nargs, ...)
{
    ...
    w = getvector(TAG_VECTOR, TYPE_STRING, nn+CELL);
    errexit();
    z = (int32)doubleword_align_up(nn+CELL);
