...
(de c!:find_literal (x)
   (prog (n w)
      ...
      (setq n 0)
      (prog nil
   !G49  (cond ((not (and w (not (equal (car w) x)))) (return nil)))
...
(de c!:cfndef (current_procedure current_c_name args body)
   (prog (env n w current_args current_block restart_label current_contents
         ...
         literal_vector reloadenv does_call blockstack proglabs)
      (c!:reset_gensyms)
      (wrs c_file)
      (linelength 200)
   ...
   lab   (cond ((null x) (return nil)))
      ...
      (wrs o_file)
      (setq l_contents
         (cons (cons current_procedure literal_vector) l_contents))
      (return nil)))
...
(de c!-end nil
   (prog nil
      ...
      (prog nil
   !G52  (cond ((not defnames) (return nil)))
...
(de c!:does_return (res why where_to)
   (cond
      ...
      (t (prog (contents)
            ...
            (prog nil
         !G62  (cond ((not contents) (return nil)))
               (cond
...
(de c!:pushpop (op v)
   (prog (n w)
      ...
      (prog nil
   !G63  (cond ((not (greaterp n 0)) (return nil)))
         (progn
            (setq w n)
      ...
      (cond
         (stacks
            ...
            lab   (cond ((null v) (return nil)))
               ...
               (setq w n)
               (prog nil
            !G64  (cond ((not (geq w 5)) (return nil)))
                  (progn
                     (c!:printf1
...
(de c!:cfunction (u env)
   (prog (v)
      (setq u (cadr u))
      (cond ((not (atom u)) (error 0 "function/funarg needed")))
      (setq v (c!:newreg))
...
(de c!:cgo (u env)
   (prog (w w1)
      (setq w1 proglabs)
      (prog nil
   !G65  (cond ((not (and (null w) w1)) (return nil)))
...
(progn
   ...
   (put 'make!-random!-state 'c!:c_entrypoint "Lmake_random_state")
