...
%%     if length ans1 = 1 then oneterm := t; % Efficiency
       ...
       % We have run down the terms.  If there are any bad bits,
       % redo them.
       if badbit neq '(nil . 1)
         then <<
          if !*trdint then <<princ "Retrying..."; printsq badbit>>;
          if oneterm then ans1 := nil
      ...
      if (v := formlnr u) neq u
        then if !*nolnr
               then <<v:= simp subst('int!*,'int,v);
                     ...
                     u:=errorset!*(list('simp,mkquote v),!*backtrace);
...
fluid '(!*surds kord!* zlist);  % clogflag
...
symbolic procedure int!-fac x;
   ...
   % throughout most of the integrator we want to add new surds
   begin scalar !*surds;
      !*surds := t;
