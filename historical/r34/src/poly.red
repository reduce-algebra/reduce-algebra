...
symbolic procedure multfnc(u,v);
   ...
   begin scalar x,y;
      ...
      if null x then nil
        ...
        then x := addf(if null (y := mkspm(mvar u,ldeg u+ldeg v))
                         then nil
...
symbolic procedure divdm(v,u);
   ...
   if atom v and atom u
     then if remainder(v,u)=0 then v/u
           else (if !*rounded and !*roundall then !*rn2rd x else x)
                 where x=mkrn(v,u)
...
fluid '(!*exp !*ezgcd !*gcd !*heugcd !*mcd dmode!*);
...
%      else if quotf1(v,u) then return u;
      w := gcdf2(u,v);
      if !*gcd and (null quotf1(u,w) or null quotf1(v,w))
...
%         assoc(u,asymplis!*);
   (if domainp x then !:expt(x,n) else exptf1(x,n)) where x=mkprod u;
...
symbolic procedure !:difference(u,v);
   if null u then -v else if null v then u
       ...
       if atom u and atom v then u-v else dcombine(u,v,'difference);
...
% These commands are for updating purposes.  They will be removed
% shortly.

remprop('repart,'polyfn);

remprop('impart,'polyfn);

remprop('conj,'polyfn);

put('repart,'simpfn,'simprepart);

% True beginning of code.
...
symbolic procedure mkrepart u;
   if eqcar(u,'impart) or eqcar(u,'repart) then !*k2q u
    else mksq(list('repart, u),1);

symbolic procedure mkimpart u;
   if eqcar(u,'repart) or eqcar(u,'impart) then nil ./ 1
...
symbolic procedure reimexpt u;
   if cadr u eq 'e
      then addsq(reimcos {'cos,reval {'times,'i,caddr u}},
                 multsq(simp {'minus,'i},
                        reimsin {'sin,reval {'times,'i,caddr u}}))
    else addsq(mkrepart u,
               multsq(simp 'i,mkimpart u));
        ...
        then <<if null dmode!* then y := 'factorf
               ...
               if y
                 then <<y := apply1(y,car u);
                        return (car y . for each j in cdr y
                                           collect(car j . cdr u))>>>>;
      return factor!-prim!-sqfree!-f!-1(car u,cdr u)
   end;
...
symbolic procedure sqfrf u;
   ...
   % form power list.
   begin integer n; scalar !*gcd,v,w,x,y,z;
      !*gcd := t;
      n := 1;
      x := mvar u;
      v := gcdf(u,diff(u,x));
...
fluid '(!*surds dmode!* kernlist!* knowndiscrimsign);  % clogflag
...
%  (if degree = 2 and (null !*surds or clogflag) then quadraticf pol
  (if degree = 2 and null !*surds then quadraticf pol
...
%            J von zur Gathen: Functional Decomposition of
%            Polynomials: the Tame Case, J. Symbolic Computation
%            (1990) 9, 281-299.
...
symbolic procedure decomposesfmulti1(f,vars);
  % Multivariate case: map to strongly monic polynomial.
    begin scalar lvars,ft,rt,x1,a0,kord,u;
     integer n,m;
    ...
    again:
     ft := numr subf(f,for each x in lvars collect
           (car x . list('plus,cdr x,list('times,x1,m))));
     ...
     <<rt := reverse rt;
       rt := numr  subf(car rt,for each x in lvars collect
              (cdr x . list('difference,car x,list('times,m,x1))))
