...
symbolic procedure copyd(new,old);
   ...
   begin scalar x;
      x := getd old;
      if null x
        then rerror('rlisp,1,list(old,"has not definition in copyd"));
...
symbolic procedure toknump x; numberp x or eqcar(x,'!:dn!:);
...
symbolic procedure scan;
   begin scalar bool,x,y;
    ...
    c:  if null idp nxtsym!* then go to l
         ...
         else if nxtsym!* eq '!' then errach "Invalid QUOTE"
...
symbolic procedure eolcheck;
   ...
   a: if nxtsym!* eq !$eol!$ then progn(nxtsym!* := token(), go to a)
     end;
...
fluid '(!*defn !*echo !*fort !*int !*msg !*nat);

global '(cursym!* erfg!* ofl!* outl!* !*protfg);
...
symbolic procedure formbool(u,vars,mode);
   if mode eq 'symbolic then formc(u,vars,mode)
    ...
    else if car u eq 'boolvalue!* then errach("Too many formbools") 
    else list('boolvalue!*,formc!*(u,vars,mode));
...
%     x := convertmode(cadr u,vars,target,mode);
      ...
      return if not atom z
        then if not idp car z then typerr(z,"assignment")
          else if arrayp car z
           then list('setel,intargfn(z,vars,mode),x)
          else if cdr z and (get(car z,'rtype) eq 'vector
             or vectorp cadr z
             or flagpcar(cadr z,'vecfn))
           then putvect(u,vars,mode)
          else if y := get(car z,'setqfn)
           then form1((y . append(cdr z,cdr u)),vars,mode)
          else if mode eq 'symbolic and !*rlisp88
           then list('setf,form1(z,vars,mode),x)
             % Use SETF for symbolic non-id assignments in RLISP88, even
             % though it is not defined in Standard Lisp.
    ...
    else if flagp(z,'reserved) then rsverr z
...
symbolic procedure formfor(u,vars,mode);
   begin scalar action,algp,body,endval,incr,initval,var,x;
      ...
      return forformat(action,body,initval,x,
                       if algp then list('aeval,list('list,''plus,incr))
                        else list(if !*fastfor then 'iplus2
                                               else 'plus2,
                                  incr),
                       var,vars,mode)
   end;
...
symbolic procedure forformat(action,body,initval,
   ...
   begin scalar result;
% to get (aeval (list 'plus X 1))
% rather than (aeval X (list 'plus 1))
      if car updform eq 'aeval and length cadr updform > 2
         then <<
           result:=gensym();
...
symbolic procedure repeatstat;
  begin scalar !*blockp,body,bool;
        if flagp('until,'delim) then bool := t else flag('(until),'delim);
        body:= xread t;
...
symbolic procedure listeval(u,v);
   <<if (simpcount!* := simpcount!*+1)>simplimit!*
     ...
     u := if atom u
            ...
            then car u . for each x in cdr u collect reval1(x,v)
...
symbolic procedure in1(fl,echop);
   begin scalar chan,echo,ochan,type;
      ...
      if null ipl!* and contl!* then return nil
        ...
        then errach list("FILE STACK CONFUSION",fl,ipl!*)
       else ipl!* := cdr ipl!*
   end;
