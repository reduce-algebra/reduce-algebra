...
% defautoload(compd,compiler,expr,3);   % already in PSL
...
put('nonlnr,'simpfg,'((t (load!-package 'groebner))));
...
defautoload(nearestroot,roots,expr,2);

defautoload(gfnewt,roots,expr,3);

defautoload(gfroot,roots,expr,3);

defautoload(sturm,roots);

flag('(roots firstroot nearestroot gfnewt gfroot sturm),'opfn);

deflist('((rlrootno rlrootno)(realroots realroots)(isolater isolater)),
        'psopfn);
...
defautoload(limit,misc,expr,3);

defautoload(limit!+,misc,expr,3);

defautoload(limit!-,misc,expr,3);

symbolic operator limit,limit!+,limit!-;
