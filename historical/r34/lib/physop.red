...
if (u = '(nil)) then oporder!* := defoporder!*  % default list
         ...
         oporder!* := append(list(x),physopdelete(x,oporder!*)) >>;
...
if idp u then x:= explode u
else <<x:=explode car u;
        z:= cdr u>>;
x:=
   if (n:= locate('!-,x)) and (nth!*(x,n+1) = '!1) then
          invp intern compress x
   else intern compress x;
if  member(x,specoplist!*) then return t;
if z then << x:= x . z;  z:= nil>>;
if idp v then y:= explode v
else << y:= explode car v;
        z:=  cdr v>>;
y:= if (n:= locate('!-,y)) and (nth!*(y,n+1) = '!1) then
            invp intern compress y
    else intern compress y;
if member(y,specoplist!*) then return t;
if z then << y:= y . z;  z:= nil>>;
% check if x is on the oporder list
z:= physopmember(x,oporder!*);
if null z or null oporder!* then
  rederr2('ordop,
  list("something is wrong with the oporder list",oporder!*));
% now compare position of y
if x = y or physopmember(y,cdr z)  then return t
else if physopmember(y,delete(car z,oporder!*)) then return nil
else if idp x then return (physopmember(y,list(x)) and t)
     else if idp y or (car x neq car y) then return nil
          else return ordp(cdr x, cdr y);
end;
...
symbolic procedure physopmember(u,v); % updated 160191
% u is a physop, v is a list
% return part of v starting with u
member(u,v) or member(removeindices(u,collectindices u),v)
or (not atom u and member(car u,v));
...
symbolic procedure ncmpchk(x,y);
% x and y are physops
% checks for correct ordering in noncommuting case
ordop(x,y) or not noncommuting(x,y);
...
for each x in u do <<
if idp x  or not numberp cadr x
then msgpri("Tensor operator",x,"declared without dimension",nil,nil);
n:= cadr x;x:= car x;
if not idp x then
...
symbolic procedure isanindex u;
idp u and (memq(u,physopindices!*) or
(memq(u,frlis!*) and isanindex intern compress pnth(explode u,3)));

symbolic  procedure isavarindex u;
isanindex u and (memq(u,frlis!*) or assoc(u,frasc!*));
...
begin scalar x;
if physopp u then
   if scalopp u then  return 'scalar
   else if vecopp u then return 'vector
        else if tensopp u then return 'tensor
             else if statep u then  return 'state
                  else nil
else if atom u then nil
     else if car u = '!*sq then return getphystype physopaeval u
          else if (x:=get(car u,'phystype)) then return x
               else if (x:=get(car u,'phystypefn)) then
                        return apply1(x,cdr u)
                    else << x := !*collectphysops u;
                            x := deletemult!*
                                 for each v in x collect getphystype v;
                            return
                             if null x then nil
...
symbolic  procedure physop2sq u;
% u is a physop expr
% returns standard quotient of evaluated u
begin scalar x;
return
if null getphystype u then  simp u
else if physopp u then if (x:= get(u,'rvalue)) then physop2sq x
...
% basic simplification routine
 ...
 if physopp u then v:= if (y:= get(u,'rvalue)) then physopaeval y
...
symbolic procedure physopsimp u;
% procedure to simplify the arguments of a physop
% inspired from SIMPIDEN
begin scalar opname,x,y,flg;
if idp u then return u;
opname := car u;
x := for each j in cdr u collect physopsm!* j;
...
% special hack to introduce unrecognized IDXn indices
 else if vecopp u then << putanewindex!* cadr u;
                          return u >>
      else if tensopp u then << for j:= 1 : length(cdr u) do
...
put('difference,'phystypefn,'getphystypeor);
...
put('plus,'phystypefn,'getphystypeor);
...
symbolic procedure getphystypetimes args;
begin scalar x;
x := for each y in args collect getphystype y;
return
if memq('state,x) then  'state
else if memq('tensor,x) then 'tensor
     else if memq('vector,x) then 'vector
     else if memq ('scalar,x) then 'scalar
     else nil
end;
...
symbolic procedure invp u;
begin scalar x,z,n;
% special cases
if u = 'unit then return u;
if (not atom u and member(car u,'(comm anticomm))) then
     return list('quotient,1,u);
if idp u then x :=  explode u
else << x:= explode car u; z:= cdr u >>;
x := if (n:=locate('!-,x)) and (nth!*(x,n+1)  = '!1)  then
       intern compress
          nconc(for j:= 1 :(n-2) collect nth!*(x,j),pnth!*(x,n+2))
     else intern compress nconc(x,list('!!,'!-,'!1));
x := if null z then x else x . z;
return x
end;
...
symbolic procedure adjp u;
begin scalar x,z,n;
if not physopp u then rederr2('adjp, "invalid argument to adjp");
% special cases
if u = 'unit then return u;
if (not atom u and car u = 'comm) then
       return list('comm,adjp caddr u,adjp cadr u);
if (not atom u and car u = 'anticomm) then
     return  list('anticomm,adjp cadr u,adjp caddr u);
if idp u then x := explode u
else << x:= explode car u; z:= cdr u >>;
% special hack in order to maintain fixed order between
% inverse and adjoint ops
if (n:= locate('!-,x)) and (nth!*(x,n+1) = '!1) then
       return invp adjp invp u;
x := if (n:= locate('!+,x)) and (nth!*(x,n-1) = '!!) then
 intern compress nconc(for j:=1 :(n-2) collect nth!*(x,j),pnth!*(x,n+1))
     else intern compress nconc(x,list('!!,'!+));
return if z then x . z
       else x
end;
...
% to remove physop type from an id
 ...
 for each x in u do <<
  ...
  if get(x,'rvalue) then put(x,'avalue,list('scalar,get(x,'rvalue)));
   >>;
return nil
end;
