...
symbolic procedure exptunwind(u,v);
  ...
  a:  if null v then return u;
      ...
      v := cdr v;
      % Modification by Winfried Neun.
      % This allows expressions like e*e^x to combine.
	...
	do <<if fixp numr z2 and fixp denr z2
	     ...
	     else <<% Look for a common numerical factor in numerator.
		    y := lcoeffgcd cdr comfac numr z2;
...
symbolic procedure lcoeffgcd u;
   if domainp u then u else lcoeffgcd1(red u,lnc u);

symbolic procedure lcoeffgcd1(u,v);
   if v=1 then 1
    else if null u then v
    else if domainp u then gcdf(u,v)
    else lcoeffgcd1(red u,gcdf(lnc u,v));
