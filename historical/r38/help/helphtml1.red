...
fluid '(node_file_labels filenumber indexfilename labels2nodes outname);
...
!#if (member 'csl lispsystem!*)
    if u="main_index" then return "index.html"
    else if u="index" then return "r38_idx.html"
...
symbolic procedure open_node_file u;
  begin scalar dir,name;
   ...
   if null outname or outname<2 or remainder(outname, 50)=0 then <<
...
symbolic procedure remove_html x;
   compress reverse cdddr cddr reverse explodec x;
...
symbolic procedure emit_node_label u;
  <<open_node_file u;
    ...
    myprin2 "<a name=";
    %textout_name u;           % ACN
    myprin2 remove_html node_file_name!*;  % ACN
    myprin2 ">";
    myterpri();
...
symbolic procedure emit_node_title(u,dummy,type);
  <<fonton();
    ...
    myterpri();
% The next 2 lines are from JPFF and add in a Reduce logo on each page.
% seems a neat idea!
    channelprintf(outfile!*,"<p align=""centre""><img src=""redlogo.gif"" width=621 height=60 border=0 alt=""REDUCE""></p>");
    myterpri();
...
symbolic procedure dig4 n;
  compress ('!" . append(cdr explodec (10000+n), '(!")));

symbolic procedure refout x;
  begin
% input is eg        r38_1234.html
% desired output =   r38_1200.html#r38_1234
     scalar num, num50;
     num := cddddr explodec x;
     num := compress reverse cddr cdddr reverse num;
     num50 := num - (remainder(num, 50));
     if num50=0 then num50 := 1;
     myprin2 "r38_"; myprin2 dig4 num50; myprin2 ".html#r38_";
     myprin2 dig4 num
  end;
...
% JPFF: add logo
  ...
  <<channelprin2(outfile!*, "<dt>");
    ...
    channelprintf(outfile!*, ": <a href="); refout cddr x; myprin2 ">";
