...
COMMENT to be compatible with the 1973 version of Reduce I have needed
to remove use of "<< ... >>" block and replace them with "BEGIN ... END",
unwind use of "WHERE" into "LET" and replace "ws" with "!*ans". I believe all
these are rather simple changes not representing deep adjustment to the
system capabilities;
...
for i:= 1:8 do 
 begin f2:=-mu*g1 + deps*df(f1,epsilon) + dmu*df(f1,mu) + dsig*df(f1,sigma);
   write "F(",i,") := ",f2;
...
for i:=0:3 do for j:=i+1:3 do for k:=i:3 do
   for l:=k+1:if k=i then j else 3 do
      begin r(j,i,l,k) := r(i,j,k,l) := for q := 0:3
        ...
        r(j,i,k,l) := -r(i,j,k,l);
        if i neq k or j>l
          then begin r(k,l,i,j) := r(l,k,j,i) := r(i,j,k,l);
                 r(l,k,i,j) := -r(i,j,k,l);
                 r(k,l,j,i) := -r(i,j,k,l) end end;
