def Exp2m1(x):
   R = x.parent()
   X = x.exact_rational()
   return R(numerical_approx(2^X-1,100))
...
def table1(out):
   ...
   for i1 in range(2^11):
      ...
      if e==31: # NaN or Inf
         ...
         if y1<2^-149.:
            y1 = R24(2^-149)
         if i1 in [327,331,344,1345,1366]:
            y1 = y1.nextabove()
         if i1 in [140,335,346,369,401,1059,1133]:
            y1 = y1.nextbelow()
         v = "{" + get_hex(y1) + "}"
      T.append(y1)
      t = " " + v + ","
...
def optimize(T1,T2,verbose=true):
   l = ["-0x1.184p-14","-0x1.6a4p-12","-0x1.cfcp-10","0x1.63p-11","0x1.6fp-7"]
   # avoid modifying the input tables
   T1 = copy(T1)
   T2 = copy(T2)
   l2 = []
   for x in l:
