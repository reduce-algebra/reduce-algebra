      ...
      if e==31: # NaN or Inf, sin(x) = NaN
         ...
         if x1!=0 and y1==x1:
            if x1>0:
               ...
               y1=y1.nextabove()
         if i1 in [405,706,738,801,807,814,935,982,1025,1026,1028,1032,1040,1605,1765,1804,1868,1946,2010]:
            y1=y1.nextabove()
         if i1 in [1,2,4,8,16,581,741,812,844,922,986,1429,1730,1762,1825,1831,1836,1838,1959,2006]:
            y1=y1.nextbelow()
         v = "{" + get_hex(y1) + "}"
      t = " " + v + ","
      S1.append(y1)
...
def table1cos(out):
   ...
   for i1 in range(2^11):
      ...
      if e==31: # NaN or Inf, cos(x) = NaN
         ...
         # fix manually some values
         if y1==1:
            y1=y1.nextbelow() # 0x1.fffffep-1
         if i1 in [347,851,1371,1875]:
            y1=y1.nextabove()
         if i1 in [716,769,801,812,823,1740,1793,1804,1847]:
            y1=y1.nextbelow()
         v = "{" + get_hex(y1) + "}"
      t = " " + v + ","
      C1.append(y1)
...
def number_failures(l,S1,C1,S2,C2):
   ...
   for r in 'NZUD':
      ...
      for x in l:
         i1,i2 = get_i1i2(x)
         y1 = sin(R11r(x))
         z1 = R11r(fma(R24r(S1[i1]),R24r(C2[i2]),R24r(C1[i1])*R24r(S2[i2])))
         y2 = cos(R11r(x))
         z2 = R11r(fma(R24r(C1[i1]),R24r(C2[i2]),-R24r(S1[i1])*R24r(S2[i2])))
         if y1!=z1 or y2!=z2:
            # print (get_hex(x))
            n += 1
   return n
...
def number_failures_x(S1,C1,S2,C2,x):
   ...
   for r in 'NZUD':
      ...
      i1,i2 = get_i1i2(x)
      y1 = sin(R11(x))
      z1 = R11(fma(R24(S1[i1]),R24(C2[i2]),R24(C1[i1])*R24(S2[i2])))
      y2 = cos(R11(x))
      z2 = R11(fma(R24(C1[i1]),R24(C2[i2]),-R24(S1[i1])*R24(S2[i2])))
      if y1!=z1 or y2!=z2:
         n += 1
   return n
...
def optimize(verbose=true):
   l = ["-0x0p+0","-0x1.0a4p+10","-0x1.63p+10","-0x1.63p+11","-0x1.63p+8","-0x1.63p+9","-0x1.a84p+15","0x0p+0","0x1.63p+11","0x1.63p+8","0x1.63p+9","0x1.a84p+15"]
   l2 = []
   for x in l:
      if not x in l2:
         l2.append(x)
   l = [R11(x,16) for x in l2]
   S1 = copy(table1sin("/tmp/tmp"))
   C1 = copy(table1cos("/tmp/tmp"))
   S2 = copy(table2sin("/tmp/tmp"))
   C2 = copy(table2cos("/tmp/tmp"))
   I1 = Set()
   I2 = Set()
   for x in l:
      i1,i2 = get_i1i2(x)
