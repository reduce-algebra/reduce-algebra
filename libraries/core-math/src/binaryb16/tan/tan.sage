   ...
   while changed==true:
      ...
      for i in li:
         ...
         if len(t)<len(s):
            ...
            print ("i=", i, "S3=", get_hex(saved), "new=", get_hex(S3[i]))
            s = t
            changed = changed_si = true
         elif len(t)==len(s) and ZZ.random_element(k)==0:
            s = t
            changed = changed_si = true
         else: # restore
            S3[i] = saved
         ...
         if len(t)<len(s):
            print ("decrease to", len(t))
            print ("i=", i, "C3=", get_hex(saved), "new=", get_hex(C3[i]))
            s = t
