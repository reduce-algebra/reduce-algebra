...
def Log10p1(x):
   R = x.parent()
   X = x.exact_rational()
   return R(n(log(1+X)/log(10),200))

def Log10(x):
   R = x.parent()
   X = x.exact_rational()
   return R(n(log(X)/log(10),200))

# determine the range for which log10p1(x) rounds to the same value as log10(x)
# check_large()
# '0x1.bcp+12'
def check_large():
   ...
   while true:
      for r in 'NZUD':
         ...
         if Log10(R(x))!=Log10p1(R(x)):
            return get_hex(x)
      x = x.nextbelow()

# determine the range for which log10p1(x) rounds to the same value as
# R8(log10p1_tiny(R24(x)))
# check_tiny()
# 0x1.42p-8
def check_tiny():
   x = u = R8(2^-133)
   INVLOG10 = RealField(24)("0x1.bcb7b2p-2",16)
   while true:
      for r in 'NZUD':
         R = RealField(8,rnd='RND'+r)
         R24 = RealField(24,rnd='RND'+r)
         if Log10p1(R(x))!=R(log10p1_tiny(R24(x))):
            return get_hex(x)
         # also try with -x
         if Log10p1(R(-x))!=R(log10p1_tiny(R24(-x))):
            return get_hex(x)
...
def log10p1_tiny(x):
   R = x.parent()
   INVLOG10 = R("0x1.bcb7b2p-2",16)
   t = x*INVLOG10
   return fma(t,-x*R(1/2),t)

# check all bf16 numbers in [x1,x2) are such that
