...
function charTo5Bit (c) {
  if (c >= 'a' && c <= 'z') return 1 + (c.charCodeAt(0) - 'a'.charCodeAt(0))
  else if (c >= '0' && c <= '4') {
    return 1 + 26 + (c.charCodeAt(0) - '0'.charCodeAt(0))
  } else FATAL(`Character '${c}' cannot be encoded into a 5-bit string`)
}
...
class InstructionNameData {
  ...
  add (s) {
    ...
    if (/^[a-z0-4]{0,6}$/.test(s)) {
      let index = 0
      for (let i = 0; i < s.length; i++) index |= charTo5Bit(s[i]) << (i * 5)
    ...
    if (this.maxNameLength < s.length) this.maxNameLength = s.length
  }
  ...
  index () {
    ...
    for (let z = 0; z < names.length; z++) {
      ...
      for (let i = Math.min(name.length, kMaxPrefixSize); i > 0; i--) {
        ...
        if (suffix.length === kMaxSuffixSize) break
      }
...
class Injector {
  ...
  dataOfFile (file) {
    const obj = this.files[file]
    if (!obj) FATAL(`TableGen.dataOfFile(): File '${file}' not loaded`)
    return obj.data
  }
  ...
  inject (key, str, size) {
    ...
    if (!done) FATAL(`TableGen.inject(): Cannot find '${key}'`)

    if (size) this.tableSizes[key] = size
...
class TableGen extends Injector {
  ...
  addTask (task) {
    if (!task.name) FATAL('TableGen.addModule(): Module must have a name')
  ...
  runTasks () {
    ...
    while (pending) {
      ...
      for (let i = 0; i < tasks.length; i++) {
        const task = tasks[i]
        if (tasksDone[task.name]) continue
...
class IdEnum extends Task {
  ...
  run () {
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      if (text) line = line.padEnd(37) + '//!< ' + text
