...
function parseEnum (input) {
  ...
  for (;;) {
    ...
    if (token.type === 'symbol') {
      ...
      if (!hasOwn.call(map, symbol)) map[symbol] = value
      else console.log(`${symbol} already defined, skipping...`)

      token = tokenizer.next()
      if (token.type !== 'comma') tokenizer.revert(token)
      continue
    }
...
function compactedSize (table) {
  let size = 0
  for (let i = 0; i < table.length; i++) size += table[i].name.length + 1
  return size
}

function indexTypeFromSize (size) {
  if (size <= 256) return 'uint8_t'
  else if (size <= 65536) return 'uint16_t'
...
function stringifyEnum (map, options) {
  ...
  for (const k in map) {
    ...
    if (stripPrefix) {
      ...
      } else throw Error(`Cannot strip prefix '${stripPrefix}' in '${key}'`)
    }

    table.push({ name, value: map[k] })
  ...
  function buildIndexData () {
    ...
    for (var i = 0; i < table.length; i++) {
      while (indexArray.length < table[i].value) indexArray.push(unknownIndex)
  ...
  output +=
    `static const char ${outputPrefix}String[] =\n` + buildStringData() + '\n'
  output +=
    `static const ${indexType} ${outputPrefix}Index[] = {\n` +
    buildIndexData() +
    '};\n'
...
class Generator {
    ...
    this.outputs.forEach((fileName) => {
      ...
      if (oldContent != newContent) {
        if (this.verify) {
          ...
          if (!this.noBackup) {
            fs.writeFileSync(
              path.join(this.baseDir, fileName + '.backup'),
              oldContent,
              'utf8'
            )
  ...
  addEnumsFromSource (fileName, src) {
    let found = false
    const matches = [
      ...src.matchAll(/(?:@EnumValuesBegin(\{.*\})@|@EnumValuesEnd@)/g)
    ]
    ...
    for (let i = 0; i < matches.length; i += 2) {
      ...
      if (!end) throw new Error(`Missing @EnumValuesEnd for '${def[0]}'`)

      if (!end[0].startsWith('@EnumValuesEnd@')) {
        throw new Error(
          `Expected @EnumValuesEnd@ for '${def[0]}' and not '${end[0]}'`
        )
      }
      ...
      if (!enumName) throw Error(`Missing 'enum' in '${def[0]}`)
      ...
      if (startIndex === -1 || startIndex > endIndex) {
        throw new Error(
          `Internal Error, indexes have unexpected values: startIndex=${startIndex} endIndex=${endIndex}`
        )
      }
  ...
  injectEnumsToSource (src) {
    const matches = [
      ...src.matchAll(/(?:@EnumStringBegin(\{.*\})@|@EnumStringEnd@)/g)
    ]
    let delta = 0
    ...
    for (let i = 0; i < matches.length; i += 2) {
      ...
      if (!end) throw new Error(`Missing @EnumStringEnd@ for '${def[0]}'`)

      if (!end[0].startsWith('@EnumStringEnd@')) {
        throw new Error(
          `Expected @EnumStringEnd@ for '${def[0]}' and not '${end[0]}'`
        )
      }
      ...
      if (!enumName) throwError(`Missing 'name' in '${def[0]}`)
      ...
      const newContent = indent(
        stringifyEnum(this.enumMap[enumName], options),
        indentation
      )
