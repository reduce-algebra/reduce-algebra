...
class ObjectUtils {
  ...
  static equals (a, b) {
    ...
    if (Array.isArray(a) || Array.isArray(b)) {
      ...
      for (let i = 0; i < len; i++) {
        if (!ObjectUtils.equals(a[i], b[i])) return false
      }
    } else {
      if (a === null || b === null) return a === b

      for (const k in a) {
        if (!hasOwn.call(b, k) || !ObjectUtils.equals(a[k], b[k])) return false
      }

      for (const k in b) if (!hasOwn.call(a, k)) return false
    }
  ...
  static equalsExcept (a, b, except) {
    ...
    for (const k in b) {
      if (!hasOwn.call(except, k) && !hasOwn.call(a, k)) return false
    }
  ...
  static findKey (map, keys) {
    for (const key in keys) if (hasOwn.call(map, key)) return key
    return undefined
  }

  static hasAny (map, keys) {
    for (const key in keys) if (hasOwn.call(map, key)) return true
    return false
  }
...
class ArrayUtils {
  static min (arr, fn) {
    ...
    let v = fn(arr[0])
    for (let i = 1; i < arr.length; i++) v = Math.min(v, fn(arr[i]))
    return v
  }
  ...
  static max (arr, fn) {
    ...
    let v = fn(arr[0])
    for (let i = 1; i < arr.length; i++) v = Math.max(v, fn(arr[i]))
    return v
  }
  ...
  static deepIndexOf (arr, what) {
    for (let i = 0; i < arr.length; i++) {
      if (ObjectUtils.equals(arr[i], what)) return i
    }
    return -1
  }
  ...
  static toDict (arr, value) {
    ...
    const out = Object.create(null)
    for (let i = 0; i < arr.length; i++) out[arr[i]] = value
    return out
  }
}
...
class StringUtils {
  ...
  static countOf (s, pattern) {
    if (!pattern) FATAL('Pattern cannot be empty')
  ...
  static decToHex (n, nPad) {
    ...
    while (nPad > hex.length) hex = '0' + hex
    return '0x' + hex.toUpperCase()
  }

  static format (array, indent, showIndex, mapFn) {
    if (!mapFn) mapFn = StringUtils.asString

    let s = ''
    let threshold = 80

    if (showIndex === -1) s += indent
  ...
  static makeCxxArray (array, code, indent) {
    if (typeof indent !== 'string') indent = kIndent
  ...
  static indent (s, indentation) {
    if (typeof indentation === 'number') indentation = ' '.repeat(indentation)
...
class IndexedString {
  ...
  index () {
    ...
    for (k in map) if (partialMap[k] === k) array.push(k)
    array.sort()
