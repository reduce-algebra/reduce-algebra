...
class ArmTableGen extends core.TableGen {
  ...
  parse () {
    ...
    const stringData = StringUtils.extract(
      rawData,
      '// ${InstInfo:Begin}',
      '// ${InstInfo:End'
    )
    ...
    while ((m = re.exec(stringData)) !== null) {
      ...
      const opcodeData = m[3].trim()
      const rwInfo = m[4].trim()
      const instFlags = m[5].trim()

      let displayName = name
      if (name.endsWith('_v')) displayName = name.substring(0, name.length - 2)
    ...
    if (
      this.insts.length === 0 ||
      this.insts.length !== StringUtils.countOf(stringData, 'INST(')
    ) {
...
class EncodingTable extends core.Task {
  ...
  run () {
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      const opcodeData = inst.opcodeData
        .replace(/\(/g, '{ ')
        .replace(/\)/g, ' }')

      if (!hasOwn.call(map, encoding)) map[encoding] = []
    ...
    return (
      this.ctx.inject('EncodingId', StringUtils.disclaimer(encodingIds), 0) +
      this.ctx.inject(
        'EncodingDataForward',
        StringUtils.disclaimer(tableHeader),
        0
      ) +
...
class CommonTable extends core.Task {
  constructor () {
    super('CommonTable', ['IdEnum', 'NameTable'])
  }
