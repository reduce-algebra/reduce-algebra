...
class Utils {
  static toHex (val, pad) {
    ...
    if (pad != null && s.length < pad) s = '0'.repeat(pad - s.length) + s
  ...
  static camelCase (s) {
    ...
    else return s[0].toLowerCase() + s.substr(1)
  }
...
class Block extends Node {
  ...
  insertNode (index, node) {
    ...
    if (index >= this.nodes.length) this.nodes.push(node)
  ...
  addVarDecl (type, name, init) {
    ...
    if (!(node instanceof Var)) node = new Var(type, name, init)
  ...
  addLine (code) {
    if (typeof code !== 'string') FATAL('Block.addLine(): Line must be string')
  ...
  prependEmptyLine () {
    if (!this.isEmpty()) this.nodes.splice(0, 0, new Line(''))
    return this
  }

  addEmptyLine () {
    if (!this.isEmpty()) this.nodes.push(new Line(''))
    return this
  }

  toString () {
    let s = ''
    for (const node of this.nodes) s += String(node)
    return s
  }
}
exports.Block = Block
...
class If extends Node {
  constructor (cond, body) {
    ...
    if (!(body instanceof Block)) FATAL('If() - body must be a Block')
...
class Case extends Node {
  ...
  toString () {
    let s = ''
    for (const node of this.body.nodes) s += String(node)
