...
(function ($scope, $as) {
    ...
    splitOperands: function (s) {
      ...
      for (;;) {
        if (i === s.length || (c = s[i]) === ',') {
          ...
          if (!op) FAIL(`Found empty operand in '${s}'`)
  ...
  class Operand {
    ...
    get optional () {
      return this._getFlag(OperandFlags.Optional)
    }
    ...
    get implicit () {
      return this._getFlag(OperandFlags.Implicit)
    }

    set implicit (value) {
      this._setFlag(OperandFlags.Implicit, value)
    }

    get commutative () {
      return this._getFlag(OperandFlags.Commutative)
    }

    set commutative (value) {
      this._setFlag(OperandFlags.Commutative, value)
    }

    get zext () {
      return this._getFlag(OperandFlags.ZExt)
    }

    set zext (value) {
      this._setFlag(OperandFlags.ZExt, value)
    }
    ...
    isReg () {
      return !!this.reg && this.type !== 'reg-list'
    }
  ...
  class Instruction {
    ...
    _combineAttribute (key, value) {
      ...
      if (Array.isArray(value)) {
        ...
        for (const k in value) this[key][k] = value[k]
      }
    }
  ...
  class InstructionGroup extends Array {
    ...
    unionCpuFeatures (name) {
      ...
      for (let i = 0; i < this.length; i++) {
        ...
        for (const k in features) result[k] = features[k]
      }
      return result
    }
    ...
    checkAttribute (key, value) {
      let n = 0
      for (let i = 0; i < this.length; i++) n += Number(this[i][key] === value)
      return n
    }
  }
  ...
  class ISA {
    ...
    query (args, copy) {
      ...
      if (filter) result = result.filter(filter, args.filterThis)
    ...
    _queryByName (name, copy) {
      ...
      if (Array.isArray(name)) {
        ...
        for (let i = 0; i < names.length; i++) {
          ...
          if (result === EmptyInstructionGroup) result = new InstructionGroup()

          for (let j = 0; j < insts.length; j++) result.push(insts[j])
        }
        return result
      }
    ...
    _postProc (groups) {
      for (const group of groups) {
        for (const iRule of group.instructions) {
          const names = iRule.inst.split(' ')
          for (const name of names) {
            const insts = this._instructionMap[name]
            if (!insts) {
              FAIL(
                `Instruction ${name} referenced by '${group.group}' group doesn't exist`
              )
            }

            for (const k in iRule) {
              if (k === 'inst' || k === 'data') continue
    ...
    _addCpuLevels (items) {
      if (!Array.isArray(items)) FAIL("Property 'cpuLevels' must be array")
    ...
    _addExtensions (items) {
      if (!Array.isArray(items)) FAIL("Property 'extensions' must be array")
    ...
    _addAttributes (items) {
      if (!Array.isArray(items)) FAIL("Property 'attributes' must be array")
    ...
    _addSpecialRegs (items) {
      if (!Array.isArray(items)) FAIL("Property 'specialRegs' must be array")
    ...
    _addShortcuts (items) {
      if (!Array.isArray(items)) FAIL("Property 'shortcuts' must be array")
    ...
    _addInstruction (inst) {
      ...
      if (inst.aliasOf) this._aliases[inst.name] = inst.aliasOf
