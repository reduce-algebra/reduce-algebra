...
(function ($scope, $as) {
    ...
    static parseDtArray (s) {
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (v.indexOf('-') !== -1) {
          ...
          if (!m) FAIL(`Couldn't parse '${s}' data-type`)

          const type = m[1] || ''
          let size = parseInt(m[2], 10)
          const last = parseInt(m[3], 10)
  ...
  class Operand extends base.Operand {
    ...
    isRelative () {
      if (this.type === 'imm') {
        return (
          this.name === 'relA' || this.name === 'relS' || this.name === 'relZ'
        )
      } else return false
    }
  }
  ...
  class Instruction extends base.Instruction {
    constructor (db, data) {
      ...
      const encoding = hasOwn.call(data, 'a32')
        ? 'a32'
        : hasOwn.call(data, 't32')
          ? 't32'
          : hasOwn.call(data, 't16')
            ? 't16'
            : ''
      ...
      for (const k in data) {
        if (k === 'name' || k == encoding || k === 'operands') continue
    ...
    _assignAttribute (key, value) {
      switch (key) {
        case 'it':
          for (const it of value.split(' ')) this.it[it.trim()] = true
          break
    ...
    _assignAttributeKeyValue (name, content) {
      ...
      for (let i = 0; i < attributes.length; i++) {
        ...
        const key = eq === -1 ? attr : attr.substr(0, eq)
        const val = eq === -1 ? true : attr.substr(eq + 1)
    ...
    _assignOperands (s) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (consecutive) def = consecutive[2]
        ...
        const shiftOp = Utils.parseShiftOp(def)
        if (shiftOp) {
          op.shiftOp = shiftOp
        ...
        if (def.startsWith('[')) {
          ...
          if (!info) FAIL(`Unknown register operand '${reg}' in '${def}'`)
    ...
    _assignOpcode (s) {
      ...
      for (let i = arr.length - 1; i >= 0; i--) {
        ...
        if (/^[0-1]+$/.test(key)) {
          ...
          if ((m = key.match(/\[\s*(\d+)\s*\:\s*(\d+)\s*\]$/))) {
            ...
            if (a < b) FAIL(`Invalid bit range '${key}' in opcode '${s}'`)
            from = b
            size = a - b + 1
            mask = ((1 << size) - 1) << b
            ...
            if (lbit || hbit) {
              ...
              if (lbit && hbit) {
                FAIL(
                  `Couldn't recognize the format of '${key}' in opcode '${s}'`
                )
              }
      ...
      for (const key in fields) {
        ...
        if (field.bits) field.mask = (1 << field.bits) - 1
        else if (field.mask) field.bits = 32 - Math.clz32(field.mask)
      ...
      if (opcodeIndex !== 16 && opcodeIndex !== 32) {
        FAIL(
          `The number of bits '${opcodeIndex}' used by the opcode '${s}' doesn't match 16 or 32`
        )
      }
    ...
    _assignSpecificAttribute (key, value) {
      ...
      if (/^ARM\w+[+-]$/.test(key)) {
        ...
        if (sign === '+') this.availableFrom = armv
        else this.availableUntil = armv
        return true
      }
