...
(function ($scope, $as) {
  ...
  function mustEnclose (node) {
    return node.isUnary() ? node.child.isOperator() : !!node.isBinary()
  }
  ...
  class ExpNode {
    ...
    isOperator () {
      return this.type === 'unary' || this.type === 'binary'
    }
    ...
    evaluate (ctx) {
      throw new Error('ExpNode.evaluate() must be overridden')
    }
  ...
  class ImmNode extends ExpNode {
    ...
    toString (ctx) {
      return ctx ? ctx.stringifyImmediate(this.imm) : String(this.imm)
    }
  }
  ...
  class VarNode extends ExpNode {
    ...
    toString (ctx) {
      return ctx ? ctx.stringifyVariable(this.name) : String(this.name)
    }
  }
  ...
  class CallNode extends ExpNode {
    ...
    evaluate (ctx) {
      const evaluatedArgs = this.args.map(function (arg) {
    ...
    toString (ctx) {
      if (this.name === '$bit') {
        ...
        const argsCode = this.args
          .map(function (arg) {
            return arg.toString(ctx)
          })
          .join(', ')
        if (ctx) return `${ctx.stringifyFunction(this.name)}(${argsCode})`
        else return `${this.name}(${argsCode})`
      }
    }
  }
    ...
    evaluate (ctx) {
      ...
      switch (this.op) {
        ...
        case '||':
          return left || right ? 1 : 0
        default:
          return ctx.binary(this.op, left, right)
      }
    }
  ...
  function Shl (left, right) {
    return Binary('<<', left, right)
  }
  function Shr (left, right) {
    return Binary('>>', left, right)
  }
  ...
  const Category = (function (_, S, A, D, $) {
    ...
    return function (c) {
      if (c < kTableLength) return Table[c]
      return kCharNone
    }
  ...
  function newToken (type, position, data, value) {
    return {
      type, // Token type, see `kToken...`.
      position, // Token position in expression's source.
      data, // Token data (content) as string.
      value // Token value (only if the token is a value).
    }
  }
  ...
  function parseHex (source, from) {
    ...
    while (i < source.length) {
      ...
      if (c >= '0'.charCodeAt(0) && c <= '9'.charCodeAt(0)) {
        ...
        (c >= 'g'.charCodeAt(0) && c <= 'z'.charCodeAt(0)) ||
        (c >= 'g'.charCodeAt(0) && c <= 'Z'.charCodeAt(0))
      ) {
        throwExpressionError(
          `Invalid hex number 0x${source.substring(from, i + 1)}`
        )
      } else {
        break
      }
    ...
    if (i === from) throwExpressionError('Invalid number starting with 0x')
  ...
  function tokenize (source) {
    ...
    let i = 0
    let j = 0 // Current index in `source` and temporary.
    ...
    while (i < len) {
      ...
      if (cat === kCharSpace) {
        ...
        if (
          c === '0'.charCodeAt(0) &&
          i + 1 < len &&
          source.charCodeAt(i + 1) === 'x'.charCodeAt(0)
        ) {
        ...
        while (
          ++i < len &&
          ((cat = Category(source.charCodeAt(i))) === kCharAlpha ||
            cat === kCharDigit)
        ) {
          continue
        }
        ...
        do {
          for (j = Math.min(i - start, kMaxOperatorLen); j > 0; j--) {
            ...
            if (
              hasOwn.call(kUnaryOperators, part) ||
              hasOwn.call(kBinaryOperators, part) ||
              j === 1
            ) {
  ...
  class Parser {
    ...
    peek () {
      return this.tIndex < this.tokens.length
        ? this.tokens[this.tIndex]
        : NoToken
    }
    ...
    parseExpression () {
      ...
      for (;;) {
        ...
        if (value === null) {
          ...
          if (token.type === kTokenPunct) {
            do {
              ...
              if (unaryLast) unaryLast.child = node
              else unaryFirst = node
          ...
          if (token.type === kTokenValue) {
            ...
            if (after.data === '(') value = this.parseCall(token.data)
            else if (after.data === '[') {
            ...
            if (token.data !== ')') throwTokenizerError(token)
          } else {
        ...
        token = this.peek()
        if (
          token.type === kTokenPunct &&
          hasOwn.call(kBinaryOperators, token.data)
        ) {
          const opName = token.data
          if (opName === ':') break
          ...
          if (!stack.length) {
            ...
            let aNode = stack.pop()
            const aPrec = aNode.info().prec
            const bPrec = bNode.info().prec
            ...
            if (aPrec > bPrec) {
              ...
              while (stack.length) {
                if (rightAssociate(aNode.info(), bPrec)) break
                aNode = stack.pop()
              }
          ...
          if (opName === '?') {
            ...
            if (ternTok.data !== ':') {
              throwExpressionError(
                `Unterminated ternary if '${token.data}'`,
                token.position
              )
            }
      ...
      if (value === null) throwExpressionError('Invalid expression')
    ...
    parseBitAccess (name) {
      ...
      token = this.next()
      if (token.type != kTokenValue) throwTokenizerError(token)
  ...
  class Collector extends Visitor {
    ...
    visit (node) {
      if (node.type === this.nodeType) {
        if (hasOwn.call(this.dict, node.name)) this.dict[node.name]++
        else this.dict[node.name] = 1
      }
