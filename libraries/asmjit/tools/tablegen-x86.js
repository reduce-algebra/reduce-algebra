...
class Filter {
  static unique (instArray) {
    ...
    for (let i = 0; i < instArray.length; i++) {
      ...
      if (inst.altForm) continue

      const s = inst.operands
        .map((op) => {
          return op.isImm() ? 'imm' : op.toString()
        })
        .join(', ')
      if (known[s] === true) continue
...
class GenUtils {
  ...
  static cpuFeaturesOf (dbInsts) {
    function cmp (a, b) {
      if (a.startsWith('AVX512') && !b.startsWith('AVX512')) return 1
      if (b.startsWith('AVX512') && !a.startsWith('AVX512')) return -1

      if (a.startsWith('AVX') && !b.startsWith('AVX')) return 1
      if (b.startsWith('AVX') && !a.startsWith('AVX')) return -1

      if (a === 'FPU' && b !== 'FPU') return 1
      if (b === 'FPU' && a !== 'FPU') return -1
  ...
  static assignVexEvexCompatibilityFlags (f, dbInsts) {
    const vexInsts = dbInsts.filter((inst) => {
      return inst.prefix === 'VEX'
    })
    const evexInsts = dbInsts.filter((inst) => {
      return inst.prefix === 'EVEX'
    })

    function isCompatible (vexInst, evexInst) {
      if (vexInst.operands.length !== evexInst.operands.length) return false

      for (let i = 0; i < vexInst.operands.length; i++) {
        const vexOp = vexInst.operands[i]
        const evexOp = evexInst.operands[i]

        if (vexOp.data === evexOp.data) continue

        if (vexOp.reg && vexOp.reg === evexOp.reg) continue
        if (vexOp.mem && vexOp.mem === evexOp.mem) continue
    ...
    if (
      evexInsts[0].operands.length == 2 &&
      vexInsts[0].operands.length === 3
    ) {
      f.EvexTwoOp = true
      return true
    }
  ...
  static flagsOf (dbInsts) {
    ...
    const mib =
      dbInsts.length > 0 && /^(?:bndldx|bndstx)$/.test(dbInsts[0].name)
    if (mib) f.Mib = true
    ...
    for (i = 0; i < dbInsts.length; i++) {
      ...
      if (dbInst.tsib) f.Tsib = true

      if (dbInst.vsibReg) f.Vsib = true

      if (dbInst.prefix === 'VEX' || dbInst.prefix === 'XOP') f.Vex = true

      if (dbInst.encodingPreference === 'EVEX') f.PreferEvex = true
      ...
      if (VexToEvexMap[dbInst.name]) f.EvexTransformable = true
    }
  ...
  static eqOps (aOps, aFrom, bOps, bFrom) {
    ...
    for (;;) {
      ...
      if (aOp.data !== bOp.data) return false
...
class X86TableGen extends core.TableGen {
  ...
  parse () {
    ...
    while ((m = re.exec(data)) !== null) {
      const enum_ = m[1]
      const name = enum_ === 'None' ? '' : enum_.toLowerCase()
      const encoding = m[2].trim()
      const opcode0 = m[3].trim()
      const opcode1 = m[4].trim()
      ...
      this.addInst({
        ...
        dbInsts, // All dbInsts returned from asmdb query.
        encoding, // Instruction encoding.
        opcode0, // Primary opcode.
        opcode1, // Secondary opcode.
        flags,
  ...
  merge () {
    const s =
      StringUtils.format(this.insts, '', true, function (inst) {
        return (
          'INST(' +
  ...
  newInstFromGroup (dbInsts) {
    ...
    function GetAccess (dbInst) {
      ...
      const op = operands[0]
      if (op.read && op.write) return 'RW'
      else if (op.read) return 'RO'
      else return 'WO'
    }
    ...
    function formatEmit (dbi) {
      ...
      for (let choice = 0; choice < 2; choice++) {
        ...
        for (let j = 0; j < dbi.operands.length; j++) {
          ...
          if (reg) {
            if (reg === 'xmm' || reg === 'ymm' || reg === 'zmm') s += 'Vec'
            else if (reg === 'k') s += 'KReg'
            else if (
              reg === 'r32' ||
              reg === 'r64' ||
              reg === 'r16' ||
    ...
    const id = this.insts.length
    const name = dbi.name
    const enum_ = name[0].toUpperCase() + name.substr(1)

    const opcode = dbi.opcodeHex
    const modR = dbi.modR
    const mm = dbi.mm
    const pp = dbi.pp
    const encoding = dbi.encoding
    const isVec = isVecPrefix(dbi.prefix)
    let evexCount = 0

    const access = GetAccess(dbi)
    ...
    for (let i = 0; i < dbInsts.length; i++) {
      ...
      if (dbi.prefix === 'VEX' || dbi.prefix === 'XOP') {
        const newVexL = String(
          dbi.l === '128' ? 0 : dbi.l === '256' ? 1 : dbi.l === '512' ? 2 : '_'
        )
        const newVexW = String(dbi.w === 'W0' ? 0 : dbi.w === 'W1' ? 1 : '_')

        if (vexL !== undefined && vexL !== newVexL) vexL = 'x'
        else vexL = newVexL
        if (vexW !== undefined && vexW !== newVexW) vexW = 'x'
        else vexW = newVexW
      }

      if (dbi.prefix === 'EVEX') {
        evexCount++
        const newEvexW = String(dbi.w === 'W0' ? 0 : dbi.w === 'W1' ? 1 : '_')
        if (evexW !== undefined && evexW !== newEvexW) evexW = 'x'
        else evexW = newEvexW

        if (dbi.tupleType) {
          if (tupleType !== '_' && tupleType !== dbi.tupleType) {
            console.log(
              `${dbi.name}: WARNING: TupleType ${tupleType} != ${dbi.tupleType}`
            )
          }
      ...
      if (opcode !== dbi.opcodeHex) {
        console.log(`${dbi.name}: ISSUE: Opcode ${opcode} != ${dbi.opcodeHex}`)
        return null
      }
      if (modR !== dbi.modR) {
        console.log(`${dbi.name}: ISSUE: ModR ${modR} != ${dbi.modR}`)
        return null
      }
      if (mm !== dbi.mm) {
        console.log(`${dbi.name}: ISSUE: MM ${mm} != ${dbi.mm}`)
        return null
      }
      if (pp !== dbi.pp) {
        console.log(`${dbi.name}: ISSUE: PP ${pp} != ${dbi.pp}`)
        return null
      }
      if (encoding !== dbi.encoding) {
        console.log(`${dbi.name}: ISSUE: Enc ${encoding} != ${dbi.encoding}`)
        return null
      }
      if (access !== GetAccess(dbi)) {
        console.log(
          `${dbi.name}: ISSUE: Access ${access} != ${GetAccess(dbi)}`
        )
        return null
      }
      if (isVec != isVecPrefix(dbi.prefix)) {
        console.log(`${dbi.name}: ISSUE: Vex/Non-Vex mismatch`)
        return null
      }
    ...
    if (tupleType !== '_') cdshl = tupleTypeToCDSHL[tupleType] || '?'

    const ppmm =
      pp.padEnd(2).replace(/ /g, '0') + mm.padEnd(4).replace(/ /g, '0')

    const composed = composeOpCode({
...
class IdEnum extends core.IdEnum {
  ...
  comment (inst) {
    ...
    let text = ''
    const features = GenUtils.cpuFeaturesOf(dbInsts)
    ...
    if (features.length) {
      ...
      if (fma !== -1) {
        avxFeatures.splice(fma, 1)
        avxFeatures.splice(0, 0, 'FMA')
      }
    ...
    const arch = GenUtils.cpuArchOf(dbInsts)
    if (arch) text += (text ? ' ' : '') + arch
  ...
  run () {
    ...
    function splitOpcodeToComponents (opcode) {
      ...
      return [prefix].concat(
        opcode.substring(i + 1, opcode.length - 1).split(',')
      )
    }
    ...
    function joinOpcodeComponents (components) {
      ...
      if (values.length >= 8) values[7] = values[7].padEnd(4)
      return prefix + '(' + values.join(',') + ')'
    }

    function indexMainOpcode (opcode) {
      if (opcode === '0') return ['00', 0]

      let opcodeByte = ''
      const components = normalizeOpcodeComponents(
        splitOpcodeToComponents(opcode)
      )
    ...
    function indexAltOpcode (opcode) {
      if (opcode === '0') opcode = noOp
      else {
        opcode = joinOpcodeComponents(
          normalizeOpcodeComponents(splitOpcodeToComponents(opcode))
        )
      }
...
function StringifyOpArray (a, map) {
  ...
  for (let i = 0; i < a.length; i++) {
    const op = a[i]
    let mapped = null
    if (typeof map === 'function') mapped = map(op)
    else if (hasOwn.call(map, op)) mapped = map[op]
    else FATAL(`UNHANDLED OPERAND '${op}'`)
    s += (s ? ' | ' : '') + mapped
  }
  return s || '0'
}
...
class OSignature {
  ...
  mergeWith (other) {
    ...
    for (k in af) {
      ...
      if (index !== null && index !== -1) indexKind = kind
    }
  ...
  toString () {
    ...
    if (flags.implicit) s = '<' + s + '>'

    return s
  }

  toAsmJitOpData () {
    const opFlags = Object.create(null)
    let regMask = 0

    for (const k in this.flags) {
      switch (k) {
        case 'r8lo':
          opFlags.RegGpbLo = true
          break
        case 'r8hi':
          opFlags.RegGpbHi = true
          break
        case 'r16':
          opFlags.RegGpw = true
          break
        ...
        case 'r64':
          ...
          break
        case 'creg':
          opFlags.RegCReg = true
          break
        case 'dreg':
          opFlags.RegDReg = true
          break
        case 'sreg':
          opFlags.RegSReg = true
          break
        case 'bnd':
          opFlags.RegBnd = true
          break
        ...
        case 'm512':
          ...
          break
        case 'm1024':
          opFlags.Mem1024 = true
          break

        case 'mem':
          opFlags.MemUnspecified = true
          break
        case 'mib':
          opFlags.MemUnspecified = true
          opFlags.FlagMib = true
          break
        case 'tmem':
          opFlags.MemUnspecified = true
          opFlags.FlagTMem = true
          break

        case 'memBase':
          opFlags.FlagMemBase = true
          break
        case 'memDS':
          opFlags.FlagMemDs = true
          break
        case 'memES':
          opFlags.FlagMemEs = true
          break
        case 'memZAX':
          regMask |= 1 << 0
          break
    ...
    const outputFlags = StringifyOpArray(
      ArrayUtils.sorted(opFlags, cmpOp),
      function (k) {
        return `F(${k})`
      }
    )
...
class ISignature extends Array {
  ...
  opEquals (other) {
    ...
    for (let i = 0; i < len; i++) if (!this[i].equals(other[i])) return false
  ...
  mergeWith (other) {
    ...
    if (!sameArch || this.implicit !== other.implicit) return false
    ...
    function keyOf (inst, regOps) {
      ...
      for (let i = 0; i < inst.length; i++) {
        ...
        if (regOps & (1 << i)) {
          s +=
            '{' +
            ArrayUtils.sorted(ObjectUtils.and(op.flags, RegOp)).join('|') +
            '}'
        }
      }
      return s || '?'
    ...
    for (aIndex = 0; aIndex < this.length; aIndex++) {
      ...
      for (bIndex = 0; bIndex < this.length; bIndex++) {
        ...
        for (i = 0; i < len; i++) {
          ...
          if (regOps & (1 << i)) hasMatch &= reg
          else if (reg) hasMatch = 0
        }
        ...
        if (hasMatch) {
          ...
          if (mem === memOp) {
            ...
            if (!diffSizeHash[key]) diffSizeHash[key] = [bInst]
            else diffSizeHash[key].push(bInst)
          }
        }
      }
          ...
          if (diff) {
            diff.forEach((diffInst) => {
              if (
                ...
                if (
                  !/^(bndcl|bndcn|bndcu|ptwrite|(v)?cvtsi2ss|(v)?cvtsi2sd|vcvtusi2ss|vcvtusi2sd)$/.test(
                    instName
                  )
                ) {
                  implicit = false
      ...
      for (bIndex = 0; bIndex < sameSizeSet.length; bIndex++) {
        ...
        if (!implicit) DEBUG(`${this.name}: Explicit: ${bInst}`)
      }
    }
  }
...
class InstSignatureTable extends core.Task {
  ...
  run () {
      ...
      insts.forEach((inst) => {
        ...
        if (signatures.length === len) {
          ...
          for (let j = 0; j < len; j++) {
            ...
            const signatureComment = signature.toString()
          ...
          const count = signatureEntries.length
          var index = findSignaturesIndex(signatureEntries)

          if (index === -1) index = indexSignatures(signatureEntries)
    ...
    const s =
      ...
      '    { o0, o1, o2, o3, o4, o5 }                                       \\\n' +
      '  }\n' +
      StringUtils.makeCxxArrayWithComment(
        iSignatureArr,
    ...
    this.inject(
      'InstSignatureTable',
      disclaimer(s),
      oSignatureArr.length * 8 + iSignatureArr.length * 8
    )
  }
  ...
  makeSignatures (dbInsts) {
    ...
    for (let i = 0; i < dbInsts.length; i++) {
      ...
      for (let modrm = 0; modrm < modrmCount; modrm++) {
        const row = new ISignature(inst.name)
        ...
        for (var j = 0; j < ops.length; j++) {
          ...
          if (iop.immValue !== null) break

          // Shorten the number of signatures of 'mov' instruction.
          if (inst.name === 'mov' && mem.startsWith('moff')) break
          ...
          if (seg) {
            switch (inst.name) {
              ...
              case 'umonitor':
                ...
                break
              default:
                console.log(`UNKNOWN MEM IN INSTRUCTION '${inst.name}'`)
                break
            }

            if (iop.memRegOnly) reg = iop.memRegOnly

            if (seg === 'ds') op.flags.memDS = true
            if (seg === 'es') op.flags.memES = true
            if (reg === 'reg') {
              op.flags.memBase = true
            }
            if (reg === 'r32') {
              op.flags.memBase = true
            }
            ...
            if (reg === 'zax') {
              op.flags.memBase = true
              op.flags.memZAX = true
            }
            if (reg === 'zsi') {
              op.flags.memBase = true
              op.flags.memZSI = true
            }
            if (reg === 'zdi') {
              op.flags.memBase = true
              op.flags.memZDI = true
            }
          } else if (reg) {
            if (reg == 'r8') {
              op.flags.r8lo = true

              if (!inst.w || inst.w === 'W0') op.flags.r8hi = true
            } else {
              op.flags[reg] = true
            }
          }
          ...
          if (mem) {
            ...
            if (
              /^(call|enqcmd|enqcmds|lcall|ljmp|movdir64b)$/.test(inst.name)
            ) {
              op.flags.mem = true
            }
          }
          ...
          if (imm) {
            if (iop.immSign === 'any' || iop.immSign === 'signed') {
              op.flags['i' + imm] = true
            }
            if (iop.immSign === 'any' || iop.immSign === 'unsigned') {
              op.flags['u' + imm] = true
            }
          }
        ...
        if (j === ops.length) signatures.push(row)
      }
    }
...
class AdditionalInfoTable extends core.Task {
  ...
  run () {
    ...
    insts.forEach((inst) => {
      ...
      const [r, w] = this.rwFlagsOf(dbInsts)
      const rData =
        r
          .map(function (flag) {
            return `FLAG(${flag})`
      ...
      const instFlagsIndex = instFlagsTable.addIndexed(
        'InstRWFlags(' +
      ...
      inst.additionalInfoIndex = additionaInfoTable.addIndexed(
        `{ ${instFlagsIndex}, ${rwInfoIndex}, { ${features} } }`
      )
    })
    ...
    this.inject(
      'AdditionalInfoTable',
      disclaimer(s),
      additionaInfoTable.length * 8 +
        rwInfoTable.length * 8 +
        instFlagsTable.length * 4
    )
  }
...
const NOT_MEM_AMBIGUOUS = ArrayUtils.toDict(['call', 'movq'])
...
class InstRWInfoTable extends core.Task {
  constructor () {
    ...
    this.rwCategoryByData = {
      Vmov1_8: [
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 8 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 64 },
          _,
          _,
          _,
          _
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 16 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 128 },
          _,
          _,
          _,
          _
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 32 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 256 },
          _,
          _,
          _,
          _
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 64 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 512 },
          _,
          _,
          _,
          _
      ...
      Vmov1_4: [
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 128 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 512 },
          _,
          _,
          _,
      ...
      Vmov1_2: [
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 256 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 512 },
          _,
          _,
          _,
      ...
      Vmov2_1: [
        ...
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 512 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 256 },
          _,
          _,
          _,
      ...
      Vmov4_1: [
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 128 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 32 },
          _,
          _,
          _,
          _
      ...
      Vmov8_1: [
        [
          { access: 'W', clc: 0, flags: {}, fixed: -1, index: 0, width: 128 },
          { access: 'R', clc: 0, flags: {}, fixed: -1, index: 0, width: 16 },
          _,
          _,
          _,
          _
  ...
  run () {
    ...
    insts.forEach((inst) => {
      ...
      // entirely avoided).
      const o2Insts = dbInsts.filter((inst) => {
        return inst.operands.length === 2
      })
      const oxInsts = dbInsts.filter((inst) => {
        return inst.operands.length !== 2
      })

      const rwInfoArray = [
        this.rwInfo(inst, o2Insts),
        this.rwInfo(inst, oxInsts)
      ]
      const rmInfoArray = [
        this.rmInfo(inst, o2Insts),
        this.rmInfo(inst, oxInsts)
      ]
      ...
      for (let i = 0; i < 2; i++) {
        ...
        for (let j = 0; j < rwOps.length; j++) {
          ...
          const opData = StringUtils.formatCppStruct(
            this.byteMaskFromBitRanges([
              { start: rIndex, end: rIndex + rWidth - 1 }
            ]) + 'u',
            this.byteMaskFromBitRanges([
              { start: wIndex, end: wIndex + wWidth - 1 }
            ]) + 'u',
        ...
        const rmData = StringUtils.formatCppStruct(
          ...
          StringUtils.formatCppFlags({
            ...
            'InstDB::RWInfoRm::kFlagMovssMovsd': Boolean(
              inst.name === 'movss' || inst.name === 'movsd'
            ),
            'InstDB::RWInfoRm::kFlagPextrw': Boolean(inst.name === 'pextrw'),
            'InstDB::RWInfoRm::kFlagFeatureIfRMI': Boolean(
              rmInfo.memExtensionIfRMI
            )
          }),
          rmInfo.memExtension === 'None'
            ? '0'
            : 'uint32_t(CpuFeatures::X86::k' + rmInfo.memExtension + ')'
        )
        ...
        const rwData = StringUtils.formatCppStruct(
          ...
          StringUtils.formatCppStruct(
            ...rwOpsIndex.map(function (item) {
              return String(item).padEnd(2)
            })
          )
        )
        ...
        } else this.rwInfoIndexB.push(this.rwInfoTableB.addIndexed(rwData))
      }
    })
  ...
  byteMaskFromBitRanges (ranges) {
    ...
    for (var i = arr.length - 4; i >= 0; i -= 4) {
      const value =
        (arr[i + 3] << 3) | (arr[i + 2] << 2) | (arr[i + 1] << 1) | arr[i]
  ...
  rwInfo (asmInst, dbInsts) {
    ...
    function makeRwFromOp (op) {
      if (!op.isRegOrMem()) return null

      return {
        access:
          op.read && op.write ? 'X' : op.read ? 'R' : op.write ? 'W' : '?',
        clc: 0,
        flags: {},
    ...
    function queryRwGeneric (dbInsts, step) {
      ...
      for (let i = 0; i < dbInsts.length; i++) {
        ...
        for (let j = 0; j < operands.length; j++) {
          ...
          if (op.consecutiveLeadCount) d.clc = op.consecutiveLeadCount
          ...
          switch (instName) {
            ...
            case 'vfcmulcph':
            case 'vfmulcph':
              if (j === 0) d.flags.Unique = true
              break
          }

          if (op.zext) d.flags.ZExt = true

          if (op.regIndexRel) d.flags.Consecutive = true

          for (const k in self.rwOpFlagsForInstruction(asmInst.name, j)) {
            d.flags[k] = true
          }
          ...
          if (d.fixed !== -1) {
            if (op.memSegment) d.flags.MemPhysId = true
            else d.flags.RegPhysId = true
          }
          ...
          if (rwOps[j] === null) {
            ...
            if (
              !ObjectUtils.equalsExcept(rwOps[j], d, {
                fixed: true,
                flags: true
              })
            ) {
            ...
            if (rwOps[j].fixed === -1) rwOps[j].fixed = d.fixed
    ...
    function dumpRwToData (dbInsts) {
      ...
      for (let i = 0; i < dbInsts.length; i++) {
        ...
        if (ArrayUtils.deepIndexOf(out, rwOps) !== -1) continue
    ...
    for (let i = -1; i <= 6; i++) {
      ...
      if (rwInfo) return rwInfo
    }

    // Specific rules.
    for (const k in this.rwCategoryByData) {
  ...
  rwOpFlagsForInstruction (instName, opIndex) {
    ...
    switch (instName + '@' + opIndex) {
      case 'cmps@0':
        return toMap(['MemBaseRW', 'MemBasePostModify'])
      case 'cmps@1':
        return toMap(['MemBaseRW', 'MemBasePostModify'])
      case 'movs@0':
        return toMap(['MemBaseRW', 'MemBasePostModify'])
      ...
      case 'scas@1':
        return toMap(['MemBaseRW', 'MemBasePostModify'])
      case 'bndstx@0':
        return toMap(['MemBaseWrite', 'MemIndexWrite'])
  ...
  rmInfo (asmInst, dbInsts) {
    ...
    if (info.memFixed !== -1) info.category = 'Fixed'
    else if (info.memConsistent) info.category = 'Consistent'
  ...
  rmReplaceableCategory (dbInsts) {
    ...
    for (let i = 0; i < dbInsts.length; i++) {
      ...
      for (let j = 0; j < operands.length; j++) {
        const op = operands[j]
        if (op.isMem()) ms = op.memSize
        else if (op.isReg()) rs = Math.max(rs, op.regSize)
      }
      ...
      if (category === null) category = c
        ...
        // Special cases.
        if (dbInst.name === 'mov' || dbInst.name === 'vmovddup') return 'None'
  ...
  rmReplaceableIndexes (dbInsts) {
    function maskOf (inst, fn) {
      ...
      for (let i = 0; i < operands.length; i++) {
        if (fn(operands[i])) m |= 1 << i
      }
      return m
    }

    function getRegIndexes (inst) {
      return maskOf(inst, function (op) {
        return op.isReg()
      })
    }
    function getMemIndexes (inst) {
      return maskOf(inst, function (op) {
        return op.isMem()
      })
    }
  ...
  rmFixedSize (insts) {
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      for (let j = 0; j < operands.length; j++) {
        const op = operands[j]
        if (op.mem) {
          if (savedOp && savedOp.mem !== op.mem) return -1
          savedOp = op
        }
      }
    }
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      for (let j = 0; j < operands.length; j++) {
        ...
        if (op.mem) {
          hasMem = 1
          if (!op.reg) return 0
          if (asmdb.x86.Utils.regSize(op.reg) !== op.memSize) return 0
        }
      }
    }
    return hasMem
  ...
  rmIsAmbiguous (dbInsts) {
    function isAmbiguous (dbInsts) {
      ...
      for (let i = 0; i < dbInsts.length; i++) {
        ...
        if (hasImm) {
          if (immMap[immStr] === true) continue
          immMap[immStr] = true
        }

        if (hasMem) {
          if (memMap[memStr] === true) return 1
          memMap[memStr] = true
        }
      }
      return 0
    ...
    // Special cases.
    if (!dbInsts.length) return 0

    if (NOT_MEM_AMBIGUOUS[dbInsts[0].name]) return 0

    return (
      (isAmbiguous(Filter.byArch(uniqueInsts, 'X86')) << 0) |
      (isAmbiguous(Filter.byArch(uniqueInsts, 'X64')) << 1)
    )
  }

  rmExtension (dbInsts) {
    if (!dbInsts.length) return 'None'
  ...
  rmExtensionIfRMI (dbInsts) {
    ...
    const name = dbInsts[0].name
    return /^(vpslld|vpsllq|vpsrad|vpsrld|vpsrlq|vpslldq|vpsllw|vpsraw|vpsrldq|vpsrlw)$/.test(
      name
    )
  }
}
...
class InstCommonTable extends core.Task {
  ...
  run () {
    ...
    insts.forEach((inst) => {
      const commonFlagsArray = inst.flags.filter((flag) => {
        return !flag.startsWith('Avx512')
      })
      const avx512FlagsArray = inst.flags.filter((flag) => {
        return flag.startsWith('Avx512')
      })
