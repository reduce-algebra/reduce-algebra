...
(function ($scope, $as) {
  ...
  function buildCpuRegs (defs) {
    ...
    for (const type in defs) {
      ...
      if (def.any) map[def.any] = { type, kind, index: -1 }

      for (let i = 0; i < names.length; i++) {
        const name = names[i]
        const m = /^([A-Za-z\(\)]+)(\d+)-(\d+)([A-Za-z\(\)]*)$/.exec(name)
        ...
        if (m) {
          ...
          for (let n = a; n <= b; n++) {
            ...
            map[index] = { type, kind, index }
          }
        } else {
          map[name] = { type, kind, index: i }
        }
      }
    }
  ...
  const kCpuRegisters = buildCpuRegs({
    r8: {
      kind: 'gp',
      any: 'r8',
      names: ['al', 'cl', 'dl', 'bl', 'spl', 'bpl', 'sil', 'dil', 'r8-15b']
    },
    r8hi: { kind: 'gp', names: ['ah', 'ch', 'dh', 'bh'] },
    r16: {
      kind: 'gp',
      any: 'r16',
      names: ['ax', 'cx', 'dx', 'bx', 'sp', 'bp', 'si', 'di', 'r8-15w']
    },
    r32: {
      kind: 'gp',
      any: 'r32',
      names: ['eax', 'ecx', 'edx', 'ebx', 'esp', 'ebp', 'esi', 'edi', 'r8-15d']
    },
    r64: {
      kind: 'gp',
      any: 'r64',
      names: ['rax', 'rcx', 'rdx', 'rbx', 'rsp', 'rbp', 'rsi', 'rdi', 'r8-15']
    },
    rxx: {
      kind: 'gp',
      names: ['zax', 'zcx', 'zdx', 'zbx', 'zsp', 'zbp', 'zsi', 'zdi']
    },
    sreg: {
      kind: 'sreg',
      any: 'sreg',
      names: ['es', 'cs', 'ss', 'ds', 'fs', 'gs']
    },
  ...
  class Utils {
    static splitInstructionSignature (s) {
      ...
      if (s.startsWith('[')) {
        ...
        prefixes = s
          .substring(1, prefixEnd)
          .replace('xacqrel', 'xacquire|xrelease')
          .split('|')
      ...
      const names = s
        .substring(0, nameEnd === -1 ? s.length : nameEnd)
        .split('|')
    ...
    static isRegOp (s) {
      return s && hasOwn.call(kCpuRegisters, s)
    }
    ...
    static isMemOp (s) {
      return (
        s &&
        /^(?:mem|mib|tmem|moff|(?:m(?:off)?\d+(?:dec|bcd|fp|int)?)|(?:m16_\d+)|(?:vm\d+(?:x|y|z)))$/.test(
          s
        )
      )
    }
    ...
    static isImmOp (s) {
      return (
        s && /^(?:1|i4|u4|ib|ub|iw|uw|id|ud|if|iq|uq|p16_16|p16_32)$/.test(s)
      )
    }

    // Get whether the string `s` describes a relative displacement (label).
    static isRelOp (s) {
      return s && /^rel\d+$/.test(s)
    }

    // Get a register type of a `s`, returns `null` if the register is unknown.
    static regTypeOf (s) {
      return hasOwn.call(kCpuRegisters, s) ? kCpuRegisters[s].type : null
    }

    // Get a register kind of a `s`, returns `null` if the register is unknown.
    static regKindOf (s) {
      return hasOwn.call(kCpuRegisters, s) ? kCpuRegisters[s].kind : null
    }
    ...
    static regIndexOf (s) {
      return hasOwn.call(kCpuRegisters, s) ? kCpuRegisters[s].index : null
    }

    static regSize (s) {
      if (s in RegSize) return RegSize[s]

      const reg = kCpuRegisters[s]
      if (reg && reg.type in RegSize) return RegSize[reg.type]
  ...
  class Operand extends base.Operand {
    constructor (data, defaultAccess) {
      ...
      if (mBcst) {
        ...
        s =
          s.substring(0, mBcst.index) +
          s.substring(mBcst.index + mBcst[0].length)
      }
      ...
      for (let i = 0; i < ops.length; i++) {
        ...
        if (Utils.isImmOp(op)) {
          ...
          if (!this.immSign) this.immSign = sign
          else if (this.immType !== sign) this.immSign = 'any'
          ...
          if (type.indexOf('imm') !== -1) type.push('imm')
          continue
        }
      ...
      if (!mAccess && this.isRegOrMem()) this.setAccess(defaultAccess)
    }
    ...
    isFixedReg () {
      return this.reg && this.reg !== this.regType && this.reg !== 'st(i)'
    }

    isFixedMem () {
      return this.memSegment && this.isFixedReg()
    }
    ...
    toRegMem () {
      if (this.reg && this.mem) return this.reg + '/m'
      else if (this.mem && (this.vsibReg || /fp$|int$/.test(this.mem))) {
        return this.mem
      } else if (this.mem) return 'm'
  ...
  class Instruction extends base.Instruction {
    constructor (db, data) {
      ...
      this._assignEncoding(
        semicolon !== -1 ? data.op.substring(0, semicolon) : 'NONE'
      )
      this._assignOpcode(
        semicolon !== -1
          ? data.op.substring(semicolon + 1).trim()
          : data.op.trim()
      )

      for (const k in data) {
        if (k === 'name' || k === 'op' || k === 'operands') continue
    ...
    _assignAttribute (key, value) {
      switch (key) {
        ...
        case 'k':
          this.kmask = true
          if (typeof value === 'string') super._assignAttribute(key, value)
          return
    ...
    _assignOperands (s) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (operand.mem == 'tmem') this.tsib = true
    ...
    _assignOpcode (s) {
      ...
      if (/^(EVEX|VEX|XOP)\./.test(s)) {
        ...
        for (i = 0; i < prefix.length; i++) {
          ...
          if (comp === 'P0') {
            /* ignored, `P` is zero... */ continue
          }
          if (/^(?:66|F2|F3)$/.test(comp)) {
            this.pp = comp
            continue
          }

          // Process `MM` field - 0F/0F3A/0F38/MAP5/MAP6/M8/M9.
          if (/^(?:0F|0F3A|0F38|MAP5|MAP6|M08|M09|M0A)$/.test(comp)) {
            this.mm = comp
            continue
          }

          // Process `W` field.
          if (/^WIG|W0|W1$/.test(comp)) {
            this.w = comp
            continue
          }
        ...
        for (i = 1; i < parts.length; i++) {
          ...
          this.report(
            `'${this.opcodeString}' Unhandled opcode component: ${comp}`
          )
        }
      } else {
        ...
        for (i = 0; i < parts.length; i++) {
          ...
          if (/^[0-9A-F]{2}(?:\+[ri])?$/.test(comp)) {
            ...
            if (
              !this.mm &&
              /^(?:D8|D9|DA|DB|DC|DD|DE|DF)$/.test(this.opcodeHex)
            ) {
              ...
              if (this.opcodeHex === '67') {
                ...
                if (!this.modR && !this.modRM) {
                  ...
                  if ((value & 0xc0) == 0xc0) {
                    ...
                    this.report(
                      `'${this.opcodeString}' Unsupported secondary opcode (MOD/RM) '${comp}' value`
                    )
                  }
                } else {
                  this.report(
                    `'${this.opcodeString}' Multiple opcodes, have ${this.opcodeHex}, found ${comp}`
                  )
                }
              }
            }
          ...
          if (/^(?:cb|cw|cd)$/.test(comp) && !this.rel) {
            this.rel =
              comp === 'cb' ? 1 : comp === 'cw' ? 2 : comp === 'cd' ? 4 : -1
            continue
          }
      ...
      if (
        this.opcodeHex === '' &&
        this.mm.indexOf('0F01') === this.mm.length - 4
      ) {
        this.opcodeHex = '01'
        this.mm = this.mm.substring(0, this.mm.length - 2)
      }

      if (this.opcodeHex) this.opcodeValue = parseInt(this.opcodeHex, 16)

      if (!this.opcodeHex) {
        this.report(
          `Couldn't parse instruction's opcode '${this.opcodeString}'`
        )
      }
    }
    ...
    _updateOperandsInfo () {
      ...
      for (let i = 0; i < this.operands.length; i++) {
        ...
        // Propagate broadcast.
        if (op.bcstSize > 0) this._assignAttribute('broadcast', op.bcstSize)

        // Propagate VSIB.
        if (op.vsibReg) {
          if (this.vsibReg) {
            this.report(
              'Only one operand can be a vector memory address (vmNNx)'
            )
          }
        ...
        if (op.regIndexRel) {
          if (i - op.regIndexRel < 0) {
            this.report(
              `The consecutive register information is invalid, index of the lead (${i - op.regIndexRel}) is out of range`
            )
          } else {
            ...
            if (consecutiveLead && consecutiveLead != lead) {
              ...
              consecutiveLastIndex = Math.max(
                consecutiveLastIndex,
                op.regIndexRel
              )
            }
          }
    ...
    _postProcess () {
      if (this.privilege === 'L0') this.category.SYSTEM = true

      let isValid = true
      const immCount = this.immCount
      ...
      if (immCount > 0) {
        if (
          immCount === 1 &&
          this.operands[this.operands.length - 1].data === '1'
        ) {
          ...
          const m = this.opcodeString.match(/(?:^|\s+)(ib|iw|id|if|iq|\/is4)/g)
          if (!m || m.length !== immCount) {
            isValid = false
            this.report(
              `Immediate(s) [${immCount}] not found in opcode: ${this.opcodeString}`
            )
          }
        }
      }
    ...
    isAVX () {
      return this.isVEX() || this.isEVEX()
    }

    isVEX () {
      return this.prefix === 'VEX' || this.prefix === 'XOP'
    }

    isEVEX () {
      return this.prefix === 'EVEX'
    }
    ...
    get signature () {
      ...
      if (this.prefix) {
        ...
        if (this.prefix !== '3DNOW') {
          if (this.l === 'L1') sign += '.256'
          else if (this.l === '256' || this.l === '512') sign += `.${this.l}`
          else sign += '.128'
      ...
      for (let i = 0; i < operands.length; i++) {
        ...
        if (operand.implicit) sign += `[${operand.reg}]`
        else sign += operand.toRegMem()
      }
    ...
    get immCount () {
      ...
      let n = 0
      for (let i = 0; i < ops.length; i++) if (ops[i].isImm()) n++
      return n
    }

    get modRValue () {
      if (/^[0-7]$/.test(this.modR)) return parseInt(this.modR, 10)
      else return 0
    }

    get modRMValue () {
      if (/^[0-7]$/.test(this.modRM)) return parseInt(this.modRM, 10)
      else return 0
    }
  }
  ...
  function mergeGroupData (data, group) {
    for (const k in group) {
      switch (k) {
        ...
        default:
          if (data[k] === undefined) data[k] = group[k]
          break
      }
    }
  }
      ...
      for (const group of groups) {
        for (const inst of group.data) {
          ...
          for (let j = 0; j < sgn.names.length; j++) {
            ...
            if (j > 0) data.aliasOf = sgn.names[0]
          ...
          for (let j = i + 1; j < insts.length; j++) {
            ...
            if (instA.operands.join('_') === instB.operands.join('_')) {
              ...
              if (vex && evex && vex.opcodeHex === evex.opcodeHex) {
                ...
                const ok = vex.w === evex.w && vex.l === evex.l

                if (!ok) {
                  console.log(`Instruction ${name} differs:`)
                  console.log(
                    `  ${vex.operands.join(' ')}: ${vex.opcodeString}`
                  )
                  console.log(
                    `  ${evex.operands.join(' ')}: ${evex.opcodeString}`
                  )
                }
              }
            }
