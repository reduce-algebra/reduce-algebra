  ...
  class Instruction extends base.Instruction {
    ...
    _assignAttributeKeyValue (name, content) {
      ...
      for (let i = 0; i < attributes.length; i++) {
        ...
        const key = eq === -1 ? attr : attr.substring(0, eq)
        const val = eq === -1 ? true : attr.substring(eq + 1)
    ...
    _assignOperands (s) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        const shiftOp = Utils.parseShiftOrExtendOp(def)
        if (shiftOp) {
          op.shiftOp = shiftOp
    ...
    _assignOpcode (s) {
      ...
      if (opcodeIndex !== 32) {
        FAIL(
          `The number of bits '${opcodeIndex}' used by the opcode '${s}' doesn't match 32`
        )
      }
