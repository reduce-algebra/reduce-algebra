...
def move_file(src_dir,
    ...
    if file_suffix:
        if filename in ["internal64.h", "asmnames.h", "internal.h"]:
            ...
            out_filename = "%s_%s%s" % (split_name[0], file_suffix,
                                        split_name[1])
...
def build_target(platform, platform_headers):
    ...
    def xcrun_cmd(cmd):
        return "xcrun -sdk %s %s -target %s" % (platform.sdk, cmd,
                                                platform.target)
    ...
    try:
        ...
        subprocess.check_call(
            ...
            ] + ([] if platform.sdk == "macosx" else
                 [f"--build={os.uname().machine}-apple-darwin"]),
            env=env,
        )
    finally:
...
def generate_source_and_headers(
    ...
    for header_name, tag_tuples in platform_headers.items():
        ...
        with open(os.path.join("darwin_common/include", header_name),
                  "w") as header:
            for tag_tuple in tag_tuples:
                header.write("%s#include <%s_%s%s>\n%s\n" %
                             (tag_tuple[0], basename, tag_tuple[1], suffix,
                              tag_tuple[2]))
...
if __name__ == "__main__":
    ...
    generate_source_and_headers(
        generate_osx=not args.only_ios and not args.only_tvos
        and not args.only_watchos,
        generate_ios=not args.only_osx and not args.only_tvos
        and not args.only_watchos,
        generate_tvos=not args.only_ios and not args.only_osx
        and not args.only_watchos,
        generate_watchos=not args.only_ios and not args.only_osx
        and not args.only_tvos,
    )
