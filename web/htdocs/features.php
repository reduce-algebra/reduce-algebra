...
$smarty->assign('page_title', 'REDUCE Features');

$smarty->assign('header_title', 'Features');

$smarty->assign('main_content', <<< END_OF_MAIN_CONTENT
<p>
  REDUCE is a system for general algebraic computations of interest to
  mathematicians, scientists and engineers, which is normally used
  interactively. It has been produced by a collaborative effort
  involving many <a href="about.php#contributors">contributors</a>.
</p>
<h2>Basic features</h2>
<p>
  The basic features of REDUCE, which might be useful to students and
  teachers from high school / secondary school up to university level,
  include:
</p>
<ul>
...
<p>
  More advanced features, which allow calculations at university level
  and beyond, include:
</p>
<ul>
...
<p>
  ...
  problems that are possible to do by hand. However, it
  is <a>designed</a> to support calculations that are not feasible by
  hand. Many such calculations take a significant time to set up and
  can run for minutes, hours or even days on the most powerful
  computers. REDUCE supports non-interactive batch-mode use for large
  computations.
</p>
...
<dl>
  ...
  refinements since then, with the source being subject to wide review
  by the user community. REDUCE has thus evolved into a powerful
  system whose critical components are highly reliable, stable and
  efficient.</dd>
  <dt>Full source code availability</dt>
  <dd>From the beginning, it has been possible to obtain the complete
  REDUCE source code, including the &quot;kernel.&quot; Consequently,
  REDUCE is a valuable educational resource and a good foundation for
  experiments in the discipline of computer algebra. Many users do in
  fact effectively modify the source code for their own purposes.</dd>
  <dt>Flexible updating</dt>
  <dd>One advantage of making all code accessible to the user is that
  it is relatively easy to incorporate patches to correct small
  problems or extend the applicability of existing code to new problem
  areas. World Wide Web servers (currently SourceForge and its mirror
  sites) allow users to get such updates and completely new packages
  as they become available, without having to wait for a formal system
  release.</dd>
  <dt>Portability</dt>
...
<p>
  ...
  packages for specific purposes. A particular algebra system is often
