...
const echo = false; // setting of REDUCE echo switch, accessed in FileMenu.mjs and InputEditor.mjs
let inputFromKbd, outputToFile, outputToArray; // assigned in FileMenu.mjs and InputEditor.mjs
...
document
  .getElementById("TemplatesMenuLink")
  .addEventListener("show.bs.dropdown", saveSelectionProps);
document
  .getElementById("FunctionsMenuLink")
  .addEventListener("show.bs.dropdown", saveSelectionProps);
...
function inputDivInsert(text) {
  ...
  if (insertionNode === inputDiv) {
    ...
    if (
      !(
        inputDiv.contains(insertionNode) &&
        inputDiv.contains(selProps.focusNode)
      )
    ) {
