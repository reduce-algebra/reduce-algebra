...
class ODESolveTemplate extends Template {
  constructor() {
    ...
    this.dialogue.addEventListener("show.bs.modal", () =>
      loadPackage("odesolve")
    );
  }
  ...
  result() {
    ...
    let y = this.inputs[1].value.trim(),
      x: string;
    // Conditions (optional):
    ...
    if (ode.includes("'") || conds.includes("'")) {
      ...
      ode = deprime(ode);
      conds = deprime(conds);
    } else x = this.inputs[2].value.trim();
    let text = "odesolve(" + ode;
    if (x.length > 0) {
      if (y.length == 0) y = "{}";
      text += ", " + y + ", " + x;
    } else if (y.length > 0) text += ", " + y;
    if (conds.length > 0) text += ", {" + conds + "}";
    // Options:
    const optEls = this.pattern.querySelectorAll<HTMLInputElement>(
      "input[type=checkbox]:checked"
    );
    for (const optEl of optEls) {
      const option = this.pattern.querySelector<HTMLInputElement>(
        `label[for=${optEl.id}]`
      ).innerText;
