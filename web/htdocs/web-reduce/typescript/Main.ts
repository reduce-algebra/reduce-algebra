...
declare function launchGnuplot(data: {
  channel: string;
  script: string;
...
const startREDUCEMenuItem = document.getElementById(
  "StartREDUCEMenuItem",
) as HTMLButtonElement;
const loadPackagesMenuItem = document.getElementById(
  "LoadPackagesMenuItem",
) as HTMLButtonElement;
const stopREDUCEMenuItem = document.getElementById(
  "StopREDUCEMenuItem",
) as HTMLButtonElement;
const restartREDUCEMenuItem = document.getElementById(
  "RestartREDUCEMenuItem",
) as HTMLButtonElement;
const clearDisplayMenuItem = document.getElementById(
  "ClearDisplayMenuItem",
) as HTMLButtonElement;
const ioColouringCheckbox = document.getElementById(
  "IOColouringCheckbox",
) as HTMLInputElement;
...
export const typesetMathsCheckbox = document.getElementById(
  "TypesetMathsCheckbox",
) as HTMLInputElement;
typesetMathsCheckbox.checked = true;
const centreTypesetMathsCheckbox = document.getElementById(
  "CentreTypesetMathsCheckbox",
) as HTMLInputElement;
...
export function refocus() {
  if (mobileVersion)
    // Ensure I/O Display is visible:
    window.scrollTo(0, document.getElementById("IODisplayIframe")!.offsetTop); // Focus the input editor:
  else inputDiv.focus();
}

/** Earlier Button HTML Element. */
export const earlierButton = document.getElementById(
  "EarlierButton",
) as HTMLButtonElement;
earlierButton.disabled = true;

/** Send Input Button HTML Element. */
export const sendInputButton = document.getElementById(
  "SendInputButton",
) as HTMLButtonElement;

/** Plot Button HTML Element. */
export const plotButton = document.getElementById(
  "PlotButton",
) as HTMLButtonElement;
plotButton.hidden = !($gnuplot && $debug);

/** Later Button HTML Element. */
export const laterButton = document.getElementById(
  "LaterButton",
) as HTMLButtonElement;
laterButton.disabled = true;
const fileMenuLink = document.getElementById(
  "FileMenuLink",
) as HTMLAnchorElement;
const templatesMenuLink = document.getElementById(
  "TemplatesMenuLink",
) as HTMLAnchorElement;
const functionsMenuLink = document.getElementById(
  "FunctionsMenuLink",
) as HTMLAnchorElement;

interface MJWindow extends Window {
  readonly MathJax?: any;
}
let ioDisplayWindow: MJWindow,
  ioDisplayHead: HTMLElement,
  ioDisplayBody: HTMLElement;
...
if (typeof Worker === "undefined") {
  ...
  alert(
    "This browser does not support Web Workers and so cannot run Web-REDUCE!",
  );
...
function reduceWebMessageHandler(event: {
  ...
  if (event.data.channel === "stdout" && event.data.line !== undefined) {
    if (Global.hideOutput) {
      // hide changes of switches etc.
    ...
    if (output == "") {
      ...
      if (n != -1) {
        ...
        if (match) {
          ...
          sendPlainTextToIODisplay(
            output,
            Global.inputFromKbd ? "output" : undefined,
          );
        }
      }
    }
  } else if (event.data.channel === "plot") {
    // Hermann Rolfes
    launchGnuplot(
      event.data as {
        channel: string;
...
export let sendToReduce = (str: string) => {
  ...
  for (let i = 0; i < str.length; i++) buf[i] = str.charCodeAt(i); // Returns a number that is the UTF-16 code unit value at the given index.
  worker.postMessage({
...
async function startREDUCE() {
  // async and sleep added by Hermann Rolfes
  ...
  try {
    ...
    worker = new Worker(
      mobileVersion ? "mobile/reduce.web.js" : "generated/reduce.web.js",
    );
...
document
  .getElementById("PrintDisplayMenuItem")!
  .addEventListener("click", () => ioDisplayWindow.print());
...
const fullWindowCheckbox = document.getElementById(
  "FullWindowCheckbox",
) as HTMLInputElement;
...
ioColouringStyleElement.innerText =
  "pre.input {color: red;} *.output {color: blue;}";
...
ioColouringCheckbox.addEventListener("change", () => {
  ...
  else ioColouringStyleElement.remove();
  hideViewMenuLink();
});
...
centreTypesetMathsCheckbox.addEventListener("change", (event) => {
  ioDisplayWindow.MathJax.config.chtml.displayAlign =
    centreTypesetMathsCheckbox.checked ? "center" : "left";
