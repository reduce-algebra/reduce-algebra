...
class ModalDialogue extends HTMLElement {
  constructor() {
    ...
    if (this.hasAttribute("specfnsHeader")) {
      const specfnsHeader = (
        document
          .getElementById("special-functions-header")!
    ...
    const templateContent = (
      document.getElementById("modal-dialogue-template") as HTMLTemplateElement
...
class TempFuncs {
  ...
  constructor(dialogueId: string) {
    ...
    this.inputs =
      this.dialogue.querySelectorAll<HTMLInputElement>("input[type=text]");
    this.buttons = this.dialogue.querySelectorAll<HTMLButtonElement>(
  ...
  inputDivInsert(text: string) {
    ...
    if (insertionNode === inputDiv) {
      ...
      inputDiv.textContent =
        cont.substring(0, selBeg) + text + cont.substring(selEnd); // previously used substr, change OK?
    }
...
export class Template extends TempFuncs {
  ...
  getValueCheckNonEmpty(input: HTMLInputElement, checkNonEmpty?: boolean) {
    ...
    if ((checkNonEmpty || this.checkNonEmpty) && value.length === 0) {
      alert(this.alertHeader + "A required field is empty.");
...
export class Functions extends TempFuncs {
  ...
  constructor(functionsId: string) {
    ...
    const functions = this.dialogue.querySelectorAll(
      "div.function-templates>div",
  ...
  result(): string {
    if (this.selectedFunction) {
      const values = [
        ...this.selectedFunction.getElementsByTagName("input"),
