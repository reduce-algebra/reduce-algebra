...
const pkgButtonGrid = loadPackagesDialogue.querySelector(
  "div.modal-body > div"
);
const pkgButtons = [];
let pkgNo = 1;
...
const buttons = loadPackagesDialogue.querySelectorAll<HTMLButtonElement>(
  "div.modal-footer > button"
);
buttons[0].addEventListener("click", () =>
  pkgButtons.forEach((btn) => (btn.checked = false))
);
...
function loadButtonAction() {
  ...
  const selectedPackages = pkgButtons
    .filter((btn) => btn.checked)
    .map(
      (btn) =>
        pkgButtonGrid.querySelector<HTMLLabelElement>(`label[for='${btn.id}']`)
          .innerText
    );
...
function enableManualButton() {
  ...
  setTimeout(() => {
    buttons[1].disabled = pkgButtons.filter((btn) => btn.checked).length !== 1;
  });
}
...
function showManualForSelectedPackage() {
  ...
  if (button)
    showManualForLabel(
      pkgButtonGrid.querySelector<HTMLLabelElement>(`label[for='${button.id}']`)
    );
}
...
function showManualForLabel(label: HTMLLabelElement) {
  let pkg = label.innerText,
    lookup: string = manualLookupMap.get(pkg);
