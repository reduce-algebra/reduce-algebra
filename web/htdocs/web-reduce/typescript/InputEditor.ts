...
function earlierInput(event: KeyboardEvent | MouseEvent) {
  ...
  if (inputListIndex > 0) {
    ...
    if (inputListIndex <= maxInputListIndex) laterButton.disabled = false;
  }
  if (inputListIndex == 0) earlierButton.disabled = true;
  inputDiv.focus();
}
...
const matchDelimsCheckbox = document.getElementById(
  "MatchDelimsCheckbox"
) as HTMLInputElement;
...
const closeDelimToRegexp = {
  ")": /[()"]/g,
  "}": /[{}"]/g,
  ">>": /<<|>>|"/g,
  end: /\bbegin\b|\bend\b|"/gi,
};
const openDelimToRegexp = {
  "(": /[()"]/g,
  "{": /[{}"]/g,
  "<<": /<<|>>|"/g,
  begin: /\bbegin\b|\bend\b|"/gi,
};
...
function highlightMatchingDelims() {
  ...
  const caretNode: Node = selection.anchorNode,
    caret: number = selection.anchorOffset;
  ...
  const atEOL: boolean =
    caretNode.parentNode !== inputDiv &&
    (<Element>caretNode.parentNode).tagName === "DIV" &&
    caret === (<Text>caretNode).length &&
    caretNode.parentNode.nextSibling !== null;
  if (textBefore) {
    // Check for matching delimiters closing immediately before caret.
    // The regexp flag i makes searching case-insensitive.
    let closeDelim: string | any[],
      openIndex = -1;
    const initialMatch = textBefore.match(/(?:[)}]|>>|\bend)$/i);
    if (
      initialMatch &&
      !(
        (closeDelim = initialMatch[0].toLowerCase()) === "end" &&
        /^(?:\w|\d)/.test(textAfter)
      )
    ) {
  ...
  if (textAfter) {
    ...
    let openDelim: string | any[],
      closeIndex = -1;
    const initialMatch = textAfter.match(/^(?:[({]|<<|begin\b)/i);
    if (
      initialMatch &&
      !(
        (openDelim = initialMatch[0].toLowerCase()) === "begin" &&
        /(?:\w|\d)$/.test(textBefore)
      )
    ) {
