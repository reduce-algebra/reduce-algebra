...
gnuplot.L = function (x, y) {
  if (gnuplot.zoomed) {
    const zoom = gnuplot.zoomXY(x / 10.0, y / 10.0);
...
gnuplot.R = function (x, y, w, h) {
  if (gnuplot.zoomed) {
    let dx, dy, dw, dh;
    let zoom = gnuplot.zoomXY(x / 10.0, y / 10.0);
    if (zoom.x >= gnuplot.plot_xmax) return;
    if (zoom.y >= gnuplot.plot_ybot) return;
    dx = zoom.x;
    dy = zoom.y;
    zoom = gnuplot.zoomXY((x + w) / 10.0, (y + h) / 10.0);
...
gnuplot.T = function (x, y, fontsize, justify, string) {
  ...
  let yy = y / 10.0;
  if (gnuplot.zoomed) {
    const zoom = gnuplot.zoomXY(xx, yy);
    if (zoom.clip) return;
    xx = zoom.x;
    yy = zoom.y;
    if (
      ...
      if (
        typeof gnuplot.zoom_text !== "undefined" &&
        gnuplot.zoom_text == true
      ) {
...
gnuplot.ZI = function (image, m, n, x1, y1, x2, y2) {
  if (gnuplot.zoomed) {
    ...
    let zoom = gnuplot.zoomXY(x1 / 10.0, y1 / 10.0);
    if (zoom.x >= gnuplot.plot_xmax) return;
    if (zoom.y >= gnuplot.plot_ybot) return;
    x1raw = zoom.xraw;
    y1raw = zoom.yraw;
    ...
    zoom = gnuplot.zoomXY(x2 / 10.0, y2 / 10.0);
    if (zoom.xraw <= gnuplot.plot_xmin) return;
    if (zoom.yraw <= gnuplot.plot_ytop) return;
    const x2raw = zoom.xraw;
    const y2raw = zoom.yraw;
    dw = zoom.x - dx;
    dh = zoom.y - dy;
    ...
    if (x2raw > zoom.x) {
      sw = m * (1.0 - (x2raw - zoom.x) / (x2raw - x1raw));
    }
    if (y2raw > zoom.y) {
      sh = n * (1.0 - (y2raw - zoom.y) / (y2raw - y1raw));
    }
    sw = sw - sx;
    sh = sh - sy;
