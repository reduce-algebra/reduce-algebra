...
function ___syscall_ftruncate64(fd, length_low, length_high) {
  const length = convertI32PairToI53Checked(length_low, length_high);
...
const __emscripten_system = (command) => {
  if (ENVIRONMENT_IS_NODE) {
    ...
    const _W_EXITCODE = (ret, sig) => (ret << 8) | sig;
...
function __localtime_js(time_low, time_high, tmPtr) {
  const time = convertI32PairToI53Checked(time_low, time_high);
...
const getHeapMax = () => HEAPU8.length;
...
const addDays = (date, days) => {
  ...
  while (days > 0) {
    ...
    const daysInCurrentMonth = (leap ? MONTH_DAYS_LEAP : MONTH_DAYS_REGULAR)[
      currentMonth
    ];
...
let setTempRet0 = (a0) => (setTempRet0 = wasmExports.setTempRet0)(a0);
...
let stackSave = () => (stackSave = wasmExports.stackSave)();
let stackRestore = (a0) => (stackRestore = wasmExports.stackRestore)(a0);
