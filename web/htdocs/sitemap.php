...
<style>
    ...
    #SiteMap tr[data-level='1'],
    #SiteMap tr[data-level='2'] {
        display: none;
    }
...
function processFile(string $file, int $level)
{
    ...
    if (array_key_exists($file, (array) $cache) and $date == ($c = $cache[$file])[1]) {
...
if (!isset($cache) || !$cache) $cache = array();
...
if ($recache) {
    ...
    if (!isset($written) || !$written) echo "<p>WARNING: Cannot write cache file!</p>\n";
}
...
<nav>
    <div class="nav nav-tabs nav-justified" role="tablist">
        <button class="nav-link active" id="SiteMap-tab" data-bs-toggle="tab" data-bs-target="#SiteMap" type="button" role="tab" aria-controls="SiteMap" aria-selected="true">Site Map</button>
        <button class="nav-link" id="RecentUpdates-tab" data-bs-toggle="tab" data-bs-target="#RecentUpdates" type="button" role="tab" aria-controls="RecentUpdates" aria-selected="false">Recent Updates</button>
    </div>
</nav>

<div class="tab-content">
    <div class="tab-pane fade show active" id="SiteMap" role="tabpanel" aria-labelledby="SiteMap-tab">
        <p>
    ...
    <div class="tab-pane fade" id="RecentUpdates" role="tabpanel" aria-labelledby="RecentUpdates-tab">
        <p>
        ...
        <table>
            ...
            $sitemap = array_filter($sitemap, function ($row) {
                return $row[0];
            });
