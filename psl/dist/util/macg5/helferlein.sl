...
(de fnco (x) (getmem (wplus2 symfnc (times2 addressingunitsperfunctioncell 
					(inf x)))))
...
(de printo (x) (puthex x 16) (terminalwritechar 32 32)
	    ...
	    (case (tag x)
	         ((posint-tag negint-tag)
	                      (progn (console-print-number x) (console-newline)))
...
(de dumpo (x n)
   (prog (outputbase* addr)
	...
	(setq x (wshift (wshift x -2) 2)) % avoid bus error
	(ifor (from i 0 n 1)
	  (do (progn
