...
(compiletime 
 ...
 (defmacro external-64-function (name args)
     (setf *foreign-functions* (cons name *foreign-functions*))
     (flag1 name '64-foreignfunction)
     `(de ,name ,args
        (,name ,@args)
        )))
...
% Defined in sigs.c
%
(external-function psl_sigset           (signame handler))
...
(lap '((*entry xgetw expr 1)
       (*alloc 5)
       (*foreignlink xgetw expr 1)
       (*move (reg 1) (reg r2))
       (*move (reg 2) (reg 1))
       (rldimi (reg 1) (reg r2) 32 0)
       (*exit 5)))

%(external-function ieee_handler         (set common signalhandler))
(external-function putw                 (w fp))
(external-function signal               (signame handler))
(external-function sleep		(sec))
(external-function setjmp               (env))  % added for sparc
(external-function longjmp              (env val)) % to process interrupts.

(external-function fork ())
(external-function wait(ptr))

%(external-function vvm_cflush           (addr len)) % flush the cache(s)
(external-function datetag())           % returns date of bpsl build
...
(external-function setlinebuf           (iobuffer))
