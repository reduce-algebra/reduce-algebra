...
setupbpsandheap(argc,argv)
     int argc;
     char *argv[];
{ long long ohl,ohtb,ohlb,ohub,hl,htb,hlb,hub;
  int memset = 0; int feder = 200000;
  FILE * imago;
  long long headerword [8];
  int    i, bpssize, mallocsize;
  long long   total, heapsize,mtot;
 ...
 /* ii1=malloc(0x408);
  ...
  current_size_in_bytes = (((long long) sbrk(0))<<4)>>4;
  max_image_size = 0x01ffffffffffffff; /* 1 more than allowable size */
  //printf (" %lx %lx %lx \n",heapsize_in_bytes , current_size_in_bytes, max_image_size);
  ...
  { printf("Setting heap limit as follows:\n");
    printf("Total heap & bps space = %ld (%lx), bps = %.2f, heap = %.2f\n",
...
*/
  ...
  if (imagefile == NULL)
  printf("bpssize = %ld (%X), heapsize = %ld (%lx)\nTotal image size = %ld (%lx)\n",
          bpssize, bpssize,
          heapsize, heapsize,
          (long long int) sbrk(0), (long long int) sbrk(0));
   ...
   if (imagefile != NULL) {
    ...
    /* save the new values around restore of the old ones */
       ...
       fclose (imago);
       imago = fopen (strcat(imagefile,".warrays"),"r");
       fread (headerword,8,2,imago);
       fread (headerword[0],1,headerword[1],imago);
// printf (" read %d%d bytes to %x%x\n",headerword[1],headerword[0]);
       fclose (imago);
...
{ char *p = (char *) bps;
  ...
  bpslowerbound = ((int)bps + 7) & ~7;
  nextbps  = ((int)bps + 7) & ~7;        /* Up to a multiple of 7. */
...
getheap(heapsize,feder)
     long long heapsize,feder;
{
   heaplowerbound   = (long long)calloc(2 * heapsize,1000000);
                  /* allocate first heap */;
  
  if (heaplowerbound == (int) NULL) {
    perror("GETHEAP");
    exit(-1); }
