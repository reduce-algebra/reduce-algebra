...
long my_input(char buf[])
   {
     ...
     do_win_lth = 0;
     while(c = buf[do_win_lth]) 
          do_win_buf[do_win_lth++] = c;
     do_win_buf[do_win_lth] = 0;
     do_win_loop = 0;
     return(1);
   }
...
int win_loop()
{
    while (do_win_loop && GetMessage(&msg, (HWND)NULL, (UINT)NULL, (UINT)NULL)) 
    {
	TranslateMessage(&msg);
	DispatchMessage(&msg);
    }
    if(do_win_loop) Throw(CatchBuf,1);
    return (msg.wParam);
}
	...
	case WM_CHAR: 
     ...
     /* 21.7.93 neu */
      ...
      char_in:
		 ...
		 state = LOOP;
		 if(do_win_loop &&waiting_for_input) caret(1);
	    break;
