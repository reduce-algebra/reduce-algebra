...
my_rpc(char * buf, int len)
   {         
      if(!slave_active) return(0);
         TEST_PRINT("%s RPC call\n %s\n",buf,"");
         ////   pmelden("%s RPC call\n %s\n",buf,"");
      rpc_pending = 1;
      my_psend('c',buf,len);
    //  while(rpc_pending) {my_puts(" n "); Sleep(100);}
    // return(my_preceive('a',buf,len));
      Sleep(100);
   }

input_to_slave(char * buf, int len)
   {
...
listen_slave()
   {
      ...
      next:
       ...
       llen -- ; t = lbuf[llen]; lbuf[llen] ='\0';
         ...
         // pmelden("%s dispatch - received: %s\n",&t,"");

       if (t == 'x') 
          { exit(0); }
       else if(t == 'b')
         { 
           sscanf(lbuf,"%lx",&adr_bruch);
         }
       else if(t == 'w')
         {
           if(slave_active == 0) { mach_klein(); slave_active = 1;}
           local_puts(lbuf);
           my_psend('k',lbuf,0);
         }
...
HANDLE my_popen_master(int n)
 {
      ...
      lpsa.nLength = (DWORD) sizeof (SECURITY_ATTRIBUTES);
      ...
      if(!s) iwarning("getlasterror",GetLastError());
