...
%   (binaryreadblock fid (loc (wgetv code-base 0)) code-size)
    (binaryreadblock fid code-base  code-size)
...
%   (binaryreadblock fid (loc (words-fetch bit-table 0)) bit-table-size)
     (binaryreadblock fid (mkstr (wplus2 4 bit-table)) bit-table-size)
...
(de do-relocation (code-base code-size bit-table id-table)
  ...
  (let ((code-au-size code-size )) %addressingunitsperitem)))
    (for (from i 0 (wdifference code-au-size 1) 1)
     (do 
      (let ((bit-table-entry  (bittable (loc (words-fetch bit-table 0)) 
					(wshift i 2)))
	    (code-location    (wplus2 code-base (wshift i 2) ))	memo )
	(case bit-table-entry
	 ((reloc-word) 
	  (case (bittable (loc (words-fetch bit-table 0))
		...
		((reloc-call)
		    (progn (setq memo (getmem code-location))
		            (setq memo 
		                (wplus2 memo (wshift (wshift code-base 6) -6)))
		                        %clear out first bits (see J instr!)
		                (setf (getmem code-location) memo)))
		((reloc-xidloc) 
		    (progn (setq memo (wand (getmem code-location) 16#ffff))
		           (when (local-id-number? memo)
		               (setq memo (local-to-global-id memo id-table)))
		           (putmem code-location 
		             (wor (wand (getmem code-location) (wnot 16#ffff))
		             (wand (wtimes2 4  (wplus2 memo -8000)) 16#ffff)))
		      ))))
		     %
...
(de relocate-word (code-location code-base id-table)
  (let ((reloc-tag (reloc-word-tag (getmem code-location)))
    ...
    (cond ((posintp (getmem code-location)) % a naked pointer from movex
	     (setf (getmem code-location)
		 (wplus2 code-base (getmem code-location)) ))
	  (t (setf (getmem code-location) % an item
...
(de relocate-movex (code-location code-base id-table)
    (let ((val (getmem (wplus2 code-location -4)))
	  (add (getmem code-location))
          (twentysix 26)
	  (lui nil))
	 (setq lui (wor (wshift 15 twentysix) % CAU instruction
	 ...
	 (setf (getmem (wplus2 code-location -4)) lui)
	 (setf (getmem code-location) add)))
...
(de flushcache (addr len inv) (vvm_cflush addr len inv))
