...
(de subseq (string lower-bound upper-bound)    % To be redefined
  ...
  (unless (stringp string)
    (kernel-fatal-error (kernelstring2string "SUBSEQ called with a non-string"))
    )
...
(de code-putd (fnname fntype fnexp)
  ...
  (unless (and (idp fnname)
	       (idp fntype)
	       (codep fnexp))
    (unixputn fnname) (console-newline)
    (unixputn fntype) (console-newline)   (unixputn fnexp) (console-newline) 
    (kernel-fatal-error (kernelstring2string "Bad parameters to CODE-PUTD"))
    )
  (console-print-string (kernelstring2string  " Function :"))
   (console-print-string (symnam (inf fnname)))
  (console-print-string (kernelstring2string  " "))
  (unixputn (inf fnexp)) (console-newline)
