...
new_vvm_cflush (ad,len,inv) /* does not work */

long ad,len,inv;

{ ad = ( ad / getpagesize()) * getpagesize();
  if (mprotect(ad,50 * getpagesize(),PROT_READ | PROT_WRITE | PROT_EXEC) == -1)
        perror("Error mprotect");
  if (inv) { if (msync((caddr_t) ad,(size_t) (len+ getpagesize()),MS_SYNC | MS_INVALIDATE) == -1 )
         perror("Error cacheflush") ; }
    else
       { if (msync((caddr_t) ad,(size_t) (len+ getpagesize()),MS_SYNC) == -1 )
               perror("Error cacheflush");} }
