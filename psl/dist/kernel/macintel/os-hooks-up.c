...
c2reduce(cntl,argc,argv,buffin)
...
char *argv[], buffin;
{
  ...
  if(cntl==1) { /* subsequent calls after init; everything clean */
  val=setjmp(mainenv);        /* set non-local return point for exit    */

  if (val == 0) _reduceup(buffin, reduceaus = malloc(3000),30,3000);
  if (reduceaus!= (char *) NULL)
       printf("reduceausgabe:%s\n",(int) reduceaus +4);
               };
