...
% Revisions:
%
% 08-Aug-89 (Winfried Neun)
%  Added makefixnum to simplify interfacing to C.
...
(compiletime (remprop '*ftimes2 'opencode))
(compiletime (remprop '*fplus2 'opencode))
(compiletime (remprop '*fdifference 'opencode))
(compiletime (remprop '*fquotient 'opencode))
(compiletime (remprop '*fgreaterp 'opencode))
(compiletime (remprop '*flessp    'opencode))
(compiletime (remprop '*wfloat    'opencode))
(compiletime (remprop '*wfix      'opencode))
         ...
         (ds deffloatentry (name prim) 
             (de name (x y)
               (prog (f)
                     (setq f (gtfltn))
		     (setq *bruch* 0)
                     (prim (mkstr (floatbase f)) (mkstr (floatbase (fltinf x)))
                                     (mkstr (floatbase (fltinf y))))
...
(compiletime (put '**fgreaterp 'opencode
                      '((ld (displacement (reg 1) 4) (reg f0))
                        (ld (displacement (reg 1) 8) (reg f1))
                        (ld (displacement (reg 2) 4) (reg f2))
                        (ld (displacement (reg 2) 8) (reg f3))
                        (fcmpd (reg f0) (reg f2))
                        (*move (fluid t) (reg 1))
                        (fbg  *donefgreaterp*)
                        (nop)
...
% (*fgreaterp 0 (floatbase (fltinf x)) (floatbase (fltinf y))))
(*fgreaterp (floatbase (mkstr x)) (floatbase (mkstr y))))

(defarithpred2entry lessp wlessp floatlessp biglessp)

(compiletime (put '**flessp 'opencode
                   '((ld (displacement (reg 1) 4) (reg f0)) 
                     (ld (displacement (reg 1) 8) (reg f1)) 
                     (ld (displacement (reg 2) 4) (reg f2)) 
                     (ld (displacement (reg 2) 8) (reg f3))
...
(de floatlessp (x y)
% (*flessp 0 (floatbase (fltinf x)) (floatbase (fltinf y))))
 (*flessp (floatbase (mkstr x)) (floatbase (mkstr y))))
...
(de floatfix (x)
  (sys2int (!*wfix (mkstr (floatbase (fltinf x))))))
