...
(fluid '(*gc gcknt* gctime* heap-warn-level symget *bruch* *dump))
...
(compiletime
 (progn
  ...
  (ds markid (x)
    (setf (field (getmem (wplus2 symnam (wtimes2 addressingunitsperitem x)))
    ...
    (if (weq tg id-tag)
      ...
      (if (and (withinoldheappointer info) (pointertagp tg))
        ...
        (if (weq tg2 forward-tag)
          (putmem i (mkitem tg (inf (getmem info))))
...
(de cfserror (where stackitem pos)
   (prog (info hinfo htag outputbase*)
         ...
         (setq htag (tag hinfo))
         (when (and *dump *gc) (centerdump (inf stackitem) 10))
         (when (and *dump *gc) (centerdump1 pos 20))
         (errorprintf
...
(de centerdump (loc howmuch)
  ...
  (console-print-number loc)
  (console-newline)
  (console-newline)
  (for (from  i (minus howmuch) howmuch 8)
    (do (console-print-number (wplus2 loc (wtimes2 addressingunitsperitem i)))
        (console-print-string ": ")
        (for (from j 0 7)
          (do (console-print-number (getmem (wplus2 loc (wtimes2 addressingunitsperitem 
							(wplus2 i j)))))
              (console-print-string " ")))
        (console-newline))))
...
(de copystackitem (p)
  ...
  (prog (typ info hdr x)
        ...
        (setq hdr (getmem info))
        (when (marked hdr)
          (return (putmem p (mkitem typ (inf hdr)))))
...
%                              (mark (setq news (gtheap (pairpack)))))
                      ...
                      ((floatnum-tag)
                        (progn (setf ptr (inf s))
                                ...
                                (setf (wgetv news 2) (wgetv ptr 2))
                                (return
                                 (putmem   p (mkitem floatnum-tag news)))))
                      ((vector-tag evector-tag)
                       (progn (setq strips (vecinf s))
                              (setq len (veclen strips))
                              (setf (getmem strips)
                              ...
                              (for (from i 0 len 1)
                               (do
                                (progn (setf (vecitm ptr i)
                                        (vecitm strips i))
                                       (copyitem (loc (vecitm ptr i))))))
                              (return (putmem  p (mkitem tg ptr)))))
                      ((words-tag fixnum-tag)
                       (progn (setf (getmem (inf s))
                               (mark (setq news (copywrds s))))
                              (return (putmem   p (mkitem tg news)))))
                      ((bignum-tag)
                       (progn (setf (getmem (inf s))
                               (mark (setq news (copyvector s))))
                              (return (putmem   p (mkitem tg news)))))
                      ((halfwords-tag)
                       (progn (setf (getmem (inf s))
                                   (mark (setq news (copyhalfwords s))))
                            (return (putmem  p (mkitem halfwords-tag news)))))
                      (nil
                       (fatalerror
                        (bldmsg
                       "Unexpected tag %w found at %w during garbage collection"
