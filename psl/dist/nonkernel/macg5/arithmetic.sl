         ...
         (ds deffloatentry (name prim) 
             (de name (x y)
               (prog (f)
                     (setq f (gtfltn))
		     (setq *bruch* 0)
                     (prim (floatbase (fltinf f)) (floatbase (fltinf x))
                                     (floatbase (fltinf y)))
...
(de floatgreaterp (x y)
 (not (eq 128 (*fgreaterp (floatbase (fltinf x)) (floatbase (fltinf y))))))
  % no NIL from C
...
(de floatlessp (x y)
 (not (eq 128 (*flessp (floatbase (fltinf x)) (floatbase (fltinf y))))))
  % no NIL from C
