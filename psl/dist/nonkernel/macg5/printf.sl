...
(lap '((*entry printf1 expr 15)
       (*alloc 60)
        (*move (reg 2) (indexed (reg st) 256))
        (*move (reg 3) (indexed (reg st) 248))
        (*move (reg 4) (indexed (reg st) 240)) 
        (*move (reg 5) (indexed (reg st) 232))
        (*move (reg 6) (indexed (reg st) 224))
        (*move (reg 7) (indexed (reg st) 216))
        (*move (reg 8) (indexed (reg st) 208))
        (*move (reg 9) (indexed (reg st) 200))
        (*move (reg 10) (indexed (reg st) 192))
        (*move (reg 11) (indexed (reg st) 184))
        (*move (reg 12) (indexed (reg st) 176))
        (*move (reg 13) (indexed (reg st) 168))
        (*move (reg 14) (indexed (reg st) 160))
        (*move (reg 15) (indexed (reg st) 152))
       (*wplus3 (reg 1) (reg st) 256)
       %  scs
       (*call printf2)
       (*exit 60)))
...
(compiletime (put 'memo 'opencode '((ld (reg 1) (displacement (reg 1) 0)))))
...
(de printf2 (printfargs)
  ...
  (prog (uplim i ch upch)
        ...
        (setq i 0)
%(console-print-string "formatforprintf ")
%(console-print-number printfargs ) (console-newline)
        (while (wleq i uplim)
          (setq ch (strbyt (strinf formatforprintf*) i))
%(console-print-string "Strbyt ") (console-print-number ch) (console-newline)
          (if (wneq ch (char !%))
            (writechar ch)
            (prog nil
                  (setq i (wplus2 i 1))
...
(progn
%(console-print-string "tostringwritechar ")
%(console-print-number channel)
%(console-newline)
%(console-print-number ch)
%(console-newline)
  % shares TokenBuffer                                                     
   (if (wgeq (wgetv tokenbuffer 0) (wdifference maxtokensize 1))
           (progn (setf (wgetv tokenbuffer 0) 80)
(console-print-string "Error tostringwritechar ")
(console-print-number (wgetv tokenbuffer 0))
(console-newline)
(console-print-number (wdifference maxtokensize 1))
(console-newline)
...
%(console-print-string format) (console-newline)
%(console-print-number arg1) (console-newline)
%(console-print-number arg2) (console-newline)
...
(de errprin (u)
  %. `Prin1 with quotes'                                                   
%(console-print-string u) (console-newline)
%(console-print-number u) (console-newline)
