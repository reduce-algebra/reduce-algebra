...
% Runtime:      
%
% (c) Copyright 1983, 1984, Hewlett-Packard Company, all rights reserved.
...
% Revisions:
%
% 2-Jan-90  (Winfried Neun)
%  Commentoutcode of delbps because its wrong and its in the kernel for SUN4.
% 21-Jul-89 (Winfried Neun)
%  Modified get-heap-try-reclaim for dynamic heap expansion.
...
(fluid '(heap-warn-level heapenlarge* using-other-bps-spaces*))
...
(de get-heap-trap (pointer number-of-items)
  ...
  (if (null heaptrapped)
    ...
    (if (wlessp heaplast heaptrapbound) %%%%%%%heapupperbound)
      pointer
...
(de get-heap-try-reclaim (number-of-items)
  ...
  (when (wgreaterp number-of-items heapenlarge*)
        (setq heapenlarge*    (wplus2 number-of-items 1000)))
  (!%reclaim)
  (setq heap-warn-level oldheap-warn-level)
  (setq heapenlarge*    oldheapenlarge*)
  (let ((result heaplast))
...
(commentoutcode de delbps (bottom top)
  (when (and (weq nextbps top)
	     (wgreaterp bottom heaplast))
    (setq nextbps bottom)))
