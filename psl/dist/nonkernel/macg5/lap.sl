...
(de codeapply (CodePtr ArgList)
   (if (codep CodePtr) (ccodeapply (inf CodePtr) ArgList)
...
(*entry fastapply expr 0)
       ...
       (*move (reg t6) (reg t3))
       (*wshift (reg t6) 3) % multiply by addressingunitsperfunctioncell
       (*wplus2  (reg t6) (fluid symfnc))
       (ld (reg t1) (displacement (reg t6) 0))
...
%
	...
	(*field (reg t1) (reg t1) 8 56)       % (reg t5) <- id number
	(*move (reg t1) (memory (reg bndstkptr) (wconst -8)))
	(*wshift (reg t1) 3)
