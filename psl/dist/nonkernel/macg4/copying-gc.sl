...
(fluid '(*gc gcknt* gctime* heap-warn-level symget *bruch*))
...
(compiletime
 (progn
  (ds !%chipmunk-kludge (x) (inf x))   %%%  not TEMPORARY
  ...
  (ds markid (x)
    (setf (field (getmem (wplus2 symnam (wtimes2 4 x)))
              tagstartingbit tagbitlength)
      forward-tag))
  ...
  (flag '(copyfromallbases copyfromrange copyfrombase copyfromstack
      copyitem copyitem1 copyitem11 copypairitem markandcopyfromid
      makeidfreelist moreheap centerdump centerdump1 cfserror gcstats)
    'internalfunction)
  ))
...
(de Reclaim2 nil
  (prog (oldhsize newsize staticsize)
    ...
    (cond ((equal (land HeapLast 7) 4)
        (setq HeapLast (plus HeapLast 4))))
...
(lap '((*entry !%reclaim expr 0)
	(*alloc 12)  % to flush the frame registers
        (*call !%reclaim1)
	(*exit 12)))
...
(de !%reclaim1 ()
 (prog (b oldtimehi oldtimelo enlargement-tried)
  ...
  (when *gc (errorprintf "*** Garbage collection starting"))

 another-one
  (setq *bruch* 17) % ignore cntrl c while in gc
  (beforegcsystemhook)
% (setq stacklast
...
%     (adjuststackpointer (current-stack-pointer) (wminus (framesize)))))
  ...
  (setq *bruch* 0)  % allow cntrl c again
...
(de moreheap (incr)
    (setq incr (times2 incr addressingunitsperitem))
    (when (wgreaterp (alterheapsize incr) 0)
      (setq heaptrapbound  (wplus2 heapupperbound -120 )) % update register
))
...
(de markandcopyfromid (x)
  ...
  (progn (copyitem (loc (symnam x)))
	 ...
	 (when (not (memq (int2id x) '(heaplowerbound oldheaplowerbound
...
%
% we have to use different access macros for the stack
%
(compiletime (put 'memo 'opencode '((lwz (reg 1) (displacement (reg 1) 0)))))

(compiletime (put 'putstack 'opencode '((stw (reg 2) (displacement (reg 1) 0)))))

(de copyfromstack nil
   (prog (tg x info tg2 tg3 NullSize Size stklow stklast)
     (setq NullSize (Inf -1 ))    % The size of a null vector.
     (setq stklow (wshift stacklowerbound 5))
     (setq stklast (current-stack-pointer))
     (for (from i stklow stklast
    ...
    (if (weq tg id-tag)
      ...
      (if (and (withinoldheappointer info) (pointertagp tg))
        ...
        (if (weq tg2 forward-tag)
          (putstack i (mkitem tg (inf (getmem info))))
          (setq Size (Inf (getmem info)))
          (case tg
            ((fixnum-tag floatnum-tag words-tag)
             (if (and (eq tg2 hwords-tag) (or (eq Size NullSize)
                      (wleq (+ info (* Size addressingunitsperitem))
                        oldheaplast)))
               (copystackitem i)
               (cfserror "words/fixnum/float" x i)
               ))
            ((string-tag bytes-tag)
             (if (and (eq tg2 hbytes-tag)
                  (or (eq Size NullSize)
                      (wleq (+ info (wshift Size -2)) oldheaplast)))
                                 % Size is in Bytes here!!
               (copystackitem i)
%              (cfserror "string" x i)
               ))
            ((halfwords-tag)
             (if (and (eq tg2 hhalfwords-tag)
                  (or (eq Size NullSize) (wleq (+ info (* Size 2))
                        oldheaplast)))
               (copystackitem i)
               (cfserror "halfword" x i)
               ))
            ((bignum-tag)
                (if (and (or (eq tg2 hwords-tag) (eq tg2 hvect-tag))
                  (or (eq Size NullSize)
                      (wleq (+ info (* Size addressingunitsperitem))
                        oldheaplast)))
               (copystackitem i)
	% may be a returnaddr, that's legal
               (when (wgreaterp info nextbps) (cfserror "bignum" x i))))
             ...
             (if (and (eq tg2 hvect-tag)
                  (or (eq Size NullSize)
                      (wleq (+ info (* Size addressingunitsperitem))
                        oldheaplast)))
               (copystackitem i)
               (cfserror "(e)vector" x i)
               ))
            ((pair-tag)
             (if (itemtagp tg2)
               (progn
                   (setq tg3 (tag
                      (getmem (wplus2 info addressingunitsperitem))))
                   (when (itemtagp tg3) (copystackitem i)))
               (cfserror "pair" x i)))
            (nil (cfserror "nil" x i)))
          ))))))))
 
(de cfserror (where stackitem pos)
   (prog (info hinfo htag outputbase*)
         (setq outputbase* 16)
         (setq info (inf stackitem))
         (setq hinfo (getmem info))
         (setq htag (tag hinfo))
         (when *gc (centerdump (inf stackitem) 10))
         (when *gc (centerdump1 pos 20))
         (when *gc
         (errorprintf
          "CopyFromStack error in %w, stackitem %w heaptag %w heapinf %w%n"
                 where (inf stackitem) htag (inf hinfo))))) 
...
(de centerdump (loc howmuch)
  ...
  (for (from  i (minus howmuch) howmuch 8)
    (do (unixputn (wplus2 loc (wtimes2 addressingunitsperitem i)))
        (console-print-string ": ")
        (for (from j 0 7)
          (do (unixputn (getmem (wplus2 loc (wtimes2 addressingunitsperitem 
							(wplus2 i j)))))
              (console-print-string " ")))
        (console-newline))))

(de centerdump1 (loc howmuch)
  (console-newline)
  (console-print-string "**** Center dump at 16#")
  (unixputn loc)
  (console-newline)
  ...
  (for (from  i (minus howmuch) howmuch 8)
    (do (unixputn (wplus2 loc (wtimes2 addressingunitsperitem i)))
        (console-print-string ": ")
        (for (from j 0 7)
          (do (unixputn (memo (wplus2 loc (wtimes2 addressingunitsperitem
                                                        (wplus2 i j)))))
              (console-print-string " ")))
        (console-newline))))
...
(de copyitem (p)
  ...
  (prog (typ info hdr x)
        (when (null (setq x (getmem p))) (return nil))
        (setq typ (tag x))
        (unless (pointertagp typ)
         (if (wneq typ id-tag) (return nil)
               (setq info (idinf x))
               (if (markedid info) (return nil)
				   (return (markandcopyfromid info)))))
...
(de copypairitem (p x)
  ...
  % be meaningful and should be ignored!
  (prog (typ info hdr )
        (when (null x) (return nil))
        (setq typ (tag x))
        (unless (pointertagp typ)
         (if (wneq typ id-tag) (return nil)
               (setq info (idinf x))
               (unless (markedid info) (return (markandcopyfromid info)))))
  ...
  (prog (news cdrs)
        (if (eq (tag s) pair-tag)
               (progn 
                    ...
                    (setf (wgetv news 0) memos)
                    (setf (wgetv news 1) (setq cdrs (cdr s)))
                    (copypairitem (loc (wgetv news 0)) memos)
                    (return (copypairitem (loc (wgetv news 1)) cdrs)))
         (return (copyitem11 p s memos)))))

(de copyitem11 (p s memos)
  % Copier for GC
...
%                              (mark (setq news (gtheap (pairpack)))))
			       (mark (prog1 (setq news heaplast)
                      ...
                      ((string-tag)
                        (when (eq (tag (getmem (strinf s))) hbytes-tag)
                              (setf (getmem (strinf s))
                               (mark (setq news (copystring s))))
                              (return (setf (getmem p) news))))
	              ((floatnum-tag)
       		        (progn (setf ptr (inf s))
                		(setf (getmem ptr) (mark (setq news (gtfltn))))
...
(de copystackitem (p)
  ...
  % be meaningful and should be ignored!
  (prog (typ info hdr x)
        (when (null (setq x (memo p))) (return nil))
        (setq typ (tag x))
        (unless (pointertagp typ)
         (if (wneq typ id-tag) (return nil)
               (setq info (idinf x))
        ...
        (setq hdr (getmem info))
        (when (marked hdr)
          (return (putstack p (mkitem typ (inf hdr)))))
        (return (copystackitem1 p x hdr))))

(de copystackitem1 (p s memos)
  % Copier for GC
  % COPYITEM1 is executed for SIDE-EFFECT--its return value is not likely to
  % be meaningful and should be ignored!
  (prog (news len ptr strips tg cdrs)
...
%                              (mark (setq news (gtheap (pairpack)))))
                      ...
                      ((floatnum-tag)
                        (progn (setf ptr (inf s))
                                ...
                                (setf (wgetv news 2) (wgetv ptr 2))
                                (return
                                 (putstack p (mkitem floatnum-tag news)))))
                      ((vector-tag evector-tag)
                       (progn (setq strips (vecinf s))
                              (setq len (veclen strips))
                              (setf (getmem strips)
                              ...
                              (for (from i 0 len 1)
                               (do
                                (progn (setf (vecitm ptr i)
                                        (vecitm strips i))
                                       (copyitem (loc (vecitm ptr i))))))
                              (return (putstack p (mkitem tg ptr)))))
                      ((words-tag fixnum-tag)
                       (progn (setf (getmem (inf s))
                               (mark (setq news (copywrds s))))
                              (return (putstack p (mkitem tg news)))))
                      ((bignum-tag)
                       (progn (setf (getmem (inf s))
                               (mark (setq news (copyvector s))))
                              (return (putstack p (mkitem tg news)))))
                      ((halfwords-tag)
                       (progn (setf (getmem (inf s))
                                   (mark (setq news (copyhalfwords s))))
                            (return (putstack p (mkitem halfwords-tag news)))))
                      (nil
                       (fatalerror
                        (bldmsg
                       "Unexpected tag %w found at %w during garbage collection"
...
(de makeidfreelist ()
  (prog (previous)
        (for (from i 0 255 1) (do (clearidmark i)))
        (setq previous 256)
...
(de gcstats ()
  (if (wgeq (known-free-space) 100)
      (if (wlessp gctime* 10000)
       (Errorprintf "*** GC %w: %w (~ %w ms cpu time, gc : %w %%)"
                      gcknt* (date-and-time) (sys2int(timc))
                             (wquotient (wtimes2 100 gctime*) (timc)))
       (Errorprintf "*** GC %w: %w (~ %w ms cpu time, gc : %w %%)"
                      gcknt* (date-and-time) (sys2int(timc))
                             (wquotient gctime* (wquotient (timc) 100))))
...
(de set!-heap!-size (items) 
   (prog (actual heap-warn-level heapenlarge*) 
       (setq actual (wquotient (wdifference heapupperbound heaplowerbound)
				addressingunitsperitem)) 
       (when (null items) (return actual))
       (when (greaterp actual items) 
          (print "Cannot shrink heap") (return nil)) 
       (setq heapenlarge* (wdifference items actual)) 
