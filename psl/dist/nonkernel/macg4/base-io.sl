...
(compiletime (tr pa1m-neq))
(compiletime (tr pa1r-equal))
(compiletime (tr eval-eq-able?))
...
% Revisions:
%
% 18-Feb-89 (Winfried Neun)
%  Changed add1 to iadd1 in independentwritechar (!)
%  Changed indpendentwritechar and independentreadchar to be alloc 0 for
%  most cases which is valueable e.g. for Sparc
...
(compiletime 
 (ds opentestlegalchannelandreturniferror (channel)

  % Cause an error if CHANNEL is not legal.
  
  (when (wneq channel (wand channel maxchannels))  % impure but fast!
        (return (inputchannelerror channel ))))    % expects maxchannels=2**n -1
)
(compiletime (flag '(inputchannelerror independentreadchar-hardcase) 
                   'internalfunction))

(de inputchannelerror (channel)
                (ioerror (list channel " is not a legal channel ")))
% 
% the former function is now the hardcase
%

(de independentreadchar-hardcase (channel)
  ...
  (opentestlegalchannelandreturniferror channel)
...
(de independentreadchar (channel)
 ...
 (prog (nextpo nextpochan)
  (setq nextpo nextposition)
  (setq nextpochan (wgetv nextpo channel))
  (opentestlegalchannelandreturniferror channel)

  % If the input buffer is empty or *echo , go to hardcase
  
  (return
  (cond ((or *echo (wgreaterp nextpochan
                              (wgetv bufferlength channel)))
                       (independentreadchar-hardcase  channel))

  % Else Pull the next character out of the buffer.
  
       (t  (setf (wgetv nextpo channel) (+ nextpochan 1))
           (strbyt (strinf (igetv iobuffer channel)) nextpochan))
 ))))
...
(de independentwritechar (channel chr)
 ...
 (prog (nextpo nextpochan)
  ...
  (setq nextpochan (wgetv nextpo channel))
  (opentestlegalchannelandreturniferror channel)
  (setf (wgetv nextpo channel) (setq nextpochan (+ nextpochan 1)))
  (setf (strbyt (strinf (igetv iobuffer channel)) nextpochan) chr)
  (return
  (when (or (equal chr (char eol))
            (>= (iadd1 nextpochan)                    % Indexed from 0
...
(de clearonechannel (chn filedes how fn)
  ...
  (let ((bufflen  (sysmaxbuffer filedes)))
    ...
    (setf (wgetv closefunction chn)  'illegalstandardclosechannel)
