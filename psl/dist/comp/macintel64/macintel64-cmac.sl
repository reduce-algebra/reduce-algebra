...
% (and the assembler doesnt do it) RIP relative addresses have to be used.
...
(de FakeRegP (Expression)
      (or (and (eqcar expression 'extrareg) (FakeRegP (cadr expression)))
          (and (PosIntP  Expression)
  ...
  (let (resultingcode* r)
    (CMacroPatternExpand
	 ...
	 (if (and 
	    ...
	    (ResolveOperand '(REG t2) Destination))
     (if *writingasmfile
      (get '*Moveasm 'CMacroPatternTable)
      (get '*Move 'CMacroPatternTable)
 ))))
...
(de any_but_not_fluid-p (r) % ha ha
     (not (or (Fakeregp r)
        (and (pairp r) (member (car r)  '(!$fluid !$global fluid global))))))

(de macintel64-absolute-p (r)
          (or (eqcar r 'quote) (stringp r)))


(DefCMacro *Moveasm                       %  (*Move Source Destination)
...
%%		      (dec ArgTwo))
   ((macintel64-absolute-p regp)
		      (mov ArgOne "@GOTPCREL(%rip)" ArgTwo)
                      (mov  (displacement ArgTwo 0) ArgTwo))
   ((regp macintel64-fluid-p)
		      (mov "_symval@GOTPCREL(%rip)" (reg t3))
		      (*move ArgOne
			 (displacement (reg t3) ArgTwo)))
   ((macintel64-fluid-p regp)
		      (mov "_symval@GOTPCREL(%rip)" ArgTwo)
		      (*move (displacement ArgTwo ArgOne) ArgTwo))
...
%%                    (dec ArgTwo))
   ...
   ((regp       anyp) (mov argone argtwo))
%   ((quotep     anyp) (movq argone argtwo))
   ((inump    any_but_not_fluid-p) (movq argone argtwo))
   (                  (*move argone (reg t3))
                      (*move (reg t3) argtwo))
)
...
%% ((Anyp Minus1p)      (dec ArgOne))
 ...
 ((any_but_not_fluid-p Inump)        (addq ArgTwo ArgOne))
 (                    (*MOVE  ARGTWO (Reg t3))
		      (add (Reg t3) ARGONE))
)
...
%%	   ((Anyp Minus1p)    (inc ArgOne))
	   ...
	   ((any_but_not_fluid-p Inump)  (subq ArgTwo Argone)) 
	   (                  (*MOVE  ARGTWO (Reg t3))
			      (sub (Reg t3) ARGONE))
)
