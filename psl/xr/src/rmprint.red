...
symbolic procedure fmp!-switch mode;        
      if mode then
	<<if not outputhandler!* = 'fancy!-output then 
	  <<outputhandler!-stack!* := 
...
symbolic procedure fancy!-prin2!*(u,n);
  ...
  (begin scalar str,id; integer l;
	...
	(if id and !*fancy!-lower 
          then fancy!-char!-downcase x else x).fancy!-line!*;
    >>;
...
symbolic procedure fancy!-char!-downcase u;
   begin scalar x;
      if (x := atsoc(u,
...
symbolic procedure fancy!-lower!-digits1(u,s);
   if null u then (if s then '(!}) else nil) else
   if (digit car u and s) or (not digit car u and not s)
     then car u . fancy!-lower!-digits1(cdr u,s) else
   if digit car u then
      if cdr u and digit cadr u then
	'!_ . '!{ . car u . fancy!-lower!-digits1(cdr u,t) else
	'!_ . car u . fancy!-lower!-digits1(cdr u,nil)
    else
	'!} . car u . fancy!-lower!-digits1(cdr u,nil);
...
symbolic procedure fancy!-in!-brackets(u,l,r);
  ...
  fancy!-level
   (begin scalar pos,fl,fp,w,r1,r2,rec; 
     rec := {0};
...
% Prints expression in an exponent notation.
   (begin scalar !*list,x,pp,q,w,w1,w2,pos,fl;
      pos:=fancy!-pos!*; fl:=fancy!-line!*;
      pp := not((q:=get('expt,'infix))>p);  % Need to parenthesize
      w1 := cadr l; w2 := caddr l;
      testing!-width!* := t;
      if eqcar(w2,'quotient) and cadr w2 = 1 then
...
symbolic procedure fancy!-oprin op;
 fancy!-level
  begin scalar pos,fl,x,first;
    if (x:=get(op,'fancy!-prtch)) then fancy!-prin2!*(x,1)
      else
    if (x:=get(op,'fancy!-infix!-symbol)) then fancy!-special!-symbol(x,4)
      else
...
symbolic procedure fancy!-condpri(u,p);
   <<if p>0 then fancy!-prin2 "\bigl(";
     while (u := cdr u) do
	<<if not(caar u eq 't)
	    then <<fancy!-prin2 'if; fancy!-prin2 " "; fancy!-maprin0 caar u;
		   fancy!-prin2 "\,"; fancy!-prin2 'then; fancy!-prin2 "\,">>;
...
symbolic procedure fancy!-quotpri u;
   begin scalar n1,n2,p,fl,w,pos,testing!-width!*;
     if overflowed!* then return 'failed;
     testing!-width!*:=t;
     pos:=fancy!-pos!*;
     fl:=fancy!-line!*;
...
symbolic procedure fancy!-prefix!-operator(u);
 % Print as function, but with a special character.  
   begin scalar sy;
     sy := get(u,'fancy!-functionsymbol) or get(u,'fancy!-special!-symbol);
     if sy then fancy!-special!-symbol(sy,2) else
     fancy!-prin2!*(u,t);
   end;
...
symbolic procedure fancy!-sqrtpri!*(u,n);
  fancy!-level
   begin 
     if not numberp n or liter n then return 'failed;
     fancy!-prin2!*("\sqrt",0);
     if n neq 2 then
     <<fancy!-prin2!*("[",0);
       fancy!-prin2!*("\,",1);
...
symbolic procedure fancy!-sumpri!*(u,p,mode);
  if p>get('minus,'infix) then 
    fancy!-in!-brackets({'fancy!-sumpri!*,mkquote u,0,mode},'!(,'!))
   else
  fancy!-level
   begin scalar w,w0,w1,w2,lo,hi,var;
     var := caddr u;
...
put('dfp,'fancy!-pprifn,'fancy!-partialdfpri);
...
symbolic procedure fancy!-dfpri0(u,l,symb);
 ...
 fancy!-level
  begin scalar x,d,z,q; integer n,m;
    u:=cdr u;
    q:=car u;
    u:=cdr u;
    while u do
