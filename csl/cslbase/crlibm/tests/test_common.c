...
int rand_int(){
  int val;
  int i;
/* On some systems the low bits of rand are less reliable then the high
   ones, so I mix in some high order bits here...
 */
  val = ((rand() % (65536 + 0x135)) & 0x000000ff);
  for(i=0; i<(sizeof(int)); i++){
    val = val << 8;
    val += ((rand() % (65536 + 0x135)) & 0x000000ff ); /* we keep only 8 bits */
  }
  return val;
}
