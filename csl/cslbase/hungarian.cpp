...
// Adjusted by Arthur Norman as part of making a sparse variant.
// Reformatted using astyle for a consistent indentation policy that
// can match much of the rest of my code.
...
void hungarian_print_boolean_matrix(char** C, int rows, int cols)
{   int i,j;
    fprintf(stderr , "\n");
    for(i=0; i<rows; i++)
    {   fprintf(stderr, " [");
        for(j=0; j<cols; j++)
            fprintf(stderr, "%2d ",C[i][j]);
        fprintf(stderr, "]\n");
    }
    fprintf(stderr, "\n");
}
...
{   hungarian_print_boolean_matrix(p->assignment, p->num_rows, p->num_cols) ;
}
...
// I initialise the cost matrix here with large values in all the parts that
// correspond to the original problem, but zeros where I have expanded the
// matrix to make it square.
        for(j=0; j<p->num_cols; j++)
        {   p->cost[i][j] =  (i < org_rows && j < org_cols) ? 0x01000000 : 0;
            p->assignment[i][j] = 0;
        }
    }
    return rows;
...
void hungarian_edge(hungarian_problem_t* p, int i, int j, int cost)
{   p->cost[i][j] = cost;
}
