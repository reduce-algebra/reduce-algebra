...
{   // split values into 4 32-bit parts
    ...
    uint64_t third32  = (products[0][2] & 0xffffffff) +
                        (products[0][3] >> 32);
    uint64_t second32 = (products[0][1] & 0xffffffff) +
                        (products[0][2] >> 32);
    uint64_t first32  = (products[0][0] & 0xffffffff) +
                        (products[0][1] >> 32);
...
std::pair <uint128_t, uint128_t> uint128_t::divmod(
...
std::uint8_t uint128_t::bits() const
{   std::uint8_t out = 0;
    if (UPPER)
    {   out = 64;
        uint64_t up = UPPER;
...
std::string uint128_t::str(std::uint8_t base,
