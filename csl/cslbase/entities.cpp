...
%   ("cups"                                      8746, 65024)
    ...
    ("eparsl"                                    10723)
    ("eplus"                                     10865)
ML standard seems to allow epsi as a synonym for epsilon. This
 me pain with my input-level conversion in that with that in place
nter epsilonALT-x I get an epsilon symbol. If I then press ALT-x
that decodes back to just epsi which is rendered as e followed by
symbol. ALt-x again maps that to e03e8, then 03bf and finally back to
psi - and the "lon" has been lost forever. I am swapping the order
 two definitions so that "epsilon" now comes before "epsi" to see
t makes me happier. That will still be a bit odd even if it works
y I hope in that if one enters epsi and repeatedlu hits ALT-x
 will eventually say epsilon.
    ("Epsilon"                                   917)
    ("epsilon"                                   949)
    ("epsi"                                      949)
    ("epsiv"                                     1013)
