...
// Note that object property keys are explicit strings to try to stop the Google Closure Compiler from mangling them!
    ...
    system: function (command) {
        ...
        try {
            ...
            if (commandString.startsWith("gnuplot")) {
                ...
                const files = matches && matches.map(match => {
                ...
                const filedata = files?.map(file => { return file.data.buffer; });
                // Transfer file contents efficiently rather than copying them:
                postMessage({ "channel": "plot", "script": cmds, "files": files }, { "transfer": filedata });
                return 0; // success
            }
        }
        catch (error) {
