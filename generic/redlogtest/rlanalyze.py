...
def parse_args():
    ...
    parser.add_argument(
        "--format",
        choices=["txt", "html", "html_table", "csv", "json", "py"],
        default="txt",
        help=(
            "output text (default), a full html document, an html table, "
            "comma-separated values, json, or a python list of dictionaries"),
    )
    parser.add_argument(
        "--select",
        choices=["cpu", "gc"],
        help="include only cpu times or only gc times",
    )
    parser.add_argument(
        "--sort",
        choices=["alpha", "speed"],
