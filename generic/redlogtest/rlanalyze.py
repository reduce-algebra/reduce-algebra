...
class Benchmark(dict):
    ...
    def update_ref_data(self, regressions: str):
        for lisp in "csl", "psl":
            with open(
                    os.path.join(regressions, self["name"]) + "." + lisp +
                    "time", "r") as file:
        ...
        cpu_ref_mean = statistics.mean(
            [self["cpu_ref_csl"], self["cpu_ref_psl"]])
    ...
    def update_now_data(self, timings: str):
        for lisp in "csl", "psl":
            with open(
                    os.path.join(timings, lisp + "-times",
                                 self["name"] + ".time"), "r") as file:
                for line in file.readlines():
                    if "Time" in line:
                        line = line.split()
        ...
        cpu_now_mean = statistics.mean(
            [self["cpu_now_csl"], self["cpu_now_psl"]])
    ...
    def update_delta_data(self):
        ...
        self.update({
            "cpu_delta_csl": cpu_delta_csl,
            "cpu_delta_psl": cpu_delta_psl,
            "cpu_delta_mean": cpu_delta_mean,
            "gc_delta_csl": gc_delta_csl,
            "gc_delta_psl": gc_delta_psl,
            "gc_delta_mean": gc_delta_mean,
        })
        return self
    ...
    def update_ratio_data(self):
        ...
        self.update({
            "cpu_ratio_csl": cpu_ratio_csl,
            "cpu_ratio_psl": cpu_ratio_psl,
            "cpu_ratio_mean": cpu_ratio_mean,
            "gc_ratio_csl": gc_ratio_csl,
            "gc_ratio_psl": gc_ratio_psl,
            "gc_ratio_mean": gc_ratio_mean,
        })
        return self
...
class Benchmarks(list):
    ...
    def from_file_system(self, regressions: str, timings: str):
        for path, directories, files in os.walk(regressions):
            for file in files:
                ...
                benchmark.update({
                    "name":
                    os.path.join(os.path.relpath(path, regressions), basename)
                })
    ...
    def summarize(self) -> Benchmark:
        ...
        for column in (
            ...
            summary.update(
                {column: sum(map(lambda benchmark: benchmark[column], self))})
    ...
    def select(self, columns: list):
        return Benchmarks(
            list(map(lambda benchmark: benchmark.select(columns), self)))
    ...
    def html_view(self, summary: Benchmark, *, full: bool = False) -> str:
        ...
        if len(self[0]) == 13:
            ...
            for section in "CSL", "PSL", "Mean":
                html_doc += ('<th style="text-align:center;" colspan="4">' +
                             section + "</th>")
        elif len(self[0]) == 25:
            html_doc += '<th style="text-align:left;">Benchmark</th>'
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="generate a digest of Redlog benchmarking")
    parser.add_argument(
        "regressions",
        help="the regressions directory (typically inside packages/redlog)",
    )
    parser.add_argument(
        "timings",
        help="the timing directory (which contains csl-times, psl-times)")
    parser.add_argument(
        "--columns",
        choices=["all", "gc"],
        help="select GC data or full (CPU + GC) data; default is CPU data",
    )
    parser.add_argument(
        "--format",
        choices=["html", "html_table", "csv", "json", "py"],
