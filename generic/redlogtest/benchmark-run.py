...
def parse_args():
    ...
    parser.add_argument(
        ...
        metavar="SOURCE",
        help=(
            "An existing benchmark directory. *.red files are used as input. If present, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Overwrite existing directories specified via '--svn-reduce DIR' and 'RESULT' "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Pass --dry-run on to to generated GNU parallel command. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Report benchmarks found: +, - indicate inclusion, exclusion, "
    ...
    parser.add_argument(
        "--bar", action="store_true", help="Show progress as a progress bar"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Exclude benchmarks with a cpu time record (cpu_csl.txt, cpu_psl.txt) larger than "
            "SECONDS in SOURCE"
        ),
    )
    reduce_group = parser.add_mutually_exclusive_group(required=True)
    reduce_group.add_argument(
        "--reduce", metavar="DIR", type=str, help="Use existing Reduce directory DIR"
    )
    reduce_group.add_argument(
        "--svn-reduce",
        metavar="DIR",
...
def setup_reduce(reduce: str, svn_reduce: str, revision: str, force: bool) -> str:
    def dump_and_exit(file_name: str, completed_process):
        ...
        sys.stderr.write(
            "error: dumping command output to "
            + file_name
            + " and exiting"
            + os.linesep
        )
    ...
    def configure(reduce: str, lisp: str = "both"):
        ...
        completed_process = subprocess.run(
            cmd, cwd=reduce, capture_output=True)
        if completed_process.returncode != 0:
    ...
    def compile(reduce: str):
        ...
        completed_process = subprocess.run(
            ["make"], cwd=reduce, capture_output=True)
        if completed_process.returncode != 0:
...
def init_result_dir(
    ...
    if os.path.exists(result):
        if force:
            ...
            sys.stderr.write(
                "error: delete existing " + result + " or use -f, --force" + os.linesep
            )
            exit(255)
    os.makedirs(os.path.join(result, "GLOBAL"))
    for lisp in "csl", "psl":
        with open(
            os.path.join(result, "GLOBAL", "revision_" + lisp + ".txt"), "w"
        ) as file:
            subprocess.run(
                [
                    "svn",
                    ...
                    "--show-item",
                    "revision",
                    os.path.join(reduce, "bin", "red" + lisp),
                ],
                stdout=file,
            )
    ...
    for path, directories, files in os.walk(source):
        ...
        for file in files:
                        ...
                        if os.path.exists(source_cpu_file):
                            with open(source_cpu_file, "r") as file:
                                if int(file.read().rstrip()) > 1000 * exclude_by_time:
                                    if verbose:
                                        sys.stderr.write(
                                            "- t " + rel_red_file + os.linesep
                                        )
                                    raise Continue
                        else:
                            sys.stderr.write(
                                "warning: ignoring exclude_by_time for "
                                + rel_red_file
                                + " - "
                                + cpu_file
                                + " does not exist"
            ...
            result_pslheap_file = os.path.join(
                result, relpath, basename + ".pslheap")
...
if __name__ == "__main__":
    args = parse_args()
    reduce = setup_reduce(args.reduce, args.svn_reduce,
                          args.revision, args.force)
    source = os.path.abspath(args.source)
    result = os.path.abspath(args.result)
    rel_red_files = init_result_dir(
        source,
        result,
        args.force,
        args.include,
        args.exclude,
        args.exclude_by_time,
        reduce,
        args.verbose,
    )
    with open(os.path.join(result, "GLOBAL", "parse_args.txt"), "w") as file:
        file.write(str(args))
    parallel_cmd = build_parallel_command(
        args.dry_run,
        args.bar,
        args.jobs,
        reduce,
