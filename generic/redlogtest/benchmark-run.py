...
def parse_args():
    ...
    parser.add_argument(
        "ref", metavar="SOURCE", help="a directory containing benchmark sources"
    )
    parser.add_argument(
        "now",
        metavar="RESULT",
    ...
    reduce_group.add_argument(
        "--reduce", metavar="DIR", type=str, help="use existing reduce DIR"
    )
    reduce_group.add_argument(
        "--svn-reduce",
        metavar="DIR",
...
def init_now_dir(
    ...
    if os.path.exists(now):
        if force:
            ...
            sys.stderr.write(
                "error: delete existing " + now + " or use -f, --force" + os.linesep
            )
            exit(1)
    os.makedirs(os.path.join(now, "GLOBAL"))
    for lisp in "csl", "psl":
        with open(
            os.path.join(now, "GLOBAL", "revision_" + lisp + ".txt"), "w"
        ) as file:
            subprocess.run(
                [
                    "svn",
    ...
    for path, directories, files in os.walk(ref):
        ...
        for file in files:
                ...
                try:
                    for lisp in "csl", "psl":
                        ...
                        if os.path.exists(ref_cpu_file):
                            ...
                            sys.stderr.write(
                                "warning: ignoring timelimit for "
                                + rel_red_file
                                + " - "
                                + cpu_file
                                + " does not exist"
            ...
            now_pslheap_file = os.path.join(
                now, relpath, basename + ".pslheap")
...
if __name__ == "__main__":
    ...
    rel_red_files = init_now_dir(
        ref, now, args.force, args.include, args.exclude, args.timelimit, args.verbose
    )
