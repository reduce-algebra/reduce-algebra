...
def _log(msg: str, *, cwd: str = os.getcwd()):
    print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " [" + cwd + "] " +
          msg)
...
def setup_parser(parser):
    ...
    reduce_group.add_argument("--reduce",
                              metavar="DIR",
                              type=str,
...
def setup_reduce(reduce: str, svn_reduce: str, revision: str,
                 force: bool) -> str:
    def dump_and_exit(file_name: str, completed_process):
        ...
        sys.stderr.write("error: dumping command output to " + file_name +
                         " and exiting" + os.linesep)
    ...
    def checkout(rev: str, reduce: str, force: bool):
        ...
        if os.path.exists(reduce):
            if force:
                ...
                sys.stderr.write("error: delete existing " + reduce +
                                 " or use -f, --force" + os.linesep)
                sys.exit(17)
    ...
    def compile(reduce: str):
        ...
        completed_process = subprocess.run(["make"],
                                           cwd=reduce,
                                           capture_output=True)
        if completed_process.returncode != 0:
                    ...
                    for lisp in "csl", "psl":
                        ...
                        if os.path.exists(source_cpu_file):
                            ...
                            sys.stderr.write(
                                ...
                                rel_red_file + " - " + cpu_file +
                                " does not exist" + os.linesep)
                            break
                except Continue:
                    continue
            if verbose:
...
def benchmark_run(args):
    reduce = setup_reduce(args.reduce, args.svn_reduce, args.revision,
                          args.force)
