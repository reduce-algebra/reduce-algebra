...
def _install_reduce(svn_reduce: str, revision: str, force: bool) -> str:
    def dump_and_exit(file_name: str, completed_process):
        ...
        logger.critical("dumping command output to " +
                        file_name + " and exiting")
    ...
    def checkout(rev: int, svn_reduce: str, force: bool):
        ...
        if os.path.exists(svn_reduce):
            if force:
                ...
                logger.critical(
                    "found existing " + svn_reduce + " - delete or use -f, --force"
                )
                sys.exit(17)
    ...
    def configure(svn_reduce: str, lisp: str = "both"):
        ...
        completed_process = subprocess.run(
            cmd, cwd=svn_reduce, capture_output=True)
        if completed_process.returncode != 0:
...
def _init_result(
    ...
    completed_process = subprocess.run(
        ["svnversion"], cwd=reduce, capture_output=True)
    ...
    for path, directories, files in os.walk(source):
        ...
        for file in files:
            ...
            if basename != os.path.basename(relpath):
                logger.warning(
                    "skipping " + red_file + " - basename does not match directory name"
                )
                continue
...
def _run_using_gnu_parallel(
    ...
    def _parallel_command() -> str:
        ...
        if bar:
            parallel_args += " --bar"
        parallel_cmd = " " + os.path.join(
            os.path.dirname(__file__), "bash_scripts", "job.sh"
        )
        parallel_cmd_args = " "
...
def cron(
    ...
    formatter = logging.Formatter(
        '<tr><td class="timestamp">%(asctime)s</td>'
...
def run(
    ...
    with open(os.path.join(result, "GLOBAL", "call.txt"), "w") as file:
        ...
        file.write(funcname + inspect.formatargvalues(*
                   inspect.getargvalues(myframe)))
