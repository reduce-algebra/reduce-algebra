...
def _install_reduce(svn_reduce: str, revision: str, force: bool) -> str:
    ...
    def dump_and_exit(file_name: str, completed_process):
        ...
        logger.critical("dumping command output to " + file_name +
                        " and exiting")
    ...
    def checkout(rev: int, svn_reduce: str, force: bool):
        ...
        if os.path.exists(svn_reduce):
            if force:
                ...
                logger.critical("found existing " + svn_reduce +
                                " - delete or use -f, --force")
                sys.exit(17)
    ...
    def configure(svn_reduce: str, lisp: str = "both"):
        ...
        completed_process = subprocess.run(
            "./scripts/findhost.sh $(./config.guess)",
            shell=True,
            cwd=svn_reduce,
            capture_output=True,
        )
...
def _init_result(
    ...
    if os.path.exists(result):
        if force:
            ...
            logger.critical("found existing " + result +
                            " - delete or use -f, --force")
            sys.exit(17)
    os.makedirs(os.path.join(result, "GLOBAL"))
    completed_process = subprocess.run(["svnversion"],
                                       cwd=reduce,
                                       capture_output=True)
    ...
    for path, directories, files in os.walk(source):
        ...
        for directory in directories:
            if directory == "SUPPORT":
                shutil.copytree(
                    ...
                    os.path.join(result, relpath, directory),
                    dirs_exist_ok=True,
                )
        ...
        for file in files:
            ...
            if basename != os.path.basename(relpath):
                logger.warning("skipping " + red_file +
                               " - basename does not match directory name")
                continue
                    ...
                    for lisp in "csl", "psl", "boot":
                        ...
                        if os.path.exists(source_cpu_file):
                            with open(source_cpu_file, "r") as file:
                                if int(file.read().rstrip()
                                       ) > 1000 * exclude_by_time:
                                    logger.debug("t -" + red_file)
                                    raise Continue
                        else:
                            logger.warning("ignoring exclude_by_time for " +
                                           red_file + " - " + cpu_file +
                                           " does not exist")
                            break
                except Continue:
            ...
            if sed_script is None:
                ...
                with open(source_red_file,
                          "r") as i, open(result_red_file, "w") as o:
                    completed_process = subprocess.run([sed_prog, sed_script],
                                                       stdin=i,
                                                       stdout=o)
                if completed_process.returncode != 0:
                    logger.critical("exit code " +
                                    str(completed_process.returncode) +
                                    " in " + sed_prog + " '" + sed_script +
                                    "'")
                    sys.exit(17)
            source_pslheap_file = os.path.join(path, basename + ".pslheap")
            result_pslheap_file = os.path.join(result, relpath,
                                               basename + ".pslheap")
...
def _run_using_gnu_parallel(
    ...
    def _parallel_command() -> str:
        ...
        # parallel_command
        parallel_cmd = [
            os.path.join(os.path.dirname(__file__), "bash_scripts", "job.sh")
        ]
        ...
        one = (lisp.replace("csl", "redcsl").replace("psl", "redpsl").replace(
            "boot", "bootstrapreduce"))
        two = str(psl_heapsize)
        three = " ".join(map(shlex.quote, red_files))
        parallel_cmd_args += [":::", one, ":::", two, ":::", three]
        # parallel_cmd
        parallel_cmd = " ".join(["parallel"] + parallel_args + parallel_cmd +
                                parallel_cmd_args)
        return parallel_cmd
...
def cron(
    ...
    force: bool = False,
    jobs: int = 1,
    psl_heapsize: int = 4000,
    log: str = "info",
    exclude: str = None,
    include: str = None,
    exclude_by_time: int = None,
    reduce: str = None,
    svn_reduce: str = None,
    revision: str = None,
):
    ...
    try:
        ...
        logger.removeHandler(handler)
    print(html.begin)
    analytics.summary(now=result, dump=True, full_html=False)
