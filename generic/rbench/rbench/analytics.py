...
class Benchmark(pd.DataFrame):
    ...
    def add_means(self):
        ...
        for key0 in self.columns.levels[0]:
            self[(key0, "cpu_mean")] = self[
                [(key0, "cpu_csl"), (key0, "cpu_psl")]
            ].mean(axis=1)
            self[(key0, "gc_mean")] = self[[(key0, "gc_csl"), (key0, "gc_psl")]].mean(
                axis=1
            )
            self[(key0, "valid_mean")] = self.apply(
                _valid_mean, args=(key0,), axis=1)
        return self
    ...
    def fast(self, seconds: float = 0.5):
        ...
        if len(level0) == 1:
            ...
            return self[
                (self[(level0[0], "cpu_mean")] <= seconds)
    ...
    def plot_scatter_csl_psl(self, **keywords):
        ...
        p = Benchmark(all_rows).plot_scatter(
            c="csl", colormap="bwr", sharex=False, include_bool=True, **keywords
        )
        return p
...
def read_filetree(root: str, key0: str = None):
    ...
    df.columns = pd.MultiIndex.from_tuples(
        [(key0 or attrs["revision"], c) for c in df.columns]
    )
    df.attrs = attrs
    return df
...
def summary(
    ...
    with open(summary_html, mode="w") as summary:
        ...
        ref_attrs = pd.DataFrame(
            ref.attrs.values(), index=ref.attrs.keys(), columns=["ref"]
        )
        now = read_filetree(now, "now").add_means()
        now_attrs = pd.DataFrame(
            now.attrs.values(), index=now.attrs.keys(), columns=["now"]
        )
        ...
        if not combo_bad.empty:
            ...
            summary.write(
                html.p(
                    'Benchmark problems with existing "now" logs that were tested '
        ...
        summary.write(
            html.p(
                'Plots are split into "fast" (average of the CSL and PSL CPU times &le; 0.5 s) '
        ...
        summary.write(
            html.p(
                "Benchmark problems with an average of CSL and PSL CPU times &le; 0.5 s. "
        ...
        summary.write(
            html.p(
                "Benchmark problems with an average of CSL and PSL CPU times &gt; 0.5 s. "
                "All times are in seconds."
            )
        )
