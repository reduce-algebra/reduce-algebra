...
def summary(ref: Benchmark,
    ...
    if not combo_bad.empty:
        ...
        print(
            html.
            p('Benchmark problems with existing "now" logs that were tested different from '
              'existing "ref" logs:'))
        print(html.p(combo_bad.to_html(show_dimensions=True)))
    print(html.h3("Scatter Plots"))
    print(
        html.
        p('Plots are split into "fast" (average of the CSL and PSL CPU times &le; 0.5 s) '
    ...
    print(
        html.
        p("Benchmark problems with an average of CSL and PSL CPU times &le; 0.5 s. "
          "All times are in seconds."))
    print(html.p(combo.fast().to_html(show_dimensions=True)))
    print(html.h3("Slow"))
    print(
        html.
        p("Benchmark problems with an average of CSL and PSL CPU times &gt; 0.5 s. "
          "All times are in seconds."))
    print(html.p(combo.slow().to_html(show_dimensions=True)))
    if full_html:
        print(html.end)
    if not dump:
        webbrowser.open(pathlib.Path(os.path.abspath(summary_html)).as_uri(),
                        new=1)
        sys.stdout.close()
