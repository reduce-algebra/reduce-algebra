...
def read_filetree(root: str, key0: str = None):
    ...
    rbdf.columns = pd.MultiIndex.from_tuples(
        [(key0 or attrs["revision"], c) for c in rbdf.columns]
    )
    rbdf.attrs = attrs
    return rbdf
...
def summary(
    ...
    with open(summary_html, mode="w") as summary:
        ...
        _ = combo.slow(60)
        combos.append(
            (_, f"{len(_.index):d} problems with at least 1 CPU time $\\geq$ 60 s")
        )
        # Global information
...
unchanged.
 """
        ...
        for (c, t) in combos:
            if not c.empty:
                ax = c.plot.scatter2(
                    x="ref", y="now", c1="cpu_csl", c2="cpu_psl", title=t
                )
...
"""
        ...
        for (c, t) in combos:
            if not c.empty:
                ax = c.plot.scatter2(
                    c1="ref", c2="now", x="cpu_csl", y="cpu_psl", title=t
                )
                summary.write(html.img(ax))
        summary.write("</div>")
        # Schedule
