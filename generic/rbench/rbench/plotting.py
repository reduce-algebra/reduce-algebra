...
def plot_scatter1(
    rbdf, *, x: str, y: str, c: str, color: str = None, colorx: str = None, **keywords
):
    l0 = rbdf.columns.levels[0]
    l1 = rbdf.columns.levels[1]
    fig, ax = plt.subplots()
    if x in l0 and y in l0 and c in l1:
        _scatter(rbdf, x=(x, c), y=(y, c), ax=ax,
                 color=color or "g", **keywords)
    else:
        raise ValueError("bad choice of x, y, c")
    if "cpu_" in c:
        cx = c.replace("cpu_", "sigxcpu_")
        x_only = rbdf.loc[~rbdf.index.isin(rbdf.dropna(subset=[(y, c)]).index)].dropna(
            subset=[(x, c)]
        )
        _scatter(x_only, x=(x, c), y=(y, cx), ax=ax,
                 color=colorx or "k", **keywords)
        y_only = rbdf.loc[~rbdf.index.isin(rbdf.dropna(subset=[(x, c)]).index)].dropna(
            subset=[(y, c)]
        )
        _scatter(y_only, x=(x, cx), y=(y, c), ax=ax,
                 color=colorx or "k", **keywords)
        neither = rbdf.loc[~rbdf.index.isin(
            rbdf.dropna(subset=[(y, c)]).index)]
        neither = neither.loc[
            ~neither.index.isin(neither.dropna(subset=[(x, c)]).index)
        ]
        _scatter(neither, x=(x, cx), y=(y, cx), ax=ax,
                 color=colorx or "k", **keywords)
    low_x, high_x = ax.get_xlim()
    low_y, high_y = ax.get_ylim()
    low = min(low_x, low_y)
...
def plot_scatter2(
    ...
    **keywords,
    ...
    if x in l0 and y in l0 and c1 in l1 and c2 in l1:
        plot_scatter(rbdf, x=(x, c1), y=(y, c1), ax=ax,
                     color=color1 or "r", **keywords)
        plot_scatter(rbdf, x=(x, c2), y=(y, c2), ax=ax,
                     color=color2 or "b", **keywords)
    elif x in l1 and y in l1 and c1 in l0 and c2 in l0:
        plot_scatter(rbdf, x=(c1, x), y=(c1, y), ax=ax,
                     color=color1 or "g", **keywords)
        plot_scatter(rbdf, x=(c2, x), y=(c2, y), ax=ax,
                     color=color2 or "m", **keywords)
    else:
...
def plot_schedule(
    ...
    **keywords,
    ...
    if key0 is None:
        ...
        if len(l0) > 1:
            raise ValueError(
                f"specify one of {str(list(l0))[1:-1]} as keyword argument key0"
            )
        key0 = l0[0]
    ...
    if "start_csl" in rbdf.columns and "end_csl" in rbdf.columns:
        ...
        csl_rows = csl_rows.rename(
            {"start_csl": "start", "end_csl": "end"}, axis=1)
    ...
    if "start_psl" in rbdf.columns and "end_psl" in rbdf.columns:
        ...
        psl_rows = psl_rows.rename(
            {"start_psl": "start", "end_psl": "end"}, axis=1)
    ...
    if "start_boot" in rbdf.columns and "end_boot" in rbdf.columns:
        ...
        boot_rows = boot_rows.rename(
            {"start_boot": "start", "end_boot": "end"}, axis=1)
    ...
    all_rows["start_sec"] = all_rows.apply(
        lambda df: (df["start"] - t0).total_seconds() + 1, axis=1
    )
    all_rows["end_sec"] = all_rows.apply(
        lambda df: (df["end"] - t0).total_seconds() + 1, axis=1
    )
