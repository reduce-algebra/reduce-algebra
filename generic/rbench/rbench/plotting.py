...
def plot_scatter1(rbdf,
    ...
    if "cpu_" in c:
        ...
        x_only = rbdf.loc[~rbdf.index.isin(rbdf.dropna(
            subset=[(y, c)]).index)].dropna(subset=[(x, c)])
        _scatter(x_only,
                 x=(x, c),
                 y=(y, cx),
                 ax=ax,
                 color=colorx or "k",
                 **keywords)
        y_only = rbdf.loc[~rbdf.index.isin(rbdf.dropna(
            subset=[(x, c)]).index)].dropna(subset=[(y, c)])
        _scatter(y_only,
                 x=(x, cx),
                 y=(y, c),
                 ax=ax,
                 color=colorx or "k",
                 **keywords)
        neither = rbdf.loc[~rbdf.index.
                           isin(rbdf.dropna(subset=[(y, c)]).index)]
        neither = neither.loc[~neither.index.
                              isin(neither.dropna(subset=[(x, c)]).index)]
        _scatter(neither,
                 x=(x, cx),
                 y=(y, cx),
                 ax=ax,
    ...
    high = max(high_x, high_y)
    ax.axline([low, low], [high, high], c="k", linewidth=0.1)
    for val in set(x_only[(y, cx)].to_list()).union(
            set(neither[(y, cx)].to_list())):
        ax.axline([low, val], [high, val], c="r", linewidth=0.5)
    for val in set(y_only[(x, cx)].to_list()).union(
            set(neither[(x, cx)].to_list())):
...
def plot_schedule(
    ...
    **keywords,
    ...
    all_rows["end_sec"] = all_rows.apply(lambda df:
                                         (df["end"] - t0).total_seconds() + 1,
                                         axis=1)
